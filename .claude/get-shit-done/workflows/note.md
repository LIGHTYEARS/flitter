...
- **Global scope**: `/Users/bytedance/.oh-my-coco/studio/flitter/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/bytedance/.oh-my-coco/studio/flitter/.claude/notes/`)
...
2. Glob `/Users/bytedance/.oh-my-coco/studio/flitter/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/bytedance/.oh-my-coco/studio/flitter/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/bytedance/.oh-my-coco/studio/flitter/.claude/notes/` — works in any directory
