...
if [ "$ANTIGRAVITY_AGENT" = "1" ]; then
  ...
  # Running inside Trae CLI — skip claude for independence
  SELF_CLI="claude"
else
...
Commit:
```bash
node "/Users/bytedance/.oh-my-coco/studio/flitter/.trae/get-shit-done/bin/gsd-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
