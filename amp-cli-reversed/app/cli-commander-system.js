// Module: cli-commander-system
// Original: segment1[2026951:2182962]
// Type: Scope-hoisted
// Exports: PC0, $IT, kC0, xC0, fC0, IC0, gC0, $C0, EC0, CC0, MC0, DC0, urT, MA, wC0, BC0, NC0, WC0, qC0, zC0
// Category: cli

fter--separator ");throw Error("
Missing command or URL.Usage: amp mcp add < name > < command | url > [args...]
")}if(t.length===1&&t[0]==="--")throw Error("
No command provided after--separator ");let s=t[0];if(s.startsWith("--")&&s!=="--")throw Error(`Unknown option '${s}'. Commands cannot start with dashes`);let A;if(/^https?:\/\//.test(s)){if(t.length>1)throw Error("
Cannot mix URL with additional command arguments ");if(r.env&&r.env.length>0)throw Error("
Environment variables cannot be used with URL - based MCP servers.Use--header instead ");let n=xC0(r.header||[]);A={url:s,headers:Object.keys(n).length>0?n:void 0}}else A=PC0(t,r);let l=await i.get("
mcpServers ",c)??{};if(l[e])throw Error(`MCP server "
$ {
  e
}
" already exists`);l[e]=A,await i.set("
mcpServers ",l,c);let o=c==="
workspace "?i.getWorkspaceSettingsPath():i.getSettingsFilePath();process.stdout.write(`Added amp.mcpServers.${e} to ${o}
`),process.exit(0)}catch(c){process.stderr.write((c instanceof Error?c.message:String(c))+`
`),process.exit(1)}}),a.command("list").summary("List all MCP server configurations").description(`
...
`).option("--json","Output as JSON").action(async(e,t)=>{try{let r=(await R(t.optsWithGlobals())).settings,h=await r.get("mcpServers","global")??{},i=await r.get("mcpServers","workspace")??{},c=[];for(let[s,A]of Object.entries(h))c.push({name:s,type:"url"in A?"url":"command",source:"global",spec:A});for(let[s,A]of Object.entries(i))c.push({name:s,type:"url"in A?"url":"command",source:"workspace",spec:A});if(e.json)process.stdout.write(JSON.stringify(c,null,2)+`
`);else if(c.length===0)process.stdout.write(`
No MCP servers configured.
`);else{let s=c.filter((n)=>n.source==="workspace"),A=c.filter((n)=>n.source==="global"),l=(n)=>{if("url"in n.spec)return n.spec.url;let p=n.spec.command;if(n.spec.args&&n.spec.args.length>0)p+=" "+n.spec.args.join(" ");return p},o=Math.max(...c.map((n)=>n.name.length));if(s.length>0){process.stdout.write(`
Workspace.amp / settings.json `);for(let n of s){let p=l(n);process.stdout.write(`
$ {
  n.name.padEnd(o)
}
$ {
...
`)}if(A.length>0){process.stdout.write(`
Global~/.config/amp / settings.json `);for(let n of A){let p=l(n);process.stdout.write(`
$ {
  n.name.padEnd(o)
}
$ {
...
`)}process.stdout.write(`
`)}}process.exit(0)}catch(r){process.stderr.write((r instanceof Error?r.message:String(r))+`
`),process.exit(1)}}),a.command("remove <name>").summary("Remove an MCP server configuration").description(`
...
amp mcp remove postgres`).action(async(e,t,r)=>{let h;try{if(h=(await R(r.optsWithGlobals())).settings,!e||e.trim()==="")throw Error("Invalid server name: name cannot be empty");let i="workspace",c=await h.get("mcpServers",i)??{};if(!c[e]){if(i="global",c=await h.get("mcpServers",i)??{},!c[e])throw Error(`
MCP server "${
e
}
" does not exist`)}if(delete c[e],Object.keys(c).length===0)await h.delete("
mcpServers ",i);else await h.set("
mcpServers ",c,i);let s=i==="
workspace "?h.getWorkspaceSettingsPath():h.getSettingsFilePath();process.stdout.write(`Removed amp.mcpServers.${e} from ${s}
`),process.exit(0)}catch(i){process.stderr.write((i instanceof Error?i.message:String(i))+`
`),process.exit(1)}}),a.addCommand(uC0(async(e)=>{let t=await R(e);return{secretStorage:t.secretStorage,settings:t.settings}})),a.command("doctor [name]").summary("Check MCP server status").description(`
Wait
for MCP service initialization and display the status of configured servers.

If[name] is provided, only show status
for that specific server.
`).action(async(e,t,r)=>{let{mcpDoctor:h}=await Promise.resolve().then(()=>(sUR(),c5T)),i=await R(r.optsWithGlobals());if(!i.getThreadDeps)throw Error("Internal error: getThreadDeps not available for doctor command");await h(r,i.getThreadDeps,e)}),a.command("approve <name>").summary("Approve a workspace MCP server").description(`
Approve a workspace MCP server
for execution.
...
amp mcp approve project - mcp`).action(async(e,t,r)=>{try{let h=await R(r.optsWithGlobals());if(!h.getThreadDeps)throw Error("Internal error: getThreadDeps not available for approve command");let i=await h.getThreadDeps(r);try{await i.mcpService.approveWorkspaceServer(e),process.stdout.write(`
Approved MCP server "${
e
}
"
`),process.exit(0)}finally{await i.asyncDispose()}}catch(h){process.stderr.write((h instanceof Error?h.message:String(h))+`
`),process.exit(1)}})}function PC0(T,R){if(R.header&&R.header.length>0)throw Error("HTTP headers cannot be used with command-based MCP servers. Use --env instead");let a,e,t=T.indexOf("--");if(t!==-1){let h=T.slice(t+1);if(h.length===0)throw Error("No command provided after -- separator");a=h[0],e=h.length>1?h.slice(1):void 0}else{if(R.env&&R.env.length>0&&T.length===0)throw Error("Environment variables provided but no command specified. Use: amp mcp add <name> --env KEY=VAL -- <command>");a=T[0],e=T.length>1?T.slice(1):void 0}let r=kC0(R.env||[]);return{command:a,args:e,env:Object.keys(r).length>0?r:void 0}}function $IT(T,R){return R.concat(T)}function kC0(T){let R={};for(let a of T){let e=a.indexOf("=");if(e<=0)throw Error(`
Invalid--env format: $ {
  a
}.Expected KEY = VALUE`);let t=a.slice(0,e),r=a.slice(e+1);if(t.trim()==="")throw Error(`
Invalid--env format: $ {
  a
}.Expected KEY = VALUE`);if(r==="")throw Error(`
Invalid--env format: $ {
  a
}.Expected KEY = VALUE`);R[t]=r}return R}function xC0(T){let R={};for(let a of T){let e=a.indexOf("=");if(e<=0)throw Error(`
Invalid--header format: $ {
  a
}.Expected KEY = VALUE`);let t=a.slice(0,e),r=a.slice(e+1);if(r==="")throw Error(`
Invalid--header format: $ {
  a
}.Expected KEY = VALUE`);R[t]=r}return R}function fC0(T){return"url"in T}function IC0(T){let R=T.clientId??T.client_id,a=T.authUrl??T.authorizationUrl??T.auth_url??T.authorization_url,e=T.tokenUrl??T.token_url;if(!R||!a||!e)return;return{clientId:R,clientSecret:T.clientSecret??T.client_secret,authUrl:a,tokenUrl:e,scopes:T.scopes,redirectUrl:T.redirectUrl??T.redirect_url}}function gC0(T){if(!fC0(T))return T;let R={url:T.url};if(T.headers)R.headers=T.headers;if(T.transport)R.transport=T.transport;if(T.oauth){let a=IC0(T.oauth);if(a)R.oauth=a}return R}function $C0(T){return Object.fromEntries(Object.entries(T).map(([R,a])=>[R,gC0(a)]))}async function EC0(T){let R;if(T.trim().startsWith("{
"))R=T;else{let{readFile:t}=await import("fs/promises");try{R=await t(T,"utf-8")}catch(r){throw new GR(`
Failed to read--mcp - config file: $ {
  r instanceof Error ? r.message : String(r)
}
`)}}let a;try{a=JSON.parse(R)}catch(t){throw new GR(`
Failed to parse--mcp - config as JSON: $ {
  t instanceof Error ? t.message : String(t)
}
`)}let e;try{e=dC0.parse(a)}catch(t){if(t instanceof K.ZodError){let r=t.issues.map((h)=>`
$ {
  h.path.join(".")
}: $ {
  h.message
...
`)}throw new GR(`
Failed to validate MCP server configuration: $ {
  String(t)
}
`)}return $C0(e)}function CC0(T,R){return{...T,async get(a,e){if(a!=="mcpServers")return T.get(a);let t=Object.fromEntries(Object.entries(R).map(([i,c])=>[i,{...c,_target:"flag"}])),r=await T.get(a,"global"),h=await T.get(a,"workspace");if(e==="global")return cY({global:r,override:t});else if(e==="workspace")return h;return cY({global:r,workspace:h,override:t})},async keys(){let a=await T.keys();if(!a.includes("mcpServers"))a.push("mcpServers");return a}}}function MC0(T,R){T.onUntrustedWorkspaceServer=(a)=>{let e=R.getSettingsFilePath();LC0.write(oR.yellow(`
Warning: Untrusted MCP server '${a}'
found in workspace settings.
To fix: Add "amp.mcpTrustedWorkspaces": ["${
  R.getWorkspaceRootPath()
}
"] to ${e}
`))}}class ZZT{mcpService;workspaceFolder;pendingServers=new Set;notificationTimeout;_pendingServersSubject=new f0([]);serversSubscription;pendingServers$=this._pendingServersSubject;constructor(T,R){this.mcpService=T,this.workspaceFolder=R,this.setupListener()}setupListener(){this.mcpService.onUntrustedWorkspaceServer=(R,a)=>{if(this.pendingServers.add(R),this.notificationTimeout)clearTimeout(this.notificationTimeout);this.notificationTimeout=setTimeout(()=>{this._pendingServersSubject.next(Array.from(this.pendingServers)),this.notificationTimeout=void 0},700)};let T=this.mcpService.trustStore.changes?this.mcpService.trustStore.changes.pipe(Y3(void 0)):AR.of(void 0);this.serversSubscription=v3(this.mcpService.servers,T).pipe(L9(([R,a])=>Q9(async()=>{let e=new Set;for(let t of R)if(t.requiresApproval&&t.specHash){if(!await this.mcpService.trustStore.hasEntry?.({serverName:t.name,specHash:t.specHash}))e.add(t.name)}return e}))).subscribe((R)=>{if(this.pendingServers.size===0)return;let a=!1,e=Array.from(this.pendingServers);for(let t of e)if(!R.has(t))this.pendingServers.delete(t),a=!0;if(a)this._pendingServersSubject.next(Array.from(this.pendingServers))})}async trustAlways(){try{await this.mcpService.allowWorkspace(this.workspaceFolder);let T=Array.from(this.pendingServers);for(let R of T)try{await this.mcpService.approveWorkspaceServer(R)}catch(a){J.error("Failed to approve MCP server",{serverName:R,error:a})}}catch(T){J.error("Failed to allow workspace",{error:T})}this.clear()}async trustOnce(T){try{await this.mcpService.approveWorkspaceServer(T),this.pendingServers.delete(T),this._pendingServersSubject.next(Array.from(this.pendingServers))}catch(R){J.error("Failed to approve MCP server",{serverName:T,error:R})}}async deny(){let T=Array.from(this.pendingServers);for(let R of T)try{await this.mcpService.denyWorkspaceServer(R)}catch(a){J.error("Failed to deny MCP server",{serverName:R,error:a})}this.clear()}clear(){this.pendingServers.clear(),this._pendingServersSubject.next([])}dispose(){if(this.notificationTimeout)clearTimeout(this.notificationTimeout);if(this.serversSubscription)this.serversSubscription.unsubscribe()}}function DC0(T){let R=["private","unlisted","public","workspace"];if(T?.team?.groups&&T.team.groups.length>0)R.push("group");return R}function urT(T,R=null){let a=T!==null&&typeof T==="object"&&"visibility"in T?T.visibility:void 0;if(!a||typeof a!=="string")return;let e=DC0(R);if(e.includes(a))return a;if(a==="group"&&!e.includes("group")){if(R?.team?.groups!==void 0)return Error(`
Group visibility is not available.You are not a member of any group in this workspace.
`);return Error(`
Group visibility is not available.
`)}return Error(`
Invalid visibility.Must be one of: $ {
  e.join(", ")
}
`)}function MA(T){switch(T){case"public":return{visibility:"public_discoverable"};case"unlisted":return{visibility:"public_unlisted"};case"workspace":return{visibility:"thread_workspace_shared"};case"private":return{visibility:"private",sharedGroupIDs:[]};case"group":return{visibility:"private",shareWithAllCreatorGroups:!0}}}async function wC0(T,R,a){let e=mLT(T);if(!e.success)throw Error(`
Invalid permission format: $ {
  e.error.message
}
`);if(e.data.length===0)throw Error("No permission entry provided");let t=fj(e.data);if(!t.success){let i=t.error.issues.map((c)=>c.message).join(", ");throw Error(`
Invalid permission entry: $ {
  i
}
`)}let r=await R.get("permissions",a)??[],h=[e.data[0],...r];await R.set("permissions",h,a)}async function BC0(T){try{let R=T.args.map((a)=>{if(/[\s/\\*?[\]{}()"]/.test(a))return`
"${a}"
`;return a}).join(" ");await wC0(R,T.settings,T.scope),T.exit(0)}catch(R){let a=R instanceof Error?R.message:"Unknown error";T.stderr.write(`
Error: $ {
  a
}
`),T.exit(1)}}async function NC0(T){let R=T.builtinOnly?PN:await T.settings.get("permissions",T.scope)??[];if(T.json)T.stdout.write(JSON.stringify(R,null,2));else{let a=J2(R);T.stdout.write(a),T.stdout.write(`
`)}T.exit(0)}function WC0(T){let R={};for(let a=0;a<T.length;a++){let e=T[a];if(e&&e.startsWith("--")){let t=e.slice(2),r=T[a+1];if(r&&!r.startsWith("--"))R[t]=r,a++}}return R}function qC0(T){if(T.action==="ask")return 1;if(T.action==="reject")return 2;return 0}async function zC0(T){let R=T.json?UC0:HC0;try{let a=WC0(T.remainingArgs),e=await T.settings.get("permissions",T.scope)??[],t=fj(e);if(!t.success){let i={success:!1,error:"Invalid permissions configuration",details:t.error.issues.map((c)=>({path:c.path.join("."),message:c.message,code:c.code}))};R.outputError(T,a,i),T.exit(1);return}let r=yLT(t.data),h=await CD(T.toolName,a,r,T.context,LD,T.threadId,"user");if(!h.matchedEntry)h=await CD(T.toolName,a,PN,T.context,LD,T.threadId,"built-in");R.outputResult(T,a,h),T.exit(qC0(h))}catch(a){let e={success:!1,error:a instanceof Error?a.message:"Unknown error"};R.outputError(T,{},e),T.exit(1)}}function FC0(T,R){let a=T.command("permissions").alias("permission").summary("Manage permissions").description(`
Configure and test tool permissions to control what actions the AI agent can take.

See $ {
  oR.underline("https://ampcode.com/manual#permissions")
}
for more details`).enablePositionalOptions();GC0(a,R),KC0(a,R),VC0(a,R),XC0(a,R)}function GC0(T,R){T.command("list").alias("ls").summary("List permissions").description("List user-configured permissions.").option("--json","output as JSON").option("--builtin","output only builtin permissions").option("-w, --workspace","List workspace permissions instead of global").action(async(a,e)=>{let t=await R(e),r=e.optsWithGlobals();await NC0({builtinOnly:r.builtin??!1,settings:t.settings,exit:process.exit.bind(process),json:r.json,stdout:process.stdout,scope:r.workspace?"workspace":"global"})})}function KC0(T,R){T.command("test").summary("Test permissions").description("Test permissions against configured rules without running the tool.").argument("<tool-name>","Name of the tool to test").option("-c, --context <context>","Execution context (thread or subagent)").option("-t, --thread-id <id>","Thread ID for evaluation").option("--json","output as JSON").option("-q, --quiet","suppress output, only use exit status").option("-w, --workspace","Test against workspace permissions instead of global").allowUnknownOption(!0).action(async(a,e,t)=>{let r=await R(t),h=()=>!1;await zC0({toolName:a,context:e.context,threadId:e.threadId,json:e.json,settings:r.settings,remainingArgs:t.args,stdout:e.quiet?{write:h}:process.stdout,stderr:process.stderr,exit:process.exit.bind(process),scope:e.workspace?"workspace":"global"})}).addHelpText("after",["Example: ","  # Test permissions of Amp trying to use bash","  amp permissions test Bash --cmd ls","  # ...when running in a subagent","  amp permissions test --context subagent Bash --cmd ls","","See output of:","","  amp tools show","","for all tool schemas."].join(`
`))}function VC0(T,R){T.command("edit").summary("Edit permissions").description("Edit permissions interactively in your $EDITOR or from stdin.").option("-w, --workspace","Edit workspace permissions instead of global").action(async(a,e)=>{let t=await R(e);await MQT({settings:t.settings,stdin:process.stdin,stdout:process.stdout,stderr:process.stderr,exit:process.exit.bind(process),resolveEditor:eB,scope:a.workspace?"workspace":"global"})})}function XC0(T,R){T.command("add").summary("Add permission rule").description("Add a permission rule to the beginning of the list. Rules are evaluated in order, so newer rules take precedence.").argument("<action>","Permission action: allow, reject, ask, or delegate").option("--to <program>","Program to delegate decision to for delegate action").option("--context <type>","Limit to specific context (thread or subagent)").option("-w, --workspace","Add to workspace permissions instead of global").argument("<tool>",'Tool name (supports globs like "mcp__playwright__*")').allowUnknownOption(!0).action(async(a,e,t,r)=>{let h=await R(r);await BC0({args:r.parent?.args.slice(1)??[],settings:h.settings,stderr:process.stderr,exit:process.exit.bind(process),scope:t.workspace?"workspace":"global"})})}async function ZC0(T,R,a){let e=(await(await ep({configService:R},a)).messages.create({model:mb,max_tokens:300,system:"You are a classifier that answers yes/no questions. You must use the provided tool to give your answer with reasoning.",messages:[{role:"user",content:T}],tools:[{name:"answer_question",description:"Provide your answer to the yes/no question with reasoning",input_schema:{type:"object",properties:{probability_yes:{type:"number",description:'Probability that the answer is "yes", from 0 to 1. Use 0 for definitely no, 0.5 for uncertain, 1 for definitely yes.'},reasoning:{type:"string",description:"Brief (2-sentence max) explanation of why you gave this probability."}},required:["probability_yes","reasoning"]}}],tool_choice:{type:"tool",name:"answer_question"}},{headers:{"anthropic-beta":"interleaved-thinking-2025-05-14"}})).content.find((c)=>c.type==="tool_use");if(!e||e.type!=="tool_use")throw Error("Expected tool use response from Claude");let t=e.input,r=t.probability_yes??0.5,h=t.reasoning??"No reasoning provided",i;if(r>=0.8)i="yes";else if(r<0.2)i="no";else i="uncertain";return{result:i,probability:r,reason:h}}class nQ{pluginExecutorKind="unknown";showToast=null;appendToThreadHandler=null;notifyForwarder=null;showOpenedURLToast=null;showInputDialog=null;showConfirmDialog=null;configService;constructor(T){this.configService=T.configService}setNotifyForwarder(T){this.notifyForwarder=T}async notify(T){if(this.notifyForwarder)try{await this.notifyForwarder(T);return}catch(R){J.error("plugin notify forwarder failed, falling back to local notification",{error:R})}if(this.showToast)this.showToast(T);else J.debug("plugin notification (TUI not loaded): %s",T)}async open(T){let R=QC0(),a=R==="darwin"?"open":R==="win32"?"start":"xdg-open";if(await new Promise((e,t)=>{YC0(`
$ {
  a
}
$ {
...
`,(r)=>{if(r)t(r);else e()})}),this.showOpenedURLToast)this.showOpenedURLToast(T);else Ay.write(`
Opened URL: $ {
  T
}

`)}async input(T){if(this.showInputDialog)return this.showInputDialog(T);Ay.write(`
Input requested: $ {
  T.title ?? "Input"
}
`),Ay.write(`
Input dialogs are not available outside the TUI

  `);return}async confirm(T){if(this.showConfirmDialog)return this.showConfirmDialog(T);return Ay.write(`
Confirmation requested: $ {
  T.title
}
`),Ay.write(`
Confirm dialogs are not available outside the TUI, defaulting to false

  `),!1}async ask(T){try{return await ZC0(T,this.configService)}catch(R){return Ay.write(`
Error asking AI: $ {
  R instanceof Error ? R.message : String(R)
}

`),{result:"uncertain",probability:0.5,reason:"Error calling AI"}}}async appendToThread(T){if(this.appendToThreadHandler)await this.appendToThreadHandler(T);else throw Error("appendToThread is not available - no active thread")}}function JC0(){let T=null;return{pluginExecutorKind:"remote",setNotifyForwarder(R){T=R},async notify(R){if(J.debug("headless plugin notification: %s",R),!T)return;try{await T(R)}catch(a){J.error("headless plugin notify forwarder failed",{error:a})}},async open(R){throw Error("pluginPlatform.open is not supported in headless mode")},async input(R){throw Error("pluginPlatform.input is not supported in headless mode")},async confirm(R){throw Error("pluginPlatform.confirm is not supported in headless mode")},async ask(R){throw Error("pluginPlatform.ask is not supported in headless mode")}}}function T40(T){T.registerCommand("toggle-skills-count",{title:"toggle skills count",category:"CLI",description:'Toggle whether "N skills" is shown in the prompt bar'},async(R)=>{let a=(await T.configuration.get())["internal.cli.showSkillsCountInPromptBar"]===!1;await T.configuration.update({["internal.cli.showSkillsCountInPromptBar"]:a},"global"),await R.ui.notify(a?"Showing skills count in prompt bar":"Hiding skills count in prompt bar")})}function t40(T,R){let a=T.command("plugins",{hidden:!0}).alias("plugin").description("Plugin management commands").action(()=>{a.help()});return a.command("list").alias("ls").description("List plugins found in .amp/plugins/").action(async(e,t)=>{let r=await R(t);try{if(r.pluginService===V5T)Ig.write(oR.dim(`
Plugins are disabled(PLUGINS = off).Set PLUGINS = all to enable.
`));else{let h=(s)=>s.length>0&&s.every((A)=>A.status!=="loading"),i=await m0(r.pluginService.plugins.pipe(da(h),KS(100))),c=process.cwd();for(let s of i){let A=I8(s.uri),l=A.scheme==="file"?A.fsPath.startsWith(c+"/")?vIT.relative(c,A.fsPath):A.fsPath:A.toString(),o=s.status==="active"?oR.green("\u2713"):oR.red("\u2717");if(Ig.write(`
$ {
  o
}
$ {
...
`);if(s.registeredCommands.length>0)for(let n of s.registeredCommands)Ig.write(oR.dim("  Command: ")+oR.cyan(`
$ {
  n.category
}: $ {
  n.title
...
`)}}}finally{await r.pluginService.dispose(),await r.asyncDispose()}process.exit(0)}),a.command("exec").argument("<plugin>","Plugin file path (e.g., .amp/plugins/foo.ts)").argument("<event>","Event name to send (e.g., tool.result)").option("--data <json>","JSON event data","{
}
").description("Execute a plugin with a given event").action(async(e,t,r,h)=>{let i;try{i=JSON.parse(r.data)}catch{py.write(oR.red(`
Error: --data must be valid JSON `)),process.exit(1)}if(!a40(e))py.write(oR.red(`
Error: plugin file not found: $ {
  e
}
`)),process.exit(1);let c=vIT.resolve(e),s=d0(zR.parse(process.env.AMP_URL??Lr)),A=new vaT(zR.file(c),{onStderr:(l)=>py.write(l),onRequest:{"ui.notify":async({message:l})=>{py.write(`
Notification: $ {
  l
}

`);return},"system.open":async({url:l})=>{let o=e40();R40(`
$ {
  o === "darwin" ? "open" : o === "win32" ? "start" : "xdg-open"
}
$ {
  JSON.stringify(l)
}
...
`);return},"client.info":async(l)=>({ampURL:s,executorKind:"unknown"})}});try{await A.start(),await A.emitEvent(t,i),await new Promise((l)=>setTimeout(l,2000))}catch(l){py.write(oR.red(`
Error: $ {
  l instanceof Error ? l.message : String(l)
}
`)),process.exit(1)}finally{await A.dispose()}process.exit(0)}),a}async function r40(T,R,a){let e=R?`
Input received on stdin: \`\`\`
${R}
\`\`\`

${T}`: T, t = (await Hs()).trees?.[0]?.repository?.url;
...
`)), process.exit(1);
        let s = !1;
        for (let A of c)
          if (A.success) W8.write(oR.green("\u2713 ") + `Installed ${oR.bold(A.skillName)}
  ...
  async runInTerminal(T, R) {
    ...
    try {
      await new Promise((e, t) => {
        ...
        r.on("exit", (h) => {
          if (h === 0) e();
          else t(Error(`Command failed with exit code ${h}`))
        }), r.on("error", (h) => {
          t(h)
        })
      })
...
function B40(T, R) {
  ...
  if (e = R.d.length, e < 32) a = Math.ceil(e / 3), t = (1 / QH(4, a)).toString();
  else a = 16, t = "2.3283064365386962890625e-10";
...
function Q0(T, R, a, e) {
  var t, r, h, i, c, s, A, l, o, n = T.constructor;
  T: if (R != null) {
    if (l = T.d, !l) return T;
    for (t = 1, i = l[0]; i >= 10; i /= 10) t++;
    if (r = R - t, r < 0) r += s9, h = R, A = l[o = 0], c = A / Ka(10, t - h - 1) % 10 | 0;
    else if (o = Math.ceil((r + 1) / s9), i = l.length, o >= i)
      if (e) {
        for (; i++ <= o;) l.push(0);
    ...
    if (e = e || R < 0 || l[o + 1] !== void 0 || (h < 0 ? A : A % Ka(10, t - h - 1)), s = a < 4 ? (c || e) && (a == 0 || a == (T.s < 0 ? 3 : 2)) : c > 5 || c == 5 && (a == 4 || e || a == 6 && (r > 0 ? h > 0 ? A / Ka(10, t - h) : 0 : l[o - 1]) % 10 & 1 || a == (T.s < 0 ? 8 : 7)), R < 1 || !l[0]) {
      if (l.length = 0, s) R -= T.e + 1, l[0] = Ka(10, (s9 - R % s9) % s9), T.e = -R || 0;
      else l[0] = T.e = 0;
      return T
    }
    if (r == 0) l.length = o, i = 1, o--;
    else l.length = o + 1, i = Ka(10, s9 - r), l[o] = h > 0 ? (A / Ka(10, t - h) % Ka(10, h) | 0) * i : 0;
    if (s)
      for (;;)
        if (o == 0) {
  ...
  if (g9) {
    if (T.e > n.maxE) T.d = null, T.e = NaN;
    else if (T.e < n.minE) T.e = 0, T.d = [0]
  }
  return T
}
...
function zs(T, R, a) {
  ...
  var e, t = T.e,
    ...
    h = r.length;
  if (R) {
    if (a && (e = a - h) > 0) r = r.charAt(0) + "." + r.slice(1) + dl(e);
    else if (h > 1) r = r.charAt(0) + "." + r.slice(1);
    r = r + (T.e < 0 ? "e" : "e+") + T.e
  } else if (t < 0) {
...
function _Q(T, R) {
  ...
  for (;;) {
    if (r = Q0(r.times(T), c, 1), a = a.times(++A), i = h.plus(c3(r, a, c, 1)), We(i.d).slice(0, c) === We(h.d).slice(0, c)) {
      ...
      if (R == null)
        if (s < 3 && IS(h.d, c - e, n, s)) o.precision = c += 10, a = r = i = new o(1), A = 0, s++;
        else return Q0(h, o.precision = p, n, g9 = !0);
...
function hA(T, R) {
  ...
  if (b.precision = A += p, a = We(m), e = a.charAt(0), Math.abs(r = _.e) < 1500000000000000) {
    ...
    if (r = _.e, e > 1) _ = new b("0." + a), r++;
    else _ = new b(e + "." + a.slice(1))
  ...
  for (;;) {
    if (h = Q0(h.times(o), A, 1), s = c.plus(c3(h, new b(t), A, 1)), We(s.d).slice(0, A) === We(c.d).slice(0, A)) {
      ...
      if (c = c3(c, new b(n), A, 1), R == null)
        if (IS(c.d, A - p, y, i)) b.precision = A += p, s = h = _ = c3(l.minus(1), l.plus(1), A, 1), o = Q0(_.times(_), A, 1), t = i = 1;
        else return Q0(c, b.precision = u, y, g9 = !0);
...
function VM(T, R) {
  ...
  if (R = R.slice(e, t), R) {
    ...
    for (; e--;) R += "0";
    if (T.d.push(+R), g9) {
      if (T.e > T.constructor.maxE) T.d = null, T.e = NaN;
      else if (T.e < T.constructor.minE) T.e = 0, T.d = [0]
    }
  } else T.e = 0, T.d = [0];
  return T
}
...
function N40(T, R) {
  ...
  if (L40.test(R)) a = 16, R = R.toLowerCase();
  else if (C40.test(R)) a = 2;
  else if (M40.test(R)) a = 8;
  else throw Error(DA + R);
  if (r = R.search(/p/i), r > 0) c = +R.slice(r + 1), R = R.substring(2, r);
  else R = R.slice(2);
...
function lJT(T, R) {
  ...
  if (a = R.divToInt(t), a.isZero()) Zo = e ? 3 : 2;
  else {
...
function krT(T, R, a, e) {
  var t, r, h, i, c, s, A, l, o, n = T.constructor,
    p = a !== void 0;
  if (p)
    if (wr(a, 1, up), e === void 0) e = n.rounding;
    else wr(e, 0, 8);
  else a = n.precision, e = n.rounding;
  if (!T.isFinite()) A = nJT(T);
  else {
    ...
    if (!l[0]) A = p ? "0p+0" : "0";
      ...
      if (p) {
        ...
        for (r -= c; r--;) A += "0";
      else if (r < c) A = A.slice(0, r) + "." + A.slice(r)
    }
...
function X40(T, R) {
  ...
  var a, e = this.precision,
    t = this.rounding,
    r = e + 4;
  if (!T.s || !R.s) a = new this(NaN);
  else if (!T.d && !R.d) a = Cs(this, r, 1).times(R.s > 0 ? 0.25 : 0.75), a.s = T.s;
  else if (!R.d || T.isZero()) a = R.s < 0 ? Cs(this, e, t) : new this(0), a.s = T.s;
  else if (!T.d || R.isZero()) a = Cs(this, r, 1).times(0.5), a.s = T.s;
  else if (R.s < 0) this.precision = r, this.rounding = 1, a = this.atan(c3(T, R, r, 1)), R = Cs(this, r, 1), this.precision = e, this.rounding = t, a = T.s < 0 ? a.minus(R) : a.plus(R);
  else a = this.atan(c3(T, R, r, 1));
  return a
}
...
function J40(T) {
  ...
  for (R = 0; R < r.length; R += 3) {
    ...
    if ((e = T[a]) !== void 0)
      if (nt(e) === e && e >= r[R + 1] && e <= r[R + 2]) this[a] = e;
...
function AJT(T) {
  ...
  function t(r) {
    ...
    if (s.constructor = t, DIT(r)) {
      if (s.s = r.s, g9)
        if (!r.d || r.e > t.maxE) s.e = NaN, s.d = null;
        else if (r.e < t.minE) s.e = 0, s.d = [0];
      else s.e = r.e, s.d = r.d.slice();
    ...
    if (c = typeof r, c === "number") {
      ...
      if (r === ~~r && r < 1e7) {
        ...
        if (g9)
          if (h > t.maxE) s.e = NaN, s.d = null;
          else if (h < t.minE) s.e = 0, s.d = [0];
        else s.e = h, s.d = [r];
        else s.e = h, s.d = [r];
        return
      }
    ...
    if (c === "string") {
      if ((i = r.charCodeAt(0)) === 45) r = r.slice(1), s.s = -1;
      else {
        if (i === 43) r = r.slice(1);
        s.s = 1
      }
  ...
  if (T) {
    if (T.defaults !== !0) {
      ...
      for (R = 0; R < e.length;)
        if (!T.hasOwnProperty(a = e[R++])) T[a] = this[a]
    }
  }
  return t.config(T), t
}
...
function rL0() {
  var T, R, a = new this(0);
  g9 = !1;
  for (T = 0; T < arguments.length;)
    if (R = new this(arguments[T++]), !R.d) {
...
function _L0(T) {
  var R, a, e, t, r = 0,
    h = new this(1),
    i = [];
  if (T === void 0) T = this.precision;
  else wr(T, 1, up);
  if (e = Math.ceil(T / s9), !this.crypto)
    for (; r < e;) i[r++] = Math.random() * 1e7 | 0;
  else if (crypto.getRandomValues) {
    R = crypto.getRandomValues(new Uint32Array(e));
    for (; r < e;)
      if (t = R[r], t >= 4290000000) R[r] = crypto.getRandomValues(new Uint32Array(1))[0];
      else i[r++] = t % 1e7
  } else if (crypto.randomBytes) {
    R = crypto.randomBytes(e *= 4);
    for (; r < e;)
      if (t = R[r] + (R[r + 1] << 8) + (R[r + 2] << 16) + ((R[r + 3] & 127) << 24), t >= 2140000000) crypto.randomBytes(4).copy(R, r);
      else i.push(t % 1e7), r += 4;
    r = e / 4
  } else throw Error(rJT);
...
function AP(T, R) {
  let {
    decimalPlaces: a,
    roundingMode: e
  } = $L0(T, R), t = T % 1 === 0 && !R.alwaysShowCents ? T.toLocaleString("en-US", {
    style: "currency",
    currency: "USD",
...
async function qL0(T) {
  ...
  for (let e of R)
    if (e.status.type === "failed") T.stderr.write(`error connecting to ${e.name}: ${e.status.error.message||"Unknown error"}
`);
...
function aM0(T) {
  ...
  return process.once("SIGINT", a), T.pipe(M$(R), cET((e) => {
    if (e.status === "error") throw Error(`Tool execution error: ${JSON.stringify(e.error)}`);
    else if (e.status === "rejected-by-user") throw Error("Tool execution was rejected by user");
    else if (e.status === "cancelled") throw Error("Tool execution was cancelled")
  }), tN(() => {
...
`)
      ...
      if (typeof P === "string") i.write(P);
      else i.write(JSON.stringify(P, null, 2));
      i.write(`
`), s(0)
    }
  } catch (l) {
...
function mQ(T, R) {
  ...
  if (a.type === "object") a.children = Object.entries(R.properties ?? {}).map(([e, t]) => mQ(T ? `${T}.${e}` : e, t)).filter((e) => e !== null);
  else if (a.type === "array" && R.items) {
...
function sM0(T) {
  ...
  for (let a = 0; a < T.length; a++) {
    ...
    if (e?.startsWith("--")) {
      ...
      if (r && !r.startsWith("--")) {
        if (R[t])
          if (Array.isArray(R[t])) R[t].push(r);
          else R[t] = [R[t], r];
        else R[t] = r;
        a++
      }
...
function jM0(T, R, a) {
  let e = new Map;
  for (let i of T)
    if (i.type === "toolResult" && xM0(i.toolUse.name)) e.set(i.toolUse.id, i.toolResult.run);
  let t = (i, c) => {
      let s = e.get(i);
...
function grT(T, R) {
  let a = [],
    e = new Set;
  if (R?.includeSubagentMessages)
    for (let [h] of T.messages.entries()) e.add(h);
  else
    for (let [h, i] of T.messages.entries())
      if (!i.parentToolUseId) e.add(h);
  let t = new Map,
    r = new Set;
  for (let [h, i] of T.messages.entries()) {
    if (!e.has(h)) continue;
      ...
      case "assistant": {
        ...
        for (let A of i.content) {
          ...
          for (let o = h + 1; o < T.messages.length; o++) {
            ...
            let n = T.messages[o];
            if (!n) continue;
            for (let p of n.content)
              if (p.type === "tool_result" && p.toolUseID === A.id) {
                l = !0;
                break
              } if (l) break
          }
...
function EM0(T) {
  ...
  for (let h of R.slice(a + 1)) {
    ...
    for (let i of h.content)
      if (i.type === "tool_result") t.set(i.toolUseID, i.run)
  }
  let r = 0;
...
class gJT {
  ...
  optimisticSubmit(T) {
    ...
    if (RD0(this.currentAgentLoopState)) this.pendingQueuedSubmits.push(R);
    else this.pendingMessageSubmits.push(R);
    return this.buildProjection()
  }
  optimisticEdit(T) {
    let R = this.buildProjection(),
...
class vJT {
    ...
    T.messages.slice(0, R).forEach((r) => {
      if (r.role === "user") {
        for (let h of r.content)
          if (h.type === "tool_result") a.add(h.toolUseID)
      } else
        for (let h of r.content)
          if (h.type === "tool_use") a.add(h.id)
    });
    let e = new Set,
  ...
  syncApprovalRequests(T) {
    ...
    for (let t of [...this.toolRunner.sentApprovalRequests])
      if (!e.has(t)) this.toolRunner.sentApprovalRequests.delete(t);
    for (let t of a) {
      let r = pS(t);
...
function tD0(T) {
  let R = new f0(T.thread),
    ...
    h = (i) => {
      ...
      if (!i || !i.text && i.images.length === 0) delete c.draft, delete c.autoSubmitDraft;
      else {
        let s = [];
        if (i.text) s.push({
          type: "text",
...
async function SJT(T, R) {
  let {
    thread: a,
    threadMeta: e
  } = R, t = a?.id ?? R.threadID ?? Eh(), r = ct.get(t), h = r?.thread ?? await T.threadService.get(t);
  if (a && !h) {
...
function uQ(T) {
  ...
  let R = T;
  if (Array.isArray(R.type)) return R.type.join(" | ");
  if (R.type === "array") return `array<${
uQ(R.items)}
>`;
  if (typeof R.type === "string") return R.type;
  if (Array.isArray(R.enum) && R.enum.length > 0) return `enum(${
R.enum.map(String).join(", ")})`;
  let a = R.anyOf ?? R.oneOf;
...
async function EJT(T) {
  ...
  process.stdout.write(`${
JSON.stringify({
type:"tools",agentMode:R,count:a.length,tools:a},null,2)}
`)
}
...
async function kD0(T, R) {
  return new Promise((a) => {
    ...
    h = T.subscribe({
      ...
      v0(`drain-until-complete timed out after ${
R}
ms (sawInferenceRunning=${
t})`), i(!1)
    }, R)
  })
}
  ...
  return {
    threadPool: new SrT({
      ...
      validateThreadOwnership: async (t) => {
        if (!t || !Vt(t)) throw new GR(`Invalid thread ID: ${
t}
`, 1);
        await NIT(t, T)
      }
...
`,
  ...
  if (T.threadId) await a.switchThread(T.threadId);
  else await a.createThread();
  return {
    threadPool: a
  }
}
