// Module: html-sanitizer-repl
// Original: segment1[2182962:2238928]
// Type: Scope-hoisted
// Exports: vD0, jD0, CD0, MJT, LD0, MD0, KZ0, BJT, NJT, rf, ki, qD0, zD0, KIT, yp, Pp, FD0, VIT, GD0, KD0
// Category: cli
...
class LJT {
  ...
  props(T) {
    ...
    for (let t of e)
      if (!t.startsWith("_") && t !== "constructor" && t !== "key") try {
        let r = R[t];
...
` && T[R] !== "\r") throw new A8("invalid escape: only line-ending whitespace may be escaped", {
        ...
        R = ws(T, R)
      } else if (s in XIT) i += XIT[s];
      else throw new A8("unrecognized escape sequence", {
        toml: T,
        ptr: r
      });
      c = R
...
function ErT(T, R, a, e, t) {
  ...
  if (r === "[" || r === "{") {
    ...
    if (a) {
      if (s = ws(T, s), T[s] === ",") s++;
      else if (T[s] !== a) throw new A8("expected comma or end of structure", {
        toml: T,
        ptr: s
      })
    }
...
function r70(T, R, a, e) {
  ...
  R++;
  while ((h = T[R++]) !== "}" && h)
    if (h === ",") throw new A8("expected value, found comma", {
      toml: T,
      ptr: R - 1
    });
    else if (h === "#") R = eW(T, R);
...
function h70(T, R, a, e) {
  ...
  R++;
  while ((r = T[R++]) !== "]" && r)
    if (r === ",") throw new A8("expected value, found comma", {
      toml: T,
      ptr: R - 1
    });
    else if (r === "#") R = eW(T, R);
...
function YIT(T, R, a, e) {
  ...
  if (c.d) return null;
  if (c.d = !0, e === 1) t = i ? t[h] : t[h] = {};
  else if (e === 0 && i) return null;
  return [h, t, c.c]
}
...
class TTR {
  ...
  stateNumericDecimal(T, R) {
    while (R < T.length) {
      let a = T.charCodeAt(R);
      if (kQ(a)) this.result = this.result * 10 + (a - F3.ZERO), this.consumed++, R++;
      else return this.emitNumericEntity(a, 2)
    }
    return -1
  }
  emitNumericEntity(T, R) {
    ...
    if (T === F3.SEMI) this.consumed += 1;
    else if (this.decodeMode === Go.Strict) return 0;
  ...
  stateNamedEntity(T, R) {
    let {
      decodeTree: a
    } = this, e = a[this.treeIndex], t = (e & jt.VALUE_LENGTH) >> 14;
    while (R < T.length) {
...
` && T[r] !== "\r"; r++) c += T[r];
      ...
      if (!iw0(c)) {
        let l;
        if (c.trim().length === 0) l = "Invalid space after '<'.";
        else l = "Tag '" + c + "' is an invalid name.";
      ...
      if (r = s.index, A[A.length - 1] === "/") {
        ...
        let o = tgT(A, R);
        if (o === !0) e = !0;
        else return ma(o.err.code, o.err.msg, ft(T, l + o.err.line))
      } else if (i)
        if (!s.tagClosed) return ma("InvalidTag", "Closing tag '" + c + "' doesn't have proper closing.", ft(T, r));
        else if (A.trim().length > 0) return ma("InvalidTag", "Closing tag '" + c + "' can't have attributes or invalid starting.", ft(T, h));
      else if (a.length === 0) return ma("InvalidTag", "Closing tag '" + c + "' has not been opened.", ft(T, h));
      else {
        let l = a.pop();
        if (c !== l.tagName) {
        ...
        if (t === !0) return ma("InvalidXml", "Multiple possible root nodes found.", ft(T, r));
        else if (R.unpairedTags.indexOf(c) !== -1);
        else a.push({
          tagName: c,
          tagStartPos: h
        });
  ...
  if (!e) return ma("InvalidXml", "Start tag expected.", 1);
  else if (a.length == 1) return ma("InvalidTag", "Unclosed tag '" + a[0].tagName + "'.", ft(T, a[0].tagStartPos));
  else if (a.length > 0) return ma("InvalidXml", "Invalid '" + JSON.stringify(a.map((r) => r.tagName), null, 4).replace(/\r?\n/g, "") + "' found.", {
    line: 1,
    col: 1
  });
  return !0
...
function agT(T, R) {
  ...
  for (; R < T.length; R++)
    if (T[R] == "?" || T[R] == " ") {
      ...
      if (R > 5 && e === "xml") return ma("InvalidXml", "XML declaration allowed only at the start of the document.", ft(T, R));
      else if (T[R] == "?" && T[R + 1] == ">") {
        R++;
        break
      } else continue
...
function tgT(T, R) {
  ...
  for (let t = 0; t < a.length; t++) {
    if (a[t][1].length === 0) return ma("InvalidAttr", "Attribute '" + a[t][2] + "' has no space in starting.", vg(a[t]));
    else if (a[t][3] !== void 0 && a[t][4] === void 0) return ma("InvalidAttr", "Attribute '" + a[t][2] + "' is without value.", vg(a[t]));
    else if (a[t][3] === void 0 && !R.allowBooleanAttributes) return ma("InvalidAttr", "boolean attribute '" + a[t][2] + "' is not allowed.", vg(a[t]));
    let r = a[t][2];
    if (!hw0(r)) return ma("InvalidAttr", "Attribute '" + r + "' is an invalid name.", vg(a[t]));
    if (!e.hasOwnProperty(r)) e[r] = 1;
    else return ma("InvalidAttr", "Attribute '" + r + "' is repeated.", vg(a[t]))
  }
  return !0
}
...
class rTR {
      ...
      for (; R < T.length; R++)
        if (T[R] === "<" && !r) {
          if (t && P_(T, "!ENTITY", R)) {
            ...
            R = i
          } else if (t && P_(T, "!ATTLIST", R)) R += 8;
          else if (t && P_(T, "!NOTATION", R)) {
            R += 9;
            let {
              index: i
  ...
  readEntityExp(T, R) {
    ...
    if (jg(a), R = or(T, R), !this.suppressValidationErr) {
      if (T.substring(R, R + 6).toUpperCase() === "SYSTEM") throw Error("External entities are not supported");
      else if (T[R] === "%") throw Error("Parameter entities are not supported")
    }
    let e = "";
  ...
  readElementExp(T, R) {
    ...
    R = or(T, R);
    let e = "";
    if (T[R] === "E" && P_(T, "MPTY", R)) R += 4;
    else if (T[R] === "A" && P_(T, "NY", R)) R += 2;
    else if (T[R] === "(") {
      R++;
  ...
  readAttlistExp(T, R) {
    ...
    R = or(T, R);
    let r = "";
    if (T.substring(R, R + 8).toUpperCase() === "#REQUIRED") r = "#REQUIRED", R += 8;
    else if (T.substring(R, R + 7).toUpperCase() === "#IMPLIED") r = "#IMPLIED", R += 7;
    else [R, r] = this.readIdentifierVal(T, R, "ATTLIST");
    return {
      elementName: a,
      attributeName: e,
      attributeType: t,
...
function P_(T, R, a) {
  for (let e = 0; e < R.length; e++)
    if (R[e] !== T[a + e + 1]) return !1;
  return !0
}

function jg(T) {
  if (iW(T)) return T;
  else throw Error(`Invalid entity name ${T}`)
}
...
function Aw0(T, R = {}) {
  ...
  else if (R.hex && ow0.test(a)) return mw0(a, 16);
  else if (a.search(/.+[eE].+/) !== -1) return _w0(T, a, R);
  else {
    let e = nw0.exec(a);
    if (e) {
      let t = e[1] || "",
      ...
      if (!R.leadingZeros && (r.length > 1 || r.length === 1 && !i)) return T;
      else {
        let c = Number(a),
          s = String(c);
        if (c === 0) return c;
        if (s.search(/[eE]/) !== -1)
          if (R.eNotation) return c;
          else return T;
        else if (a.indexOf(".") !== -1)
          if (s === "0") return c;
          else if (s === h) return c;
        else if (s === `${t}${h}`) return c;
        else return T;
        let A = r ? h : a;
        if (r) return A === s || t + A === s ? c : T;
        else return A === s || A === t + s ? c : T
      }
    } else return T
  }
}
...
function _w0(T, R, a) {
  ...
  if (e) {
    ...
    if (h.length > 1 && i) return T;
    else if (h.length === 1 && (e[3].startsWith(`.${r}`) || e[3][0] === r)) return Number(R);
    else if (a.leadingZeros && !i) return R = (e[1] || "") + e[3], Number(R);
    else return T
  } else return T
}
...
function bw0(T) {
  if (T && T.indexOf(".") !== -1) {
    if (T = T.replace(/0+$/, ""), T === ".") T = "0";
    else if (T[0] === ".") T = "0" + T;
    else if (T[T.length - 1] === ".") T = T.substring(0, T.length - 1);
    return T
  }
  return T
}
...
function mw0(T, R) {
  ...
  else if (Number.parseInt) return Number.parseInt(T, R);
  else if (window && window.parseInt) return window.parseInt(T, R);
  else throw Error("parseInt, Number.parseInt, window.parseInt are not supported")
}
  ...
  constructor(T) {
    if (this.options = T, this.currentNode = null, this.tagsNodeStack = [], this.docTypeEntities = {}, this.lastEntities = {
      ...
      for (let R = 0; R < this.options.stopNodes.length; R++) {
        ...
        if (a.startsWith("*.")) this.stopNodesWildcard.add(a.substring(2));
        else this.stopNodesExact.add(a)
      }
    }
  }
}
...
function Pw0(T, R, a, e, t, r, h) {
  if (T !== void 0) {
    ...
    if (T.length > 0) {
      ...
      if (i === null || i === void 0) return T;
      else if (typeof i !== typeof T || i !== T) return i;
      else if (this.options.trimValues) return IQ(T, this.options.parseTagValue, this.options.numberParseOptions);
      else if (T.trim() === T) return IQ(T, this.options.parseTagValue, this.options.numberParseOptions);
      else return T
    }
  }
}
    ...
    for (let h = 0; h < t; h++) {
      ...
      if (i.length) {
        ...
        if (c !== void 0) {
          ...
          if (A === null || A === void 0) r[s] = c;
          else if (typeof A !== typeof c || A !== c) r[s] = A;
          else r[s] = IQ(c, this.options.parseAttributeValue, this.options.numberParseOptions)
...
function gw0(T, R, a, e) {
  ...
  if (t === !1);
  else if (typeof t === "string") R.tagname = t, T.addChild(R, e);
  else T.addChild(R, e)
}
...
function sb(T, R, a, e) {
  let t = T.indexOf(R, a);
  if (t === -1) throw Error(e);
  else return t + R.length - 1
}
...
function Ow0(T, R, a) {
  ...
  else if (T[a + 1] === "?") a = sb(T, "?>", a + 1, "StopNode is not closed.");
  else if (T.substr(a + 1, 3) === "!--") a = sb(T, "-->", a + 3, "StopNode is not closed.");
  else if (T.substr(a + 1, 2) === "![") a = sb(T, "]]>", a, "StopNode is not closed.") - 2;
  else {
    let r = fQ(T, a, ">");
    if (r) {
...
function iTR(T, R, a) {
  ...
  for (let r = 0; r < T.length; r++) {
    ...
    if (i === R.textNodeName)
      ...
      if (h[":@"]) Cw0(s, h[":@"], c, R);
      else if (Object.keys(s).length === 1 && s[R.textNodeName] !== void 0 && !R.alwaysCreateTextNode) s = s[R.textNodeName];
      else if (Object.keys(s).length === 0)
        if (R.alwaysCreateTextNode) s[R.textNodeName] = "";
        else s = "";
      if (t[i] !== void 0 && t.hasOwnProperty(i)) {
        if (!Array.isArray(t[i])) t[i] = [t[i]];
        t[i].push(s)
      } else if (R.isArray(i, c, A)) t[i] = [s];
      else t[i] = s
    }
  }
  if (typeof e === "string") {
...
function Cw0(T, R, a, e) {
  if (R) {
    ...
    for (let h = 0; h < r; h++) {
      let i = t[h];
      if (e.isArray(i, a + "." + i, !0, !0)) T[i] = [R[i]];
      else T[i] = R[i]
    }
  }
}
...
class cW {
  ...
  parse(T, R) {
    ...
    else if (typeof T !== "string") throw Error("XML data is accepted in String or Bytes[] form.");
    if (R) {
      if (R === !0) R = {};
      let t = J70(T, R);
    ...
    let e = a.parseXml(T);
    if (this.options.preserveOrder || e === void 0) return e;
    else return dw0(e, this.options)
  }
  addEntity(T, R) {
      if (R.indexOf("&") !== -1) throw Error("Entity value can't have '&'");
          ...
          for '&#xD;'
          ");
          else if (R === "&") throw Error("An entity with value '&' is not permitted");
          else this.externalEntities[T] = R
        }
        static getMetaDataSymbol() {
      ...
      class WTR {
        ...
        buildShimmerMaskedTextSpan(T) {
          ...
          let a = new Set;
          for (let e of R)
            if (e.bob > 0) a.add(e.graphemeIndex);
