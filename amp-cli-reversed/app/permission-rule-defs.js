// Module: permission-rule-defs
// Original: segment1[2304009:2428989]
// Type: Scope-hoisted
// Exports: hH0, eL, xgT, DH0, wH0, BH0, fn, is, W0R, PW, NH0, UH0, HH0, WH0, qH0, tx, td, Zm, kW, xB
// Category: cli
...
er({
  ...
  execute: async (R) => {
    ...
    if (a.length > 0) R.submitQueue(a);
    else R.enterQueueMode()
  },
  isShown: (R) => {
  ...
  execute: async (R) => {
    try {
      ...
      if (a) {
        ...
        let t = await GH(a);
        if (typeof t === "object") R.setImageAttachments([...e, t]);
        else return Error(`Failed to process image: ${t}`)
...
`, new cT({
          ...
          else if (Array.isArray(n.tools) && n.tools.length > 0)
            for (let k of n.tools) {
              let x = RH0(k),
...
function UH0(T, R, a) {
  var e = T & 31,
    ...
    c = R >> 5 & 63,
    s = R >> 11 & 31;
  if (a == null || a === "local") return new Date(r, t, e, s, c, i, h);
  else if (a === "UTC") return new Date(Date.UTC(r, t, e, s, c, i, h));
  else throw Error("unrecognized options.timezone: " + options.timezone)
}
...
`;
    ...
    if (R === "darwin") a += `
On macOS, JetBrains IDEs are typically installed in ~/Library/Application Support/JetBrains/`;
    else if (R === "win32") a += `
...
class hhT {
  ...
  transitionToInstall(T) {
    ...
    if (this.requiresManualInstall) this.step = "manual-installation";
    else this.step = "installing";
...
function zW0(T, R) {
  ...
  if (t?.checks)
    for (let [r, h] of Object.entries(t.checks)) {
  ...
  for (let t = 1; t < T.length; t++) {
    ...
    if (A) {
      let l = A[1].trim(),
        ...
        n, p;
      if (o) n = o[1], p = o[2];
      else p = l, n = l.replace(/\s+/g, "_").replace(/[^\w]/g, "");
      let _ = {
        id: n,
        label: p,
        nodeIds: [],
    ...
    if (r === "end") {
      let l = e.pop();
      if (l)
        if (e.length > 0) e[e.length - 1].children.push(l);
        else a.subgraphs.push(l);
      continue
    }
    _q0(r, a, e)
  }
...
function oq0(T) {
  ...
  for (let r = 1; r < T.length; r++) {
    ...
    if (i) {
      if (a.length > 0) a[a.length - 1].direction = i[1].toUpperCase();
      else R.direction = i[1].toUpperCase();
      continue
    }
    ...
    if (h === "}") {
      let o = a.pop();
      if (o)
        if (a.length > 0) a[a.length - 1].children.push(o);
        else R.subgraphs.push(o);
      continue
    }
...
function _q0(T, R, a) {
  ...
  while (e.length > 0) {
    ...
    e = o.remaining.trim();
    for (let n of r)
      for (let p of o.ids) R.edges.push({
        source: n,
        target: p,
        label: s,
        style: A,
...
function rd(T, R, a) {
  let [e, t] = $S(T), r = Math.max(R, e), h = Math.max(a, t), i = Oh(r, h);
  for (let c = 0; c < i.length; c++)
    for (let s = 0; s < i[0].length; s++)
      if (c < T.length && s < T[0].length) i[c][s] = T[c][s];
...
function pl(T, R, a, ...e) {
  ...
  for (let i = 0; i <= t; i++)
    for (let c = 0; c <= r; c++)
      if (i < T.length && c < T[0].length) h[i][c] = T[i][c];
  for (let i of e)
    for (let c = 0; c < i.length; c++)
      for (let s = 0; s < i[0].length; s++) {
        let A = i[c][s];
        if (A !== " ") {
          let l = c + R.x,
            o = s + R.y,
            n = h[l][o];
          if (!a && NgT(A) && NgT(n)) h[l][o] = kq0(n, A);
          else h[l][o] = A
        }
      }
  return h
...
function f9R(T, R, a, e) {
  ...
  for (let r of T.children) {
    let h = f9R(r, t, a, e);
    t.children.push(h);
    for (let i of h.nodes)
      if (!t.nodes.includes(i)) t.nodes.push(i)
  }
  return t
}
...
function vq0(T, R, a, e) {
  ...
  function h(i) {
    ...
    for (let s of i.nodeIds)
      if (!r.has(s)) r.set(s, c)
  }
  for (let i of T) h(i);
...
function am(T, R) {
  if (T.x === R.x) return T.y < R.y ? w8 : G3;
  else if (T.y === R.y) return T.x < R.x ? Z8 : K3;
  else if (T.x < R.x) return T.y < R.y ? Rm : wA;
  else return T.y < R.y ? BA : sn
}
...
function Mq0(T, R) {
  ...
  let a = am(T.from.gridCoord, T.to.gridCoord),
    e, t, r, h, i = R === "LR" ? M0(a, K3) || M0(a, sn) || M0(a, BA) : M0(a, G3) || M0(a, sn) || M0(a, wA);
  if (M0(a, Rm))
    if (R === "LR") e = w8, t = K3, r = Z8, h = G3;
    else e = Z8, t = G3, r = w8, h = K3;
  else if (M0(a, wA))
    if (R === "LR") e = G3, t = K3, r = Z8, h = w8;
    else e = Z8, t = w8, r = G3, h = K3;
  else if (M0(a, BA))
    if (R === "LR") e = w8, t = w8, r = K3, h = G3;
    else e = K3, t = G3, r = w8, h = Z8;
  else if (M0(a, sn))
    if (R === "LR") e = w8, t = w8, r = K3, h = w8;
    else e = Z8, t = Z8, r = G3, h = Z8;
  else if (i)
    if (R === "LR" && M0(a, K3)) e = w8, t = w8, r = K3, h = Z8;
    else if (R === "TD" && M0(a, G3)) e = Z8, t = Z8, r = G3, h = w8;
...
function Dq0(T, R) {
  let [a, e, t, r] = Mq0(R, T.config.graphDirection), h = rL(R.from.gridCoord, a), i = rL(R.to.gridCoord, e), c = UgT(T.grid, h, i);
  if (c === null) {
  ...
  if (l = HgT(l), c.length <= l.length) R.startDir = a, R.endDir = e, R.path = c;
  else R.startDir = t, R.endDir = r, R.path = l
}
...
function I9R(T, R, a = 1) {
  let e = 0;
  for (let y of T)
    for (let u of y) e = Math.max(e, u.length);
  let t = e + 2 * a + 2,
    r = 0;
...
function Uq0(T, R, a, e, t, r) {
  let h = am(R, a),
    ...
    l = r ? "/" : "\u2571";
  if (M0(h, G3))
    for (let o = R.y - e; o >= a.y - t; o--) i.push({
      x: R.x,
      y: o
    }), T[R.x][o] = s;
  else if (M0(h, w8))
    ...
    for (let o = R.x - e; o >= a.x - t; o--) i.push({
      x: o,
      y: R.y
    }), T[o][R.y] = c;
  else if (M0(h, Z8))
    ...
    for (let o = R.x, n = R.y - e; o >= a.x - t && n >= a.y - t; o--, n--) i.push({
      x: o,
      y: n
    }), T[o][n] = A;
  else if (M0(h, wA))
...
function qq0(T, R, a) {
  ...
  let t = a[0],
    r = am(R[0], R[1]);
  if (M0(r, G3)) e[t.x][t.y + 1] = "\u2534";
  else if (M0(r, w8)) e[t.x][t.y - 1] = "\u252C";
  else if (M0(r, K3)) e[t.x + 1][t.y] = "\u2524";
  else if (M0(r, Z8)) e[t.x - 1][t.y] = "\u251C";
  return e
}
...
function Fq0(T, R) {
  ...
  for (let e = 1; e < R.length - 1; e++) {
    let t = R[e],
      ...
      c;
    if (!T.config.useAscii)
      if (M0(h, Z8) && M0(i, w8) || M0(h, G3) && M0(i, K3)) c = "\u2510";
      else if (M0(h, Z8) && M0(i, G3) || M0(h, w8) && M0(i, K3)) c = "\u2518";
    else if (M0(h, K3) && M0(i, w8) || M0(h, G3) && M0(i, Z8)) c = "\u250C";
    else if (M0(h, K3) && M0(i, G3) || M0(h, w8) && M0(i, Z8)) c = "\u2514";
    else c = "+";
    else c = "+";
    a[r.x][r.y] = c
  }
...
function Qq0(T) {
  ...
  for (let s of T.nodes)
    if (!s.drawn && s.drawingCoord && s.drawing) T.canvas = pl(T.canvas, s.drawingCoord, R, s.drawing), s.drawn = !0;
  let e = [],
    t = [],
    r = [],
    h = [],
...
function Wv(T, R, a) {
  if (T.grid.has(Wl(a)))
    if (T.config.graphDirection === "LR") return Wv(T, R, {
      x: a.x,
      y: a.y + 4
    });
    else return Wv(T, R, {
...
function e2(T, R) {
  for (let a of T.subgraphs)
    if (a.nodes.includes(R)) return a;
  return null
}
...
function g9R(T, R) {
  ...
  for (let c of R.children)
    if (g9R(T, c), c.nodes.length > 0) a = Math.min(a, c.minX), e = Math.min(e, c.minY), t = Math.max(t, c.maxX), r = Math.max(r, c.maxY);
  ...
  for (let a = 0; a < R.length; a++)
    for (let e = a + 1; e < R.length; e++) {
      ...
      if (t.minX < r.maxX && t.maxX > r.minX) {
        if (t.maxY >= r.minY - 1 && t.minY < r.minY) r.minY = t.maxY + 1 + 1;
        else if (r.maxY >= t.minY - 1 && r.minY < t.minY) t.minY = r.maxY + 1 + 1
      }
      if (t.minY < r.maxY && t.maxY > r.minY) {
        if (t.maxX >= r.minX - 1 && t.minX < r.minX) r.minX = t.maxX + 1 + 1;
        else if (r.maxX >= t.minX - 1 && r.minX < t.minX) t.minX = r.maxX + 1 + 1
      }
    }
}
...
function t50(T) {
  ...
  for (let r of T.nodes)
    if (r.drawingCoord) r.drawingCoord.x += e, r.drawingCoord.y += t
}
...
function r50(T) {
  ...
  let i = R === "LR" && r && h,
    c, s = [];
  if (i) c = t.filter((A) => !a2(T, A)), s = t.filter((A) => a2(T, A));
  else c = t;
  for (let A of c) {
    let l = R === "LR" ? {
        x: 0,
...
`).map((M) => M.trim()).filter((M) => M.length > 0 && !M.startsWith("%%")),
  ...
  for (let M = 0; M < e.messages.length; M++) {
    for (let Q = 0; Q < e.blocks.length; Q++)
      if (e.blocks[Q].startIndex === M) S += 2, f.set(Q, S - 1);
    for (let Q = 0; Q < e.blocks.length; Q++)
      for (let W = 0; W < e.blocks[Q].dividers.length; W++)
        if (e.blocks[Q].dividers[W].index === M) S += 1, g.set(`${Q}:${W}`, S), S += 1;
    S += 1;
    let V = e.messages[M];
    if (V.from === V.to) x[M] = S + 1, k[M] = S, S += 3;
    else x[M] = S, k[M] = S + 1, S += 2;
    for (let Q = 0; Q < e.notes.length; Q++)
      if (e.notes[Q].afterIndex === M) {
        S += 1;
        let W = e.notes[Q],
          eT = W.text.split("\\n"),
          ...
          TT;
        if (W.position === "left") TT = P[oT] - iT - 1;
        else if (W.position === "right") TT = P[oT] + 2;
        else if (W.actorIds.length >= 2) {
    ...
    for (let Q = 0; Q < e.blocks.length; Q++)
      if (e.blocks[Q].endIndex === M) S += 1, v.set(Q, S), S += 1
  }
  S += 1;
  let O = S,
    j = O + y,
...
function s50(T) {
  ...
  for (let h = 1; h < T.length; h++) {
    ...
    if (t && r > 0) {
      ...
      let _ = WgT(i);
      if (_)
        if (_.isMethod) t.methods.push(_.member);
        else t.attributes.push(_.member);
      continue
    }
    ...
    if (o) {
      ...
      if (!p.match(/<\|--|--|\*--|o--|-->|\.\.>|\.\.\|>/)) {
        let _ = by(a, o[1]),
          m = WgT(p);
        if (m)
          if (m.isMethod) _.methods.push(m.member);
          else _.attributes.push(m.member);
        continue
      }
    }
    let n = o50(i);
...
function WgT(T) {
  ...
  let r = e.split(/\s+/),
    h, i;
  if (r.length >= 2) i = r[0], h = r.slice(1).join(" ");
  else h = r[0] ?? e;
  let c = h.endsWith("$"),
    s = h.endsWith("*");
  return {
...
function n50(T) {
  switch (T) {
    ...
    case "o--":
      return {
        type: "aggregation", markerAt: "from"
      };
    case "--o":
      return {
        type: "aggregation", markerAt: "to"
      };
    case "-->":
      return {
...
function my(T, R, a) {
  switch (T) {
    ...
    case "realization":
      if (a === "down") return R ? "^" : "\u25B3";
      else if (a === "up") return R ? "v" : "\u25BD";
      else if (a === "left") return R ? ">" : "\u25C1";
      else return R ? "<" : "\u25B7";
    case "composition":
      return R ? "*" : "\u25C6";
    case "aggregation":
    ...
    case "association":
    case "dependency":
      if (a === "down") return R ? "v" : "\u25BC";
      else if (a === "up") return R ? "^" : "\u25B2";
      else if (a === "left") return R ? "<" : "\u25C0";
      else return R ? ">" : "\u25B6"
  }
}
...
`).map((O) => O.trim()).filter((O) => O.length > 0 && !O.startsWith("%%")),
  ...
  for (let O of e.classes) {
    ...
    let d = 0;
    for (let D of j)
      for (let B of D) d = Math.max(d, B.length);
    let C = d + 4,
      L = 0;
  ...
  for (let O of e.relationships) {
    ...
    if (B < V) {
      ...
      if (D !== M) {
        let W = Math.min(D, M),
          eT = Math.max(D, M);
        for (let iT = W; iT <= eT; iT++)
          if (iT < k && Q < x) f[iT][Q] = L;
        if (!t && Q < x)
          if (D < M) f[D][Q] = "\u2514", f[M][Q] = "\u2510";
          else f[D][Q] = "\u2518", f[M][Q] = "\u250C"
      }
      ...
      for (let iT = Q - 1; iT >= eT; iT--)
        if (iT >= 0 && iT < x) f[D][iT] = w;
      if (D !== M) {
        let iT = Math.min(D, M),
          aT = Math.max(D, M);
        for (let oT = iT; oT <= aT; oT++)
          if (oT < k && eT >= 0 && eT < x) f[oT][eT] = L;
        if (!t && eT >= 0 && eT < x)
          if (D < M) f[D][eT] = "\u250C", f[M][eT] = "\u2518";
          else f[D][eT] = "\u2510", f[M][eT] = "\u2514"
      }
      for (let iT = eT - 1; iT > W; iT--)
        if (iT >= 0 && iT < x) f[M][iT] = w;
      if (C.markerAt === "from") {
    ...
    if (O.label) {
      let Q = ` ${O.label} `,
        W = Math.floor((D + M) / 2),
        eT;
      if (B < V) eT = Math.floor((B + 1 + V - 1) / 2);
      else if (d.y + d.height - 1 < j.y) {
...
`).map((f) => f.trim()).filter((f) => f.length > 0 && !f.startsWith("%%")),
  ...
  for (let f of e.entities) {
    ...
    let g = 0;
    for (let j of v)
      for (let d of j) g = Math.max(g, d.length);
    let I = g + 4,
      S = 0;
  ...
  for (let f of e.relationships) {
    ...
    if (Math.abs(j - C) < Math.max(v.height, g.height)) {
      let [L, w] = O < d ? [v, g] : [g, v], [D, B] = O < d ? [f.cardinality1, f.cardinality2] : [f.cardinality2, f.cardinality1], M = L.x + L.width, V = w.x - 1, Q = L.y + Math.floor(L.height / 2);
      for (let iT = M; iT <= V; iT++)
        if (iT < m) y[iT][Q] = I;
      let W = iL(D, t);
      for (let iT = 0; iT < W.length; iT++) {
        let aT = M + iT;
      ...
      if (f.label) {
        let iT = Math.floor((M + V) / 2),
          ...
          oT = Q - 1;
        if (oT >= 0)
          for (let TT = 0; TT < f.label.length; TT++) {
            let tT = aT + TT;
            if (tT >= M && tT <= V && tT < m) y[tT][oT] = f.label[TT]
          }
      }
    } else {
      let [L, w] = j < C ? [v, g] : [g, v], [D, B] = j < C ? [f.cardinality1, f.cardinality2] : [f.cardinality2, f.cardinality1], M = L.y + L.height, V = w.y - 1, Q = L.x + Math.floor(L.width / 2);
      ...
      if (Q !== W) {
        let oT = Math.floor((M + V) / 2),
          TT = Math.min(Q, W),
          tT = Math.max(Q, W);
        for (let lT = TT; lT <= tT; lT++)
          if (lT < m && oT < b) y[lT][oT] = I;
        for (let lT = oT + 1; lT <= V; lT++)
          if (lT < b) y[W][lT] = S
      }
      let eT = iL(D, t);
      if (M < b)
...
function x50(T, R = {}) {
  ...
  switch (k50(T)) {
    ...
    default: {
      let e = cq0(T);
      if (e.direction === "LR" || e.direction === "RL") a.graphDirection = "LR";
      else a.graphDirection = "TD";
      let t = $q0(e, a);
...
function v50(T) {
  ...
  let a = T.yColumns.length > 1 || !!T.groupColumn,
    e;
  if (T.chartType === "bar")
    if (T.horizontal) e = "horizontal-bar";
    else if (T.stacked && a) e = "stacked-bar";
  else e = "bar";
  else if (T.chartType === "area") e = T.stacked ? "stacked-area" : "line";
  else e = "line";
  let t = [];
  if (T.groupColumn) {
    let r = T.yColumns[0];
...
function gB(T) {
  ...
  let a = T.terminalAssistantMessage,
    ...
    t = "",
    r = -1;
  for (let i = a.content.length - 1; i >= 0; i--)
    if (a.content[i]?.type === "thinking") {
      r = i;
      break
    }
  let h = () => {
