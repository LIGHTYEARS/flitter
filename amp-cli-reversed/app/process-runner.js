...
// Original: segment1[1456590:1533108]
// Type: Scope-hoisted
// Exports: T_0, t_0, r_0, XkT, h_0, i_0, c_0, s_0, b_0, uVT, ctT, yVT, m_0, u_0, y_0, iY, P_0, k_0, cY, x_0
// Category: cli
...
syncApprovalRequests(T, R) {
  ...
  let a = R.filter((t) => t.threadId === T.threadId || t.mainThreadId === T.threadId),
    e = new Set(a.map(pS));
  for (let t of [...T.toolRunner.sentApprovalRequests])
    if (!e.has(t)) T.toolRunner.sentApprovalRequests.delete(t);
  for (let t of a) {
    let r = pS(t);
...
function k_0(T, R) {
  ...
  for (let i of [t, e, a])
    ...
    else if (i !== void 0) J.warn("Expected array value for merged array key", {
    key: T,
    value: i
  });
  let h = new Map;
...
function cY(T) {
  let R = {};
  if (T.default && typeof T.default === "object")
    for (let [a, e] of Object.entries(T.default)) R[a] = {
      ...e,
      _target: "default"
    };
  ...
  if (T.global && typeof T.global === "object")
    for (let [a, e] of Object.entries(T.global)) R[a] = {
      ...e,
      _target: "global"
    };
  ...
  if (T.workspace && typeof T.workspace === "object")
    for (let [a, e] of Object.entries(T.workspace)) R[a] = {
      ...e,
      _target: "workspace"
    };
  ...
  if (T.override && typeof T.override === "object")
    for (let [a, e] of Object.entries(T.override)) R[a] = {
      ...e
    };
...
function I_0(T, R) {
  ...
  if (T) {
    ...
    for (let e of Object.keys(T))
      if (!(e in R) && !a.includes(e)) a.push(e)
...
`).length - 1
      ...
      let f = {};
      for (let [v, g] of Object.entries(x))
        if (v.startsWith("amp.")) {
          let I = v.substring(4);
          f[I] = g
        }
...
async function j_0(T) {
  ...
  } = await f_0(T.cwd), e = await xVT({
    ...T,
    settingsFile: a
  }), t = new W0, r = e.changes.subscribe((c) => t.next(c)), h = T.workspaceTrust?.current ?? !1, i = T.workspaceTrust?.changes?.subscribe(async (c) => {
    let s = h;
    if (h = c, !s && c) {
...
async function B_0() {
  ...
  return {
    ...
    async set(r, h, i) {
      let c = ID(i),
        s = await t(r, c);
      if (h) s.setPassword(h);
      else s.deleteCredential();
      let A = `${r}@${c}`;
      a[A] = h, R.next(A)
    },
    changes: R
  ...
  return Object.defineProperty(w3, "codes", {
    ...
    ansi256ToAnsi: {
      value: (R) => {
        ...
        let a, e, t;
        if (R >= 232) a = ((R - 232) * 10 + 8) / 255, e = a, t = a;
        else {
          R -= 16;
          let i = R % 36;
...
`;
  let R = [
    ...
    ["AMP_URL", `URL for the Amp service (default is ${Lr})`],
    ["AMP_LOG_LEVEL", "Set log level (can also use --log-level)"],
    ["AMP_LOG_FILE", "Set log file location (can also use --log-file)"],
    ["AMP_SETTINGS_FILE", `Set settings file path (can also use --settings-file, default: ${dA})`]
  ];
  T += ltT(R), T += `
`, T += oR.bold("Examples:") + `

`;
  for (let e of L4.examples)
    if (T += e.description + `
...
async function CVT() {
  let T = process.env.PATH || "",
    ...
    e = !1,
    t = a.flatMap((r) => {
      if (e) return [lF(r, "amp.bat"), lF(r, "amp.exe")];
      else return [lF(r, "amp")]
    });
...
[WARN] Run "${LVT()}" in your shell to see which amp executable is currently being used.`, {
    ...
    case "missing":
      return {
        status: "missing", warning: "[WARN] amp not accessible via PATH"
      };
    case "fail": {
...
`), rt.bold(`  ${a.refreshCommand}`), rt.bold("  amp --help");
  else rt.yellow(`
...
async function M4(T) {
  ...
  else return J.debug("Command not found or failed", {
    command: T,
    result: R
  }), !1
}
...
async function hm0(T) {
  ...
  let {
    tarballUrl: t,
    integrity: r
  } = await Rm0(e), h = gs(R, ".package.staging");
  try {
    if (Sl(h)) k$(h, {
      recursive: !0
    });
  ...
  return setImmediate(async () => {
    ...
    try {
      ...
      try {
        ...
        if (p.status === "same") r.info("success", _), e.next("updated");
        else r.warn("success with warning", _), e.next("updated-with-warning")
      } catch (p) {
        e.next("update-error")
      }
    } catch (c) {
...
function AY(T, R, a, e) {
  let t = km0(T.button),
    r;
  if (t.button === "wheel_up" || t.button === "wheel_down" || t.button === "wheel_left" || t.button === "wheel_right") r = "scroll";
  else if (t.motion) r = "move";
  else if (T.pressed) r = "press";
  else r = "release";
  let h, i;
  if (R && a && e) h = (T.x - 1) / a, i = (T.y - 1) / e;
  else h = T.x - 1, i = T.y - 1;
  return {
    type: "mouse",
    action: r,
    button: t.button,
...
function Om0(T, R, a) {
  ...
  function t(r, h) {
    if (typeof r === "string") e.push({
      ...
      else if (Array.isArray(r.content)) r.content.forEach((c) => t(c, i));
      else t(r.content, i)
    }
  }
...
function Em0(T, R, a, e, t) {
  ...
  if (T.fg)
    if (R.fg) r.fg = D4(T.fg, R.fg, t);
    else r.fg = D4(T.fg, e, t);
  if (T.bg) {
    let h = uS(T.bg);
    if (T.bg.type === "none" || h === 0) {
      if (R.bg) r.bg = R.bg
    } else if (R.bg) r.bg = D4(T.bg, R.bg, t);
    else r.bg = D4(T.bg, a, t)
  }
...
class pY {
  ...
  setCell(T, R, a) {
    ...
    if (a.width > 1) {
      for (let r = 1; r < a.width; r++)
        if (T + r < this.width && this.cells[R]) this.cells[R][T + r] = a9(" ", a.style, 1)
    }
  }
  setChar(T, R, a, e, t) {
  ...
  fill(T, R, a, e, t = " ", r = {}) {
    let h = a9(t, r);
    for (let i = 0; i < e; i++)
      for (let c = 0; c < a; c++) this.setCell(T + c, R + i, h)
  }
  copyTo(T) {
    let {
      width: R,
    ...
    for (let e = 0; e < Math.min(this.height, a); e++)
      for (let t = 0; t < Math.min(this.width, R); t++) {
  ...
  performAction(T, R) {
      ...
      switch (T) {
        ...
        case "execute":
          ...
          break;
        case "clear":
          this.flushTextBuffer(), this.context.private.length = 0, this.context.intermediates.length = 0, this.context.final = "", this.context.params = [], this.context.oscEscSeen = !1, this.context.paramBuffer.length = 0, this.context.subparamBuffer.length = 0, this.context.currentSubparams = [], this.context.osc