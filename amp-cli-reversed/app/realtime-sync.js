...
// Original: segment1[1406816:1456590]
// Type: Scope-hoisted
// Exports: AVT, gp0, atT, WkT, $p0, vp0, qkT, jp0, Sw, aY, zkT, dM, Sp0, Op0, Ep0, Cp0, Lp0, oF, Mp0, etT
// Category: cli

ror(`${this.clientID} tool execution setup failed: ${a}`, c);
let s = {
  status: "error",
  error: {
...
sendTerminalResult(T, R, a, e) {
  ...
  if (t) this.pendingTerminalResults.delete(T);
  else this.pendingTerminalResults.set(T, R);
...
toToolRun(T) {
  switch (T.status) {
    case "done":
      return {
        status: "done", result: T.result, progress: T.progress, trackFiles: T.trackFiles ? [...T.trackFiles] : void 0
      };
    case "error":
      return {
        status: "error", error: {
    ...
    case "rejected-by-user":
      return {
        status: "rejected-by-user", reason: T.reason
      };
    case "cancelled":
      return {
        status: "cancelled", reason: T.reason
      };
    default:
      return {
        status: "error", error: {
...
class PH {
  ...
  async invokeTool(T) {
    ...
    let {
      ...
      toolName: a,
      args: e
    } = T, t = typeof e === "object" && e !== null ? e : {}, r = await this.options.getEnvironment();
    return r.thread = {
      ...r.thread,
...
function qp0(T, R) {
  ...
  if (T.state === "complete" || T.state === "error") {
    if (R.streamingMessageId !== null && T.messageId !== R.streamingMessageId) {
      ...
      if (R.streamingMessageId = null, R.streamingBlocks = [], R.streamingParentToolCallId = void 0, T.blocks && T.blocks.length > 0)
        if (T.role === "user") t.push({
  ...
  if (T.state === "tool_use") {
    ...
    if (R.streamingMessageId === null || R.streamingMessageId !== T.messageId) R.streamingMessageId = T.messageId, R.streamingBlocks = _g([], T.blocks ?? []), R.streamingParentToolCallId = a;
    else if (T.blocks) R.streamingBlocks = _g(R.streamingBlocks, T.blocks);
    if (R.streamingBlocks.length > 0) t.push({
...
class ttT {
  ...
  static async create(T) {
    ...
    }, c = new uH(await Yp0(T, a)), s = new ttT(i, c, h, e.v, {
  ...
  clearToolProgressFromBlocks(T) {
    for (let R of T)
      if (R.type === "tool_result") this.clearToolProgress(R.toolUseID)
  }
    ...
    let T = {
      onConnectionChange: (R) => {
        this.runTransportHandler("connectionChanges", () => {
          ...
          if (!t && !r) return;
          if (t) this.logConnectionStateTransition(this.lastConnectionState, R);
          else if (r && e) this.logConnectionRoleTransition(e, R);
          let h = R.state,
      ...
      onDelta: (R) => {
        this.runTransportHandler("deltas", () => {
          ...
          if (this.deltaState.completedMessages.length > 0) {
            for (let a of this.deltaState.completedMessages)
              if (a.role === "user") this.clearToolProgressFromBlocks(a.blocks);
...
async function Gk(T) {
  let {
    ...
    v: 0,
    messages: []
  }, n = zR.file(process.cwd()), p = s ?? fx(), _ = await t.getLatest(), m, b;
  if (c) m = c.trackedFileSystem(p), b = c.tracker;
  else b = dWT(CG, o.id, async () => {}), m = CWT(He, b, p);
  return {
    dir: n,
    tool: R,
    thread: o,
...
function ie(T, R) {
  if (R) J.info(`[headless-dtw] ${T}`, R);
  else J.info(`[headless-dtw] ${T}`);
  if (!htT) return;
  let a = oR.dim(`[${rtT()}]`),
    e = oR.cyan("[INFO]"),
    t = R ? ` ${itT(R)}` : "";
...
class bVT {
  ...
  async invokeToolForClient(T, R) {
    ...
    let {
      ...
      toolCallId: i,
      args: c
    } = R, s = typeof c === "object" && c !== null ? c : {}, A = AVT({
      configService: e,
