...
// Original: segment1[1316794:1364140]
// Type: Scope-hoisted
// Exports: rF, Qn0, Zn0, Jn0, Tl0, Rl0, QeT, kkT, al0, el0, tl0, rl0, Z1, hl0, CKT, il0, cl0, sl0, ol0, pl0
// Category: cli
...
updateConnectionState(T) {
  ...
  } else if (this.connectionInfo.state = T, T === "connected") this.connectionInfo.threadId = this.currentThreadID, this.reconnectCause = null, delete this.connectionInfo.reconnectCause;
  else if (this.reconnectCause) this.connectionInfo.reconnectCause = this.reconnectCause;
...
function rF(T) {
  switch (T.type) {
    case "queued_messages":
      return {
        type: "queued_messages", message: T
      };
    case "queued_message_added":
      return {
        type: "queued_message_added", message: T
      };
    case "queued_message_removed":
      return {
        type: "queued_message_removed", message: T
      }
  }
}
...
function Qn0(T) {
  switch (T.type) {
    case "retry_scheduled":
      return {
        type: "retry_scheduled", message: T
      };
    case "retry_started":
      return {
        type: "retry_started", message: T
      };
    case "retry_cancelled":
      return {
        type: "retry_cancelled", message: T
      }
  }
}
...
function Zn0(T) {
  switch (T.type) {
    case "error_set":
      return {
        type: "active_error_set", message: T
      };
    case "error_cleared":
      return {
        type: "active_error_cleared", message: T
      }
  }
}
...
function Jn0(T) {
  switch (T.type) {
    case "artifacts_snapshot":
      return {
        type: "artifacts_snapshot", message: T
      };
    case "artifact_upserted":
      return {
        type: "artifact_upserted", message: T
      };
    case "artifact_deleted":
      return {
        type: "artifact_deleted", message: T
      }
  }
}
...
function Tl0(T) {
  switch (T.type) {
    case "compaction_started":
      return {
        type: "compaction_started", message: T
      };
    case "compaction_complete":
      return {
        type: "compaction_complete", message: T
      }
  }
}
...
function Cl0(T, R, a, e) {
  ...
  for (let h of T.content)
    if (h.type === "text") t.push(UKT(h));
    else if (h.type === "tool_use") t.push(Sl0(h));
  else if (r && h.type === "thinking") t.push(dl0(h));
  else if (r && h.type === "redacted_thinking") t.push(El0(h));
  return {
    type: "assistant",
    message: {
      type: "message",
...
function Ll0(T, R, a) {
  let e = [];
  for (let t of T.content)
    if (t.type === "text") e.push(UKT(t));
    else if (t.type === "tool_result") e.push(Ol0(t));
  return {
    type: "user",
    message: {
      role: "user",
...
function ql0(T) {
  for (let R of T)
    if (R.type === "tool_result" && !wt(R.run.status)) return !1;
  return !0
}
    ...
    let I = IkT(n.messages),
      ...
      w = Fl0(async (D) => {
        while (S < D.messages.length) {
          ...
          let V = zl0(B);
          if (!V || !O.has(V)) {
            if (B.role === "user") await ng(Ll0(B, _, null));
...
async function Yl0(T) {
  let {
    ...
    agentMode: l,
    labels: o
  } = T, n = await m0(R.threadHandles$.pipe(da((y) => y !== null), ti(1), Gl(5000))), p = await m0(n.thread$.pipe(ti(1), Gl(5000))), _ = p.id;
...
function vkT(T) {
  if (T instanceof Error)
    if (v3T(T)) return "Unauthorized. Check your access token.";
    else if (dO(T)) return "Context window limit reached.";
  else if (fU(T)) return "Model provider overloaded. Try again in a few seconds.";
  else if (IU(T)) return "Model stream timed out. Try again in a few seconds.";
  else if ($3T(T)) return "Insufficient credit balance.";
...
class GKT {
  ...
  async checkForChanges(T, R, a) {
    ...
    for (let r of Array.from(a.keys()))
      if (!t.has(r)) e.push({
        type: "deleted",
        path: r,
        timestamp: Date.now(),
    ...
    if (e.length > 0)
      for (let r of this.callbacks) try {
        r(e)
      }
    catch (h) {
...
class vv {
  ...
  async performScan(T) {
    ...
    try {
      let [e, t] = await Promise.all([O4("git", ["ls-files", "--others", "--exclude-standard", "-z"], {
        ...
        maxBuffer: 1 / 0,
        timeout: 60000
      })]), r = e.stdout.split("\x00").filter(Boolean), h = t.stdout.split("\x00").filter(Boolean), i = [], c = [], s = this.repos.get(T);
      ...
      for (let o of Array.from(s.seenUntracked))
        if (!l.has(o)) {
      ...
      if (s.seenUntracked = l, s.lastScanTime = a, A.length)
        for (let o of this.callbacks) o(A)
    } catch (e) {
  ...
  async performFullStatusScan(T) {
    ...
    if (J.debug("Git status discovery (status)", {
      ...
      for (let A of this.callbacks) A(r)
  }
  parseStatus(T) {
    let R = [],
      a = T.split("\x00");
...
class VKT {
  ...
  async tryExternalTools(T, R, a, e) {
    let t = [];
    if (R === "auto")
      if (e.followSymlinks) t.push("ripgrep");
      else t.push("git", "ripgrep");
    else if (R === "git") {
  ...
  async runExternalTool(T, R, a, e) {
      let t = T === "git" ? "git" : await faT(),
        r = T === "git" ? ["ls-files", "--exclude-standard", "--cached", "--others"] : e.followSymlinks ? ["--files", "--follow", "--hidden", "--glob", "!.git/", "--glob", "!.jj/"] : ["--files", "--hidden", "--glob", "!.git/",