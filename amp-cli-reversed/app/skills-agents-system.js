// Module: skills-agents-system
// Original: segment1[937801:1006471]
// Type: Scope-hoisted
// Exports: gHR, Cy, $HR, vHR, jHR, SHR, OHR, zHR, e, faT, FHR, GHR, KHR, VHR, XHR, YHR, ZHR, JHR, aWR, eWR
// Category: cli
...
function ZHR(T, R, a, e) {
  return v3(...T.map((t) => JHR(t, R, a, e))).pipe(JR((t) => {
    let r = [];
    for (let i of t)
      if (i.status === "in-progress") r.push(...i?.progress ?? []);
      else if (i.status === "done") r.push(...i.result);
    else if (i.status === "error") r.push(...i.progress ?? []);
    else if (i.status === "cancelled") r.push(...i.progress ?? []);
...
async function U5T(T, R, a, e) {
  ...
  let h;
  if (a.basePath) c4T(a.basePath), h = An(a.basePath) ? a.basePath : MR.joinPath(T, a.basePath).fsPath;
  else h = T.fsPath;
  try {
    await w5T.lstat(h)
  } catch (s) {
...
`) m(), n = "";
      ...
      let y = String(b);
      if (o.length + y.length > cz) o += y.slice(0, cz - o.length);
      else o += y
    }), c.on("exit", (b) => {
      if (_) return;
      if (_ = !0, b && b >= 2)
        if (tWR(o, h)) J.warn("Ignoring broken descendant symlink while globbing files.", {
          basePath: h,
          glob: R?.pattern,
          stderr: o
        });
...
`), qR.default.string("\r"), qR.default.string("\\").then(qR.default.whitespace)).many().tie();
  ...
  unquotedString = qR.default.lazy(() => {
    if (this.platform === "win32") return qR.default.alt(qR.default.regexp(/[a-zA-Z]:[\\/]\S+/), qR.default.regexp(/\\\\\S+\\\S*/), qR.default.alt(this.unquotedEscapeSequences, qR.default.regexp(/[a-zA-Z0-9_\-:/.*~@=\\^,[\]#!]/), qR.default.regexp(/\$(?![A-Za-z_{(])/), qR.default.regexp(/\([a-zA-Z0-9_-]*\)/)).atLeast(1).tie()).desc("unquoted string");
    else return qR.default.alt(this.unquotedEscapeSequences, qR.default.regexp(/[a-zA-Z0-9_\-:/.*~@=+^,[\]#!]/), qR.default.regexp(/\$(?![A-Za-z_{(])/), qR.default.regexp(/\([a-zA-Z0-9_-]*\)/)).atLeast(1).tie().desc("unquoted string")
  });
  ...
  word = qR.default.lazy(() => {
    return qR.default.alt(this.doubleQuotedString, this.singleQuotedString, this.braceExpansion, this.unquotedString, this.commandSubstitution, this.envVariable).sepBy1(qR.default.string("")).map((T) => {
      let [R, ...a] = T;
      if (R === void 0) throw Error("Expect non-empty array of parts in argument: " + JSON.stringify(T));
      else if (a.length === 0) return R;
      else if (T.every((e) => typeof e === "string")) return T.join("");
      else return {
        type: "concatenation",
        values: T
      }
...
function W5T(T) {
  ...
  while (a.length > 0) {
    ...
    for (let h of e.arguments)
      if (h.value === xs || typeof h.value === "string") t.push(h.value);
      else if (h.value.type === "invocations") a.push(...h.value.trees), t.push(xs);
    else
      for (let i of h.value.values)
        if (typeof i === "object" && i.type === "invocations") a.push(...i.trees), t.push(xs);
        else if (i === xs || typeof i === "string") t.push(i);
    else throw Error("Unexpected word value: " + i);
    R.push({
      program: r,
      arguments: t
    })
...
function bWR(T, R) {
  ...
  if (T.workdir) return;
  try {
    let [e, t] = HO(T.command), r = e && e.arguments.length === 1 && typeof e.arguments[0]?.value === "string" ? e.arguments[0].value : void 0;
...
function dWR(T) {
  return async (R, ...a) => {
    ...
    for (let t = 0; t < R.length; t++)
      if (e += R[t], t < a.length) e += String(a[t]);
    try {
      let t = await SWR(e, {
        cwd: T
      });
...
function ZWR(T) {
  switch (T.method) {
    case "event":
      return {
        event: T.event, data: T.data, span: T.span
      };
    ...
    case "command.execute":
      return {
        name: T.name, threadID: T.threadID
      };
    case "tool.execute":
      return {
...
function X5T(T) {
  ...
  let {
    ...
    fileSystem: e,
    platform: t
  } = T, r = [], h = !1, i = Promise.resolve(), c = new W0, s = new W0, A = new W0, l = new Map;
  ...
  async function u(X) {
    ...
    try {
      let [rT, hT, pT] = await Promise.all([X.process.listCommands(), X.process.listTools(), X.process.listRegisteredEvents()]), mT = new Map(rT.map((MT) => [MT.id, MT])), yT = new Map(hT.map((MT) => [MT.name, MT])), uT = new Set(pT), bT = !_(X.registeredCommands, mT, o), jT = !_(X.registeredTools, yT, n), fT = !m(X.registeredEvents, uT);
  ...
  async function B(X) {
    ...
    if (X.waitForExit) await Promise.all(hT);
    else
      for (let pT of hT);
  }
  ...
  async function oT(X, rT = Yo, hT) {
    ...
    let mT = await Promise.all(pT.map((uT) => {
      ...
      yT = [];
    for (let uT of mT)
      if (uT.message) yT.push(uT.message);
    return {
      message: yT.length > 0 ? {
...
`),
  ...
  async function TT(X, rT = Yo) {
    ...
    for (let mT of pT)
      if (mT.action === "continue") return mT;
    return {
      action: "done"
    }
  }
  ...
  function tT() {
    ...
    for (let rT of r)
      if (rT.status === "active")
        for (let hT of rT.registeredCommands.values()) X.push(hT);
    return X
  }
  ...
  function N() {
    ...
    for (let rT of r)
      if (rT.status === "active")
        for (let hT of rT.registeredTools.values()) X.push(hT);
    return X
  }
...
function tqR(T) {
  let {
    pluginService: R,
    toolService: a
  } = T, e = [], t, r = new Promise((c) => {
    t = c
  }), h = !0, i = R.plugins.subscribe({
    next: () => {
...
function lqR(T) {
  ...
  let e = T.split("/");
  if (e.length >= 2) {
    let [t, r, ...h] = e, i = h.length > 0 ? h.join("/") : void 0;
    return {
      type: "github",
      url: `https://github.com/${t}/${r}.git`,
      skillPath: i
  ...
  try {
    ...
    for (let c of i) {
      ...
      try {
        ...
        if (s) BX(n, {
          recursive: !0
        }), _uT(u3(c, "SKILL.md"), u3(n, "SKILL.md"));
        else _uT(c, n, {
          recursive: !0
        });
...
function SaT(T, R = "") {
  ...
  for (let t of e) {
    ...
    if (t.isDirectory()) a.push(...SaT(r, h));
    else if (t.isFile()) {
      let i = fqR(r);
      a.push({
        path: h,
...
function jqR(T) {
  ...
  for (let a of T)
    if (a.size > kuT) throw new Ac(`File too large: ${a.path} (${Math.round(a.size/1024)}KB, max ${kuT/1024}KB)`);
...
function OqR(T) {
  ...
  else throw new Ac(`No SKILL.md found in ${T}`);
  let t = RzT(e, "utf-8"),
    {
      frontmatter: r,
      body: h
      ...
      return e = oT, new AR((TT) => {
        async function tT() {
          try {
            let {
              settings: N
            } = await T.getLatest(), q = await P7T(R, aT ? [aT] : [], oT.signal, N);
...
class azT {
  ...
  setCachedThread(T, R = {
    ...
    if (e) e.next(a);
    else this.threadSubjects.set(T.id, new f0(a));
  ...
  syncThreadEntryFromThread(T) {
    if (T.messages.length === 0 && !T.draft) this.deleteThreadEntry(T.id);
    else this.upsertThreadEntry(fuT(T))
  }
