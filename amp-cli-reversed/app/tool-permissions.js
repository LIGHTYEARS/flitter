...
// Type: Scope-hoisted
// Exports: vmR, Vf, Xf, yy, rG, jmR, xL, SmR, bq, OmR, FD, gN, fl, VLT, BmR, qp, elT, tlT, PMT
// Category: cli
...
if (!f) {
  if (J.debug(`Tool ${String(o)} not permitted - action: ${g}, reason: ${v}`), g === "ask") {
    ...
    try {
      ...
      else if (w.feedback) J.debug(`Tool ${String(o)} - user rejected with feedback: ${w.feedback}`), b.next({
        status: "error",
        error: {
          message: `This tool call was rejected by the user with feedback: ${w.feedback}`
        }
      }), b.complete();
      else J.debug(`Tool ${String(o)} - user rejected`), b.next({
...
if (!j && d) try {
  ...
  if (w.size > 0) {
    ...
    try {
      ...
      else if (B.feedback) J.debug(`Tool ${String(o)} - guarded file rejected with feedback: ${B.feedback}`), b.next({
        status: "error",
        error: {
          message: `This tool call was rejected by the user with feedback: ${B.feedback}`
        }
      }), b.complete();
      else J.debug(`Tool ${String(o)} - guarded file rejected`), b.next({
        status: "rejected-by-user",
...
if (x) {
  ...
  return n
}, pendingApprovals$: t, resolveApproval: i, clearApprovalsForThread: c, requestApproval: h, restoreApproval: s, dispose() {
...
function xL(T, R, a, e) {
  ...
  if (bq(t, "object") && typeof T === "object" && !Array.isArray(T)) {
    ...
    let h = new Set(Object.keys(r)),
      i = {};
    for (let [c, s] of Object.entries(T))
      if (h.has(c)) {
        let A = r[c];
        i[c] = xL(s, A, `${a}.${c}`, e)
      }
...
class PMT {
        ...
        if (this._taskMessageQueue) {
          ...
          while (h = await this._taskMessageQueue.dequeue(t, a.sessionId)) {
            if (h.type === "response" || h.type === "error") {
              ...
              if (s)
                if (this._requestResolvers.delete(c), h.type === "response") s(i);
                else {
                  let A = i,
  ...
  async connect(T) {
    ...
    this._transport.onmessage = (t, r) => {
      if (e?.(t, r), zg(t) || WmR(t)) this._onresponse(t);
      else if (cG(t)) this._onrequest(t, r);
      else if (HmR(t)) this._onnotification(t);
      else this._onerror(Error(`Unknown message type: ${JSON.stringify(t)}`))
  ...
  _onnotification(T) {
      let R = this._notificationHandlers.g