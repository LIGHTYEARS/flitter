...
var typesToCallDeepStrictEqualWith = [isKeyObject, isWeakSet, isWeakMap, @Buffer.isBuffer], SafeSetPrototypeIterator = SafeSet.prototype[SymbolIterator], SafeMapPrototypeIterator = SafeMap.prototype[SymbolIterator], SafeMapPrototypeHas = SafeMap.prototype.has, SafeMapPrototypeGet = SafeMap.prototype.get;
    ...
    for (let expectedItem of expected) {
      ...
      for (let { 0: key, 1: count } of expectedCounts)
        if (isDeepStrictEqual(key, expectedItem)) {
          expectedCounts.@set(key, count + 1), found = !0;
          break;
        }
      if (!found)
