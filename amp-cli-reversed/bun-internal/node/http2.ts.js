...
class Http2Stream extends Duplex {
  ...
  close(code, callback) {
    if ((this[bunHTTP2StreamStatus] & 8 /* Closed */) === 0) {
      let session = this[bunHTTP2Session];
      if (assertSession(session), code = code || 0, validateInteger(code, "code", 0, 4294967295), typeof callback < "u")
...
class ServerHttp2Session extends Http2Session {
  ...
  setLocalWindowSize(windowSize) {
    if (this.destroyed)
      throw @makeErrorWithCode(89);
    return validateInt32(windowSize, "windowSize", 0, 2147483647), this.#parser?.setLocalWindowSize?.(windowSize);
  }
  settings(settings, callback) {
    if (callback !== @undefined && typeof callback !== "function")
      throw @makeErrorWithCode(118, "callback", "function", callback);
    if (this.#pendingSettingsAck = !0, this.#parser?.settings(settings), typeof callback === "function") {
      let start = Date.now();
...
class ClientHttp2Session extends Http2Session {
  ...
  setNextStreamID(id) {
    if (this.destroyed)
      throw @makeErrorWithCode(89);
    if (validateNumber(id, "id"), id <= 0 || id > 4294967295)
      throw @makeErrorWithCode(156, "id", "> 0 and <= 4294967295", id);
...
class Http2Server extends net.Server {
  ...
  constructor(options, onRequestHandler) {
    ...
    super(options);
    if (this[kSessions] = new SafeSet, this.setMaxListeners(0), this.on("newListener", setupCompat), typeof onRequestHandler === "function")
