(function (){"use strict";// build/release/tmp_modules/node/wasi.ts
var $, nodeFsConstants = @processBindingConstants.fs, __getOwnPropNames = Object.getOwnPropertyNames, __commonJS = (cb, mod = @undefined) => function __require2() {
  ...
  "node_modules/wasi-js/dist/wasi.js"(exports) {
    ...
    var msToNs = (ms) => {
      let msInt = Math.trunc(ms), decimal = BigInt(Math.round((ms - msInt) * 1e6));
      ...
      let entry = wasi.FD_MAP.get(fd);
      if (!entry)
        throw new types_1.WASIError(constants_1.WASI_EBADF);
      if (entry.filetype === void 0) {
        let stats = wasi.fstatSync(entry.real), { filetype, rightsBase, rightsInheriting } = translateFileAttributes(wasi, fd, stats);
      ...
      switch (!0) {
        ...
        case stats.isCharacterDevice(): {
          let filetype = constants_1.WASI_FILETYPE_CHARACTER_DEVICE;
    ...
    function getDefaults() {
      ...
      let defaultBindings = {
        ...
        isTTY: (fd) => (@getInternalField(@internalModuleRegistry, 124) || @createInternalModuleById(124)).isatty(fd),
        fs: @getInternalField(@internalModuleRegistry, 98) || @createInternalModuleById(98),
        path: @getInternalField(@internalModuleRegistry, 107) || @createInternalModuleById(107)
      };
      return defaultConfig = {
        args: [],
        env: {},
    ...
    var WASI = class WASI2 {
      constructor(wasiConfig = {}) {
        let defaultConfig2 = getDefaults();
        ...
        let args = wasiConfig.args ?? defaultConfig2.args;
        this.memory = void 0, this.view = void 0, this.bindings = wasiConfig.bindings || defaultConfig2.bindings;
        let bindings2 = this.bindings;
        fs = bindings2.fs, this.FD_MAP = /* @__PURE__ */ new Map([
          [
        ...
        let path = bindings2.path;
        for (let [k, v] of Object.entries(preopens)) {
          let real = fs.openSync(v, nodeFsConstants.O_RDONLY), newfd = this.getUnusedFileDescriptor();
        ...
        let getiovs = (iovs, iovsLen) => {
          this.refreshMemory();
          let { view, memory } = this, { buffer } = memory, { byteLength } = buffer;
          if (iovsLen === 1) {
            let ptr = iovs, buf = view.getUint32(ptr, !0), bufLen = view.getUint32(ptr + 4, !0);
          ...
          for (let i = 0, ptr = iovs;i < iovsLen; i++, ptr += 8) {
            let buf = view.getUint32(ptr, !0), bufLen = view.getUint32(ptr + 4, !0);
            if (bufLen > byteLength - buf)
              console.log({
                buf,
                bufLen,
        ...
        if (this.wasiImport = {
          args_get: (argv, argvBuf) => {
            ...
            return args.forEach((a) => {
              this.view.setUint32(coffset, offset, !0), coffset += 4, offset += @Buffer.from(this.memory.buffer).write(`${a}\x00`, offset);
            }), constants_1.WASI_ESUCCESS;
          },
          args_sizes_get: (argc, argvBufSize) => {
            this.refreshMemory(), this.view.setUint32(argc, args.length, !0);
            let size = args.reduce((acc, a) => acc + @Buffer.byteLength(a) + 1, 0);
          ...
          environ_get: (environ, environBuf) => {
            ...
            return Object.entries(this.env).forEach(([key, value]) => {
              this.view.setUint32(coffset, offset, !0), coffset += 4, offset += @Buffer.from(this.memory.buffer).write(`${key}=${value}\x00`, offset);
            }), constants_1.WASI_ESUCCESS;
          },
          environ_sizes_get: (environCount, environBufSize) => {
            this.refreshMemory();
            let envProcessed = Object.entries(this.env).map(([key, value]) => `${key}=${value}\x00`), size = envProcessed.reduce((acc, e) => acc + @Buffer.byteLength(e), 0);
          ...
          fd_close: wrap((fd) => {
            let stats = CHECK_FD(fd, BigInt(0));
            return fs.closeSync(stats.real), this.FD_MAP.delete(fd), constants_1.WASI_ESUCCESS;
          }),
          fd_datasync: wrap((fd) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_DATASYNC);
          ...
          fd_filestat_get: wrap((fd, bufPtr) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_FILESTAT_GET), rstats = this.fstatSync(stats.real);
          ...
          fd_filestat_set_size: wrap((fd, stSize) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_FILESTAT_SET_SIZE);
          ...
          fd_filestat_set_times: wrap((fd, stAtim, stMtim, fstflags) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_FILESTAT_SET_TIMES), rstats = this.fstatSync(stats.real), atim = rstats.atime, mtim = rstats.mtime, n = nsToMs(now(constants_1.WASI_CLOCK_REALTIME)), atimflags = constants_1.WASI_FILESTAT_SET_ATIM | constants_1.WASI_FILESTAT_SET_ATIM_NOW;
            if ((fstflags & atimflags) === atimflags)
              return constants_1.WASI_EINVAL;
            let mtimflags = constants_1.WASI_FILESTAT_SET_MTIM | constants_1.WASI_FILESTAT_SET_MTIM_NOW;
          ...
          fd_pwrite: wrap((fd, iovs, iovsLen, offset, nwritten) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_WRITE | constants_1.WASI_RIGHT_FD_SEEK), written = 0;
          ...
          fd_write: wrap((fd, iovs, iovsLen, nwritten) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_WRITE), IS_STDOUT = fd == constants_1.WASI_STDOUT_FILENO, IS_STDERR = fd == constants_1.WASI_STDERR_FILENO, written = 0;
            return getiovs(iovs, iovsLen).forEach((iov) => {
              if (iov.byteLength == 0)
                return;
              if (IS_STDOUT && this.sendStdout != null)
                ...
                let w = 0;
                while (w < iov.byteLength) {
                  let i = fs.writeSync(stats.real, iov, w, iov.byteLength - w, stats.offset ? Number(stats.offset) : null);
                  if (stats.offset)
          ...
          fd_pread: wrap((fd, iovs, iovsLen, offset, nread) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_READ | constants_1.WASI_RIGHT_FD_SEEK), read = 0;
            outer:
              for (let iov of getiovs(iovs, iovsLen)) {
                let r = 0;
                while (r < iov.byteLength) {
                  let length = iov.byteLength - r, rr = fs.readSync(stats.real, iov, r, iov.byteLength - r, Number(offset) + read + r);
          ...
          fd_read: wrap((fd, iovs, iovsLen, nread) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_READ), IS_STDIN = fd == constants_1.WASI_STDIN_FILENO, read = 0;
            outer:
              for (let iov of getiovs(iovs, iovsLen)) {
                let r = 0;
                while (r < iov.byteLength) {
                  ...
                  if (IS_STDIN)
                    if (this.getStdin != null) {
                      ...
                      if (this.stdinBuffer != null) {
                        ...
                        if (rr > 0)
                          this.lastStdin = (/* @__PURE__ */ new Date()).valueOf();
                      }
                    } else {
          ...
          fd_readdir: wrap((fd, bufPtr, bufLen, cookie, bufusedPtr) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_READDIR);
            this.refreshMemory();
            let entries = fs.readdirSync(stats.path, { withFileTypes: !0 }), startPtr = bufPtr;
            for (let i = Number(cookie);i < entries.length; i += 1) {
              let entry = entries[i], nameLength = @Buffer.byteLength(entry.name);
              if (bufPtr - startPtr > bufLen)
                break;
              if (this.view.setBigUint64(bufPtr, BigInt(i + 1), !0), bufPtr += 8, bufPtr - startPtr > bufLen)
                break;
              let rstats = fs.lstatSync(path.resolve(stats.path, entry.name));
          ...
          fd_seek: wrap((fd, offset, whence, newOffsetPtr) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_SEEK);
            switch (this.refreshMemory(), whence) {
              ...
              case constants_1.WASI_WHENCE_END:
                let { size } = this.fstatSync(stats.real);
          ...
          fd_tell: wrap((fd, offsetPtr) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_TELL);
          ...
          fd_sync: wrap((fd) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_FD_SYNC);
            return fs.fsyncSync(stats.real), constants_1.WASI_ESUCCESS;
          }),
          path_create_directory: wrap((fd, pathPtr, pathLen) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_PATH_CREATE_DIRECTORY);
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let p = @Buffer.from(this.memory.buffer, pathPtr, pathLen).toString();
          ...
          path_filestat_get: wrap((fd, flags, pathPtr, pathLen, bufPtr) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_PATH_FILESTAT_GET);
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let p = @Buffer.from(this.memory.buffer, pathPtr, pathLen).toString(), rstats;
            if (flags)
          ...
          path_filestat_set_times: wrap((fd, _dirflags, pathPtr, pathLen, stAtim, stMtim, fstflags) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_PATH_FILESTAT_SET_TIMES);
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let rstats = this.fstatSync(stats.real), atim = rstats.atime, mtim = rstats.mtime, n = nsToMs(now(constants_1.WASI_CLOCK_REALTIME)), atimflags = constants_1.WASI_FILESTAT_SET_ATIM | constants_1.WASI_FILESTAT_SET_ATIM_NOW;
            if ((fstflags & atimflags) === atimflags)
              return constants_1.WASI_EINVAL;
            let mtimflags = constants_1.WASI_FILESTAT_SET_MTIM | constants_1.WASI_FILESTAT_SET_MTIM_NOW;
            if ((fstflags & mtimflags) === mtimflags)
          ...
          path_link: wrap((oldFd, _oldFlags, oldPath, oldPathLen, newFd, newPath, newPathLen) => {
            let ostats = CHECK_FD(oldFd, constants_1.WASI_RIGHT_PATH_LINK_SOURCE), nstats = CHECK_FD(newFd, constants_1.WASI_RIGHT_PATH_LINK_TARGET);
            if (!ostats.path || !nstats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let op = @Buffer.from(this.memory.buffer, oldPath, oldPathLen).toString(), np = @Buffer.from(this.memory.buffer, newPath, newPathLen).toString();
          ...
          path_open: wrap((dirfd, _dirflags, pathPtr, pathLen, oflags, fsRightsBase, fsRightsInheriting, fsFlags, fdPtr) => {
            try {
              CHECK_FD(dirfd, constants_1.WASI_RIGHT_PATH_OPEN), fsRightsBase = BigInt(fsRightsBase), fsRightsInheriting = BigInt(fsRightsInheriting);
              let read = (fsRightsBase & (constants_1.WASI_RIGHT_FD_READ | constants_1.WASI_RIGHT_FD_READDIR)) !== BigInt(0), write = (fsRightsBase & (constants_1.WASI_RIGHT_FD_DATASYNC | constants_1.WASI_RIGHT_FD_WRITE | constants_1.WASI_RIGHT_FD_ALLOCATE | constants_1.WASI_RIGHT_FD_FILESTAT_SET_SIZE)) !== BigInt(0), noflags;
              if (write && read)
              ...
              if (logOpen("path_open", p), p.startsWith("proc/"))
                throw new types_1.WASIError(constants_1.WASI_EBADF);
              let fullUnresolved = path.resolve(p), full;
              try {
              ...
              let newfd = this.getUnusedFileDescriptor();
              this.FD_MAP.set(newfd, {
                real: realfd,
                filetype: void 0,
                rights: {
          ...
          path_readlink: wrap((fd, pathPtr, pathLen, buf, bufLen, bufused) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_PATH_READLINK);
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let p = @Buffer.from(this.memory.buffer, pathPtr, pathLen).toString(), full = path.resolve(stats.path, p), r = fs.readlinkSync(full), used = @Buffer.from(this.memory.buffer).write(r, buf, bufLen);
            return this.view.setUint32(bufused, used, !0), constants_1.WASI_ESUCCESS;
          }),
          path_remove_directory: wrap((fd, pathPtr, pathLen) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_PATH_REMOVE_DIRECTORY);
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let p = @Buffer.from(this.memory.buffer, pathPtr, pathLen).toString();
          ...
          path_rename: wrap((oldFd, oldPath, oldPathLen, newFd, newPath, newPathLen) => {
            let ostats = CHECK_FD(oldFd, constants_1.WASI_RIGHT_PATH_RENAME_SOURCE), nstats = CHECK_FD(newFd, constants_1.WASI_RIGHT_PATH_RENAME_TARGET);
            if (!ostats.path || !nstats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let op = @Buffer.from(this.memory.buffer, oldPath, oldPathLen).toString(), np = @Buffer.from(this.memory.buffer, newPath, newPathLen).toString();
          ...
          path_symlink: wrap((oldPath, oldPathLen, fd, newPath, newPathLen) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_PATH_SYMLINK);
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let op = @Buffer.from(this.memory.buffer, oldPath, oldPathLen).toString(), np = @Buffer.from(this.memory.buffer, newPath, newPathLen).toString();
          ...
          path_unlink_file: wrap((fd, pathPtr, pathLen) => {
            let stats = CHECK_FD(fd, constants_1.WASI_RIGHT_PATH_UNLINK_FILE);
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let p = @Buffer.from(this.memory.buffer, pathPtr, pathLen).toString();
          ...
          poll_oneoff: (sin, sout, nsubscriptions, neventsPtr) => {
            ...
            for (let i = 0;i < nsubscriptions; i += 1) {
              let userdata = this.view.getBigUint64(sin, !0);
              sin += 8;
              let type = this.view.getUint8(sin);
              ...
              switch (type) {
                case constants_1.WASI_EVENTTYPE_CLOCK: {
                  let clockid = this.view.getUint32(sin, !0);
                  sin += 4, sin += 4;
                  let timeout = this.view.getBigUint64(sin, !0);
                  sin += 8, sin += 8;
                  let subclockflags = this.view.getUint16(sin, !0);
                  sin += 2, sin += 6;
                  let absolute = subclockflags === 1;
                  if (log.enabled)
                    log(name, { clockid, timeout, absolute });
                  if (!absolute)
                    fd_timeout_ms = timeout / BigInt(1e6);
                  let e = constants_1.WASI_ESUCCESS, t = now(clockid);
                  if (t == null)
                    e = constants_1.WASI_EINVAL;
                  else {
                    let tNS = BigInt(t), waitNs = (absolute ? timeout : tNS + timeout) - tNS;
            ...
            if (this.view.setUint32(neventsPtr, nevents, !0), nevents == 2 && fd >= 0) {
              let r = this.wasiImport.sock_pollSocket(fd, fd_type, fd_timeout_ms);
            ...
            if (waitTimeNs > 0) {
              if (waitTimeNs -= Bun.nanoseconds() - timeOrigin, waitTimeNs >= 1e6) {
                ...
                if (this.sleep != null) {
                  let ms = nsToMs(waitTimeNs);
                  this.sleep(ms);
                } else {
                  let end = BigInt(bindings2.hrtime()) + waitTimeNs;
          ...
          Object.keys(this.wasiImport).forEach((key) => {
            let prevImport = this.wasiImport[key];
      ...
      shortPause() {
        if (this.sleep == null)
          return;
        if ((/* @__PURE__ */ new Date()).valueOf() - this.lastStdin > 2000)
          this.sleep(50);
      }
      ...
      start(instance, memory) {
        ...
        if (exports2 === null || typeof exports2 !== "object")
          throw Error(`instance.exports must be an Object. Received ${exports2}.`);
        if (memory == null) {
          if (memory = exports2.memory, !(memory instanceof WebAssembly.Memory))
            throw Error(`instance.exports.memory must be a WebAssembly.Memory. Recceived ${memory}.`);
        }
        if (this.setMemory(memory), exports2._start)
          exports2._start();
      }
      getImports(module2) {
        let namespace = null, imports = WebAssembly.Module.imports(module2);
        ...
        switch (namespace) {
          ...
          default:
            throw Error(`No WASI namespace found. Only wasi_unstable and wasi_snapshot_preview1 are supported.
...
`);
      ...
      initWasiFdInfo() {
        if (this.env.WASI_FD_INFO != null) {
          let fdInfo = JSON.parse(this.env.WASI_FD_INFO);
          for (let wasi_fd in fdInfo) {
            console.log(wasi_fd);
            let fd = parseInt(wasi_fd);
            if (this.FD_MAP.has(fd))
              continue;
            let real = fdInfo[wasi_fd];
            try {
              this.fstatSync(real);
            } catch {
