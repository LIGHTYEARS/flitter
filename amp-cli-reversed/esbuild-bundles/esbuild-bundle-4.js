...
var require_punycode = __commonJS((exports, module) => {
  ...
  function mapDomain(domain, callback) {
    ...
    let labels = domain.split("."),
      encoded = map(labels, callback).join(".");
  ...
  var ucs2encode = (codePoints) => String.fromCodePoint(...codePoints),
    ...
    decode = function (input) {
      ...
      for (let index = basic > 0 ? basic + 1 : 0; index < inputLength; ) {
        ...
        for (let w = 1, k = 36; ; k += 36) {
          ...
          if (digit > floor((2147483647 - i) / w)) error("overflow");
          i += digit * w;
          let t = k <= bias ? 1 : k >= bias + 26 ? 26 : k - bias;
          if (digit < t) break;
          let baseMinusT = 36 - t;
          if (w > floor(2147483647 / baseMinusT)) error("overflow");
          w *= baseMinusT;
        }
        let out = output.length + 1;
        if (
          ((bias = adapt(i - oldi, out, oldi == 0)),
          floor(i / out) > 2147483647 - n)
        )
          error("overflow");
    ...
    encode = function (input) {
      ...
      while (handledCPCount < inputLength) {
        ...
        for (let currentValue of input) {
          if (currentValue < n && ++delta > 2147483647) error("overflow");
          if (currentValue === n) {
            let q = delta;
            for (let k = 36; ; k += 36) {
              let t = k <= bias ? 1 : k >= bias + 26 ? 26 : k - bias;
...
__export(exports_punycode, { default: () => import_punycode.default });
__reExport(exports_punycode, __toESM(require_punycode(), 1));
var import_punycode = __toESM(require_punycode(), 1);
var export_default = import_punycode.default;
