// Module: micromark-parser
// Original: segment1[1793699:2026951]
// Type: Scope-hoisted
// Exports: IfT, Wf0, qf0, Yf0, fYT, IYT, YtT, Qf0, Zf0, Jf0, TI0, rI0, hI0, iI0, cI0, sI0, oI0, ZtT, zY, nI0
// Category: rendering

tring.fromCodePoint(I$(T) ? q4(T) : T)
}
}
_stateRcdataLessThanSign(T) {
  if (T === HT.SOLIDUS) this.state = zT.RCDATA_END_TAG_OPEN;
  else this._emitChars("<"), this.state = zT.RCDATA, this._stateRcdata(T)
}
_stateRcdataEndTagOpen(T) {
  if (Pl(T)) this.state = zT.RCDATA_END_TAG_NAME, this._stateRcdataEndTagName(T);
  else this._emitChars("</"), this.state = zT.RCDATA, this._stateRcdata(T)
}
...
_stateRawtextLessThanSign(T) {
  if (T === HT.SOLIDUS) this.state = zT.RAWTEXT_END_TAG_OPEN;
  else this._emitChars("<"), this.state = zT.RAWTEXT, this._stateRawtext(T)
}
_stateRawtextEndTagOpen(T) {
  if (Pl(T)) this.state = zT.RAWTEXT_END_TAG_NAME, this._stateRawtextEndTagName(T);
  else this._emitChars("</"), this.state = zT.RAWTEXT, this._stateRawtext(T)
}
...
_stateScriptDataEndTagOpen(T) {
  if (Pl(T)) this.state = zT.SCRIPT_DATA_END_TAG_NAME, this._stateScriptDataEndTagName(T);
  else this._emitChars("</"), this.state = zT.SCRIPT_DATA, this._stateScriptData(T)
}
_stateScriptDataEndTagName(T) {
  if (this.handleSpecialEndTag(T)) this._emitChars("</"), this.state = zT.SCRIPT_DATA, this._stateScriptData(T)
}
_stateScriptDataEscapeStart(T) {
  if (T === HT.HYPHEN_MINUS) this.state = zT.SCRIPT_DATA_ESCAPE_START_DASH, this._emitChars("-");
  else this.state = zT.SCRIPT_DATA, this._stateScriptData(T)
}
_stateScriptDataEscapeStartDash(T) {
  if (T === HT.HYPHEN_MINUS) this.state = zT.SCRIPT_DATA_ESCAPED_DASH_DASH, this._emitChars("-");
  else this.state = zT.SCRIPT_DATA, this._stateScriptData(T)
}
...
_stateScriptDataEscapedLessThanSign(T) {
  if (T === HT.SOLIDUS) this.state = zT.SCRIPT_DATA_ESCAPED_END_TAG_OPEN;
  else if (Pl(T)) this._emitChars("<"), this.state = zT.SCRIPT_DATA_DOUBLE_ESCAPE_START, this._stateScriptDataDoubleEscapeStart(T);
  else this._emitChars("<"), this.state = zT.SCRIPT_DATA_ESCAPED, this._stateScriptDataEscaped(T)
}
_stateScriptDataEscapedEndTagOpen(T) {
  if (Pl(T)) this.state = zT.SCRIPT_DATA_ESCAPED_END_TAG_NAME, this._stateScriptDataEscapedEndTagName(T);
  else this._emitChars("</"), this.state = zT.SCRIPT_DATA_ESCAPED, this._stateScriptDataEscaped(T)
}
...
_stateScriptDataDoubleEscapedLessThanSign(T) {
  if (T === HT.SOLIDUS) this.state = zT.SCRIPT_DATA_DOUBLE_ESCAPE_END, this._emitChars("/");
  else this.state = zT.SCRIPT_DATA_DOUBLE_ESCAPED, this._stateScriptDataDoubleEscaped(T)
}
...
_stateMarkupDeclarationOpen(T) {
  if (this._consumeSequenceIfMatch(sr.DASH_DASH, !0)) this._createCommentToken(sr.DASH_DASH.length + 1), this.state = zT.COMMENT_START;
  else if (this._consumeSequenceIfMatch(sr.DOCTYPE, !1)) this.currentLocation = this.getCurrentLocation(sr.DOCTYPE.length + 1), this.state = zT.DOCTYPE;
  else if (this._consumeSequenceIfMatch(sr.CDATA_START, !0))
    if (this.inForeignNode) this.state = zT.CDATA_SECTION;
    else this._err(vR.cdataInHtmlContent), this._createCommentToken(sr.CDATA_START.length + 1), this.currentToken.data = "[CDATA[", this.state = zT.BOGUS_COMMENT;
...
_stateCommentLessThanSignBang(T) {
  if (T === HT.HYPHEN_MINUS) this.state = zT.COMMENT_LESS_THAN_SIGN_BANG_DASH;
  else this.state = zT.COMMENT, this._stateComment(T)
}
_stateCommentLessThanSignBangDash(T) {
  if (T === HT.HYPHEN_MINUS) this.state = zT.COMMENT_LESS_THAN_SIGN_BANG_DASH_DASH;
  else this.state = zT.COMMENT_END_DASH, this._stateCommentEndDash(T)
}
...
_stateBeforeDoctypeName(T) {
  if (I$(T)) this._createDoctypeToken(String.fromCharCode(q4(T))), this.state = zT.DOCTYPE_NAME;
  else switch (T) {
    case HT.SPACE:
    case HT.LINE_FEED:
    case HT.TABULATION:
...
_stateAfterDoctypeName(T) {
  ...
  switch (T) {
    ...
    default:
      if (this._consumeSequenceIfMatch(sr.PUBLIC, !1)) this.state = zT.AFTER_DOCTYPE_PUBLIC_KEYWORD;
      else if (this._consumeSequenceIfMatch(sr.SYSTEM, !1)) this.state = zT.AFTER_DOCTYPE_SYSTEM_KEYWORD;
      else if (!this._ensureHibernation()) this._err(vR.invalidCharacterSequenceAfterDoctypeName), R.forceQuirks = !0, this.state = zT.BOGUS_DOCTYPE, this._stateBogusDoctype(T)
  }
}
...
_stateCdataSectionBracket(T) {
  if (T === HT.RIGHT_SQUARE_BRACKET) this.state = zT.CDATA_SECTION_END;
  else this._emitChars("]"), this.state = zT.CDATA_SECTION, this._stateCdataSection(T)
}
...
_stateCharacterReference() {
  ...
  if (T === 0) this.preprocessor.pos = this.entityStartPos, this._flushCodePointConsumedAsCharacterReference(HT.AMPERSAND), this.state = !this._isCharacterReferenceInAttribute() && ufT(this.preprocessor.peek(1)) ? zT.AMBIGUOUS_AMPERSAND : this.returnState;
  else this.state = this.returnState
}
_stateAmbiguousAmpersand(T) {
  if (ufT(T)) this._flushCodePointConsumedAsCharacterReference(T);
  else {
...
class uYT {
  ...
  popUntilTagNamePopped(T) {
    ...
    while (R > 0 && this.treeAdapter.getNamespaceURI(this.items[R]) !== VR.HTML);
  ...
  remove(T) {
    let R = this._indexOf(T);
    if (R >= 0)
      if (R === this.stackTop) this.pop();
      else this.items.splice(R, 1), this.tagIDs.splice(R, 1), this.stackTop--, this._updateCurrentElement(), this.handler.onItemPop(T, !1)
  }
...
class yYT {
  ...
  clearToLastMarker() {
    ...
    if (T >= 0) this.entries.splice(0, T + 1);
...
class QtT {
  ...
  onItemPop(T, R) {
    ...
    if ((e = (a = this.treeAdapter).onItemPop) === null || e === void 0 || e.call(a, T, this.openElements.current), R) {
      let t, r;
      if (this.openElements.stackTop === 0 && this.fragmentContext) t = this.fragmentContext, r = this.fragmentContextID;
      else({
        current: t,
        currentTagId: r
      } = this.openElements);
  ...
  _insertCharacters(T) {
    ...
    if (this._shouldFosterParentOnInsertion())
      if ({
          parent: R,
          beforeElement: a
        } = this._findFosterParentingLocation(), a) this.treeAdapter.insertTextBefore(R, T.chars, a);
      else this.treeAdapter.insertText(R, T.chars);
  ...
  shouldProcessStartTagTokenInForeignContent(T) {
    ...
    let R, a;
    if (this.openElements.stackTop === 0 && this.fragmentContext) R = this.fragmentContext, a = this.fragmentContextID;
    else({
      current: R,
      currentTagId: a
    } = this.openElements);
  ...
  _fosterParentElement(T) {
    ...
    if (R.beforeElement) this.treeAdapter.insertBefore(R.parent, T, R.beforeElement);
    else this.treeAdapter.appendChild(R.parent, T)
  }
  ...
  _processStartTag(T) {
    if (this.shouldProcessStartTagTokenInForeignContent(T)) Mg0(this, T);
    else this._startTagOutsideForeignContent(T)
  }
  ...
  onEndTag(T) {
    if (this.skipNextNewLine = !1, this.currentToken = T, this.currentNotInHTML) Dg0(this, T);
    else this._endTagOutsideForeignContent(T)
  }
...
function rI0(T, R) {
  ...
  if (a) {
    if (!T.openElements.contains(a.element)) T.activeFormattingElements.removeEntry(a), a = null;
    else if (!T.openElements.hasInScope(R.tagID)) a = null
  } else OYT(T, R);
  return a
}
...
function sI0(T, R, a) {
  let e = T.treeAdapter.getTagName(R),
    t = DH(e);
  if (T._isElementCausesFosterParenting(t)) T._fosterParentElement(a);
  else {
...
function pI0(T, R) {
  if (R.tagID === sT.HTML) T._insertElement(R, VR.HTML), T.insertionMode = YT.BEFORE_HEAD;
  else Ev(T, R)
}
...
function mI0(T, R) {
  let a = R.tagID;
  if (a === sT.HEAD || a === sT.BODY || a === sT.HTML || a === sT.BR) Cv(T, R);
...
function Bc(T, R) {
  switch (R.tagID) {
    ...
    case sT.NOSCRIPT: {
      if (T.options.scriptingEnabled) T._switchToTextParsing(R, gr.RAWTEXT);
      else T._insertElement(R, VR.HTML), T.insertionMode = YT.IN_HEAD_NO_SCRIPT;
      break
    }
    case sT.NOFRAMES:
    case sT.STYLE: {
...
function Mv(T, R) {
  let a = R.type === u8.EOF ? vR.openElementsLeftAfterEof : vR.disallowedContentInNoscriptInHead;
  T._err(R, a), T.openElements.pop(), T.insertionMode = YT.IN_HEAD, T._processToken(R)
}
...
function CI0(T, R) {
  let a = T.activeFormattingElements.getElementEntryInScopeWithTagName(pR.A);
  if (a) ZtT(T, R), T.openElements.remove(a.element), T.activeFormattingElements.removeEntry(a);
  T._reconstructActiveFormattingElements(), T._insertElement(R, VR.HTML), T.activeFormattingElements.pushElement(T.openElements.current, R)
}
...
function MI0(T, R) {
  if (T._reconstructActiveFormattingElements(), T.openElements.hasInScope(sT.NOBR)) ZtT(T, R), T._reconstructActiveFormattingElements();
  T._insertElement(R, VR.HTML), T.activeFormattingElements.pushElement(T.openElements.current, R)
}
...
function XI0(T, R) {
  if (T._reconstructActiveFormattingElements(), fYT(R), YtT(R), R.selfClosing) T._appendElement(R, VR.MATHML);
  else T._insertElement(R, VR.MATHML);
  R.ackSelfClosing = !0
}

function YI0(T, R) {
  if (T._reconstructActiveFormattingElements(), IYT(R), YtT(R), R.selfClosing) T._appendElement(R, VR.SVG);
  else T._insertElement(R, VR.SVG);
  R.ackSelfClosing = !0
}
...
function bt(T, R) {
  switch (R.tagID) {
    ...
    case sT.NOSCRIPT: {
      if (T.options.scriptingEnabled) vfT(T, R);
      else jfT(T, R);
      break
    }
    case sT.PLAINTEXT: {
...
function Tg0(T) {
  ...
  if ((a || R) && T.openElements.hasInScope(sT.FORM)) {
    if (T.openElements.generateImpliedEndTags(), R) T.openElements.popUntilTagNamePopped(sT.FORM);
    else if (a) T.openElements.remove(a)
  }
}
...
function dYT(T, R) {
  if (T.tmplInsertionModeStack.length > 0) BYT(T, R);
  else JtT(T, R)
}
...
function _g0(T, R) {
  if (SYT(R)) T._appendElement(R, VR.HTML);
  else ZO(T, R);
  R.ackSelfClosing = !0
}
...
function xg(T, R) {
  ...
  if (T.hasNonWhitespacePendingCharacterToken)
    for (; a < T.pendingCharacterTokens.length; a++) ZO(T, T.pendingCharacterTokens[a]);
  else
    for (; a < T.pendingCharacterTokens.length; a++) T._insertCharacters(T.pendingCharacterTokens[a]);
...
function xg0(T, R) {
  let a = R.tagID;
  if (a === sT.CAPTION || a === sT.TABLE || a === sT.TBODY || a === sT.TFOOT || a === sT.THEAD || a === sT.TR || a === sT.TD || a === sT.TH) T.openElements.popUntilTagNamePopped(sT.SELECT), T._resetInsertionMode(), T._processStartTag(R);
  else DYT(T, R)
}
...
function BYT(T, R) {
  if (T.openElements.tmplCount > 0) T.openElements.popUntilTagNamePopped(sT.TEMPLATE), T.activeFormattingElements.clearToLastMarker(), T.tmplInsertionModeStack.shift(), T._resetInsertionMode(), T.onEof(R);
  else JtT(T, R)
}

function $g0(T, R) {
  if (R.tagID === sT.HTML) bt(T, R);
  else Zw(T, R)
}
...
function Mg0(T, R) {
  if (Yf0(R)) UYT(T), T._startTagOutsideForeignContent(R);
  else {
    let a = T._getAdjustedCurrentElement(),
      e = T.treeAdapter.getNamespaceURI(a);
    if (e === VR.MATHML) fYT(R);
    else if (e === VR.SVG) Qf0(R), IYT(R);
    if (YtT(R), R.selfClosing) T._appendElement(R, e);
    else T._insertElement(R, e);
    R.ackSelfClosing = !0
  }
}
...
function vh(T, R, a, e) {
  ...
  if (a = a > 0 ? a : 0, e.length < 1e4) h = Array.from(e), h.unshift(R, a), T.splice(...h);
  else {
    if (a) T.splice(R, a);
...
function n$0(T) {
  ...
  function m(u, P) {
    ...
    if (r = u, t.defineSkip(u.start), t.write(k), R.parser.lazy[u.start.line]) {
      ...
      while (x--)
        if (t.events[x][1].start.offset < h && (!t.events[x][1].end || t.events[x][1].end.offset > h)) return;
...
function A$0(T, R) {
  ...
  while (++a < T.length)
    if (T[a][0] === "enter" && T[a][1].type === "attentionSequence" && T[a][1]._close) {
      e = a;
      while (e--)
        if (T[e][0] === "exit" && T[e][1].type === "attentionSequence" && T[e][1]._open && R.sliceSerialize(T[e][1]).charCodeAt(0) === R.sliceSerialize(T[a][1]).charCodeAt(0)) {
          ...
          if (wfT(l, -c), wfT(o, c), h = {
              ...
              ...i.end
            }, s = [], T[e][1].end.offset - T[e][1].start.offset) s = ni(s, [
            ["enter", T[e][1], R],
            ["exit", T[e][1], R]
          ]);
          if (s = ni(s, [
              ...
              ["exit", h, R],
              ["enter", r, R]
            ]), s = ni(s, WH(R.parser.constructs.insideSpan.null, T.slice(e + 1, a), R)), s = ni(s, [
              ["exit", r, R],
              ["enter", i, R],
              ["exit", i, R],
              ["exit", t, R]
            ]), T[a][1].end.offset - T[a][1].start.offset) A = 2, s = ni(s, [
            ["enter", T[a][1], R],
            ["exit", T[a][1], R]
          ]);
          else A = 0;
  ...
  a = -1;
  while (++a < T.length)
    if (T[a][1].type === "attentionSequence") T[a][1].type = "data";
  return T
}
...
function y$0(T, R, a) {
  ...
  function t(h) {
    if (Y9(h)) return _8(T, r, "linePrefix", e.parser.constructs.disable.null.includes("codeIndented") ? void 0 : 4)(h);
    return r(h)
  }

  function r(h) {
    return T.attempt(zYT, R, a)(h)
  }
}
...
function fg(T, R) {
  ...
  if (R.length < 1e4) T.push(...R);
    ...
    while (a < R.length) T.push(...R.slice(a, a + 1e4)), a += 1e4
}

function VYT(T) {
  let R = {},
    a = -1,
    e, t, r, h, i, c, s, A = new KYT(T);
  while (++a < A.length) {
    while (a in R) a = R[a];
    if (e = A.get(a), a && e[1].type === "chunkFlow" && A.get(a - 1)[1].type === "listItemPrefix") {
      ...
      if (r < c.length && c[r][1].type === "content")
        while (++r < c.length) {
    ...
    if (e[0] === "enter") {
      ...
      r = a, t = void 0;
      while (r--)
        if (h = A.get(r), h[1].type === "lineEnding" || h[1].type === "lineEndingBlank") {
          if (h[0] === "enter") {
            if (t) A.get(t)[1].type = "lineEndingBlank";
            h[1].type = "lineEnding", t = r
          }
        }
      else if (h[1].type === "linePrefix" || h[1].type === "listItemIndent");
      else break;
      if (t) e[1].end = {
        ...A.get(t)[1].start
...
function E$0(T, R) {
  ...
  n = a;
  while (++o < i.length)
    if (i[o][0] === "exit" && i[o - 1][0] === "enter" && i[o][1].type === i[o - 1][1].type && i[o][1].start.line !== i[o][1].end.line) _ = o + 1, m.push(_), n._tokenizer = void 0, n.previous = void 0, n = n.next;
  if (h.events = [], n) n._tokenizer = void 0, n.previous = void 0;
  else m.pop();
  o = m.length;
  while (o--) {
...
function Bv(T, R) {
  ...
  function e(t) {
    if (r9(t)) return T.enter("lineEnding"), T.consume(t), T.exit("lineEnding"), a = !0, e;
    if (Y9(t)) return _8(T, e, a ? "linePrefix" : "lineSuffix")(t);
    return R(t)
  }
}
...
function H$0(T, R, a) {
  ...
  function e(i) {
    return o3(i) ? Bv(T, t)(i) : a(i)
  }

  function t(i) {
    return QYT(T, r, a, "definitionTitle", "definitionTitleMarker", "definitionTitleString")(i)
  }

  function r(i) {
    return Y9(i) ? _8(T, h, "whitespace")(i) : h(i)
  }

  function h(i) {
    return i === null || r9(i) ? R(i) : a(i)
  }
}
...
function q$0(T, R, a) {
  ...
  function e(r) {
    return T.enter("hardBreakEscape"), T.consume(r), t
  }

  function t(r) {
    if (r9(r)) return T.exit("hardBreakEscape"), R(r);
    return a(r)
  }
}
...
function Q$0(T) {
  let R = T.length;
  while (R--)
    if (T[R][0] === "enter" && T[R][1].type === "htmlFlow") break;
...
function iv0(T, R) {
  ...
  return i = [
    ["enter", c, R],
    ["enter", s, R]
  ], i = ni(i, T.slice(r + 1, r + e + 3)), i = ni(i, [
    ["enter", A, R]
  ]), i = ni(i, WH(R.parser.constructs.insideSpan.null, T.slice(r + e + 4, h - 3), R)), i = ni(i, [
    ["exit", A, R], T[h - 2], T[h - 1],
    ["exit", s, R]
  ]), i = ni(i, T.slice(h + 1)), i = ni(i, [
    ["exit", c, R]
  ]), vh(T, r, T.length, i), T
}
...
function cv0(T, R, a) {
  let e = this,
    ...
    r, h;
  while (t--)
    if ((e.events[t][1].type === "labelImage" || e.events[t][1].type === "labelLink") && !e.events[t][1]._balanced) {
      r = e.events[t][1];
      break
    }
  return i;
...
function ov0(T, R, a) {
  ...
  function t(i) {
    return YYT.call(e, T, r, h, "reference", "referenceMarker", "referenceString")(i)
  }

  function r(i) {
    return e.parser.defined.includes(_c(e.sliceSerialize(e.events[e.events.length - 1][1]).slice(1, -1))) ? R(i) : a(i)
  }
...
function nv0(T, R, a) {
  ...
  function e(r) {
    return T.enter("reference"), T.enter("referenceMarker"), T.consume(r), T.exit("referenceMarker"), t
  }

  function t(r) {
    if (r === 93) return T.enter("referenceMarker"), T.consume(r), T.exit("referenceMarker"), T.exit("reference"), R;
    return a(r)
  }
}
...
function _v0(T, R, a) {
  ...
  function t(h) {
    return T.enter("labelLink"), T.enter("labelMarker"), T.consume(h), T.exit("labelMarker"), T.exit("labelLink"), r
  }

  function r(h) {
    return h === 94 && "_hiddenFootnoteSupport" in e.parser.constructs ? a(h) : R(h)
  }
}
...
function $v0(T, R, a) {
  ...
  function r(s) {
    let A = e.events.length,
      l;
    while (A--)
      if (e.events[A][1].type !== "lineEnding" && e.events[A][1].type !== "linePrefix" && e.events[A][1].type !== "content") {
  ...
  while (++a <= T.length)
    if ((a === T.length || T[a][1].type === "lineEnding") && T[a - 1][1].type === "data") {
      ...
      if (i) {
        ...
        if (e.end = {
            ...s.start
          }, e.start.offset === e.end.offset) Object.assign(e, s);
        else T.splice(a, 0, ["enter", s, R], ["exit", s, R]), a += 2
      }
      a++
    }
  return T
...
function Wv0(T, R, a) {
  ...
  function P(j) {
    if (r9(j)) e.line++, e.column = 1, e.offset += j === -3 ? 2 : 1, O();
    else if (j !== -1) e.column++, e.offset++;
    if (e._bufferIndex < 0) e._index++;
    else if (e._bufferIndex++, e._bufferIndex === h[e._index].length) e._bufferIndex = -1, e._index++;
    A.previous = j, c = !0
  }
...
function qv0(T, R) {
  let a = R.start._index,
    ...
    h;
  if (a === t) h = [T[a].slice(e, r)];
  else {
    if (h = T.slice(a, t), e > -1) {
      let i = h[0];
      if (typeof i === "string") h[0] = i.slice(e);
      else h.shift()
    }
...
function Fv0(T) {
  let R = {
    constructs: WYT([TQT, ...(T || {}).extensions || []]),
    content: a(c$0),
    defined: [],
    document: a(o$0),
    flow: a(vv0),
...
function Kv0() {
  ...
  function t(r, h, i) {
    ...
    while (l < r.length) {
      ...
      if (n === 10 && l === o && e) c.push(-3), e = void 0;
      else {
...
function Qv0(T) {
  ...
  function e(fT) {
    let MT = {
      ...
      hR = -1;
    while (++hR < fT.length)
      if (fT[hR][1].type === "listOrdered" || fT[hR][1].type === "listUnordered")
        if (fT[hR][0] === "enter") QT.push(hR);
        else {
          let cR = QT.pop();
          hR = t(fT, cR, hR)
        }
  ...
  function s(fT, MT) {
    ...
    else if (QT[0].type !== fT.type)
      if (MT) MT.call(this, fT, QT[0]);
      else(QT[1] || qfT).call(this, fT, QT[0]);
  ...
  function aT(fT) {
    let MT = this.sliceSerialize(fT),
      ...
      QT;
    if (UT) QT = qYT(MT, UT === "characterReferenceMarkerNumeric" ? 10 : 16), this.data.characterReferenceType = void 0;
    else QT = arT(MT);
...
function qfT(T, R) {
  ...
  else throw Error("Cannot close document, a token (`" + R.type + "`, " + wv({
    start: R.start,
    end: R.end
  }) + ") is still open")
}
...
function tQT(T, R) {
  let a = -1,
    e;
  if (R.extensions)
    while (++a < R.extensions.length) tQT(T, R.extensions[a]);
  for (e in R)
    if (Rj0.call(R, e)) switch (e) {
      case "extensions":
        break;
      case "unsafe": {
        FfT(T[e], R[e]);
...
function GfT(T, R, a) {
  ...
  let e = -1;
  while (++e < R.length)
    if (T.includes(R[e])) return !0;
  return !1
}

function KfT(T, R, a, e) {
  let t = -1;
  while (++t < a.unsafe.length)
    if (a.unsafe[t].character === `
...
function sj0(T, R, a, e) {
  ...
  else i = a.enter("destinationRaw"), s += c.move(a.safe(T.url, {
    before: s,
    after: T.title ? " " : `
`,
    ...c.current()
...
function cQT(T, R, a, e) {
  let t;
  if (typeof R === "function" && typeof a !== "function") e = a, a = R;
  else t = R;
  let r = qH(t),
    h = e ? -1 : 1;
  i(T, void 0, [])();
...
function kj0(T, R, a, e) {
  let t, r, h;
  if (typeof R === "function" && typeof a !== "function") r = void 0, h = R, t = a;
  else r = R, h = a, t = e;
  cQT(T, r, i, t);
...
function lQT(T, R, a, e) {
  let t = T.referenceType,
    r = a.enter("imageReference"),
    h = a.enter("label"),
    i = a.createTracker(e),
    c = i.move("!["),
    s = a.safe(T.alt, {
  ...
  if (h(), a.stack = A, r(), t === "full" || !s || s !== l) c += i.move(l + "]");
  else if (t === "shortcut") c = c.slice(0, -1);
  else c += i.move("]");
  return c
}
...
function _QT(T, R, a, e) {
  ...
  else c = a.enter("destinationRaw"), s += h.move(a.safe(T.url, {
    before: s,
    after: T.title ? " " : ")",
    ...h.current()
  }));
...
function Zj0(T, R, a) {
  ...
  while (++i < t.length) {
    ...
    if (c = A, /[!-/:-@[-`{-~]/.test(e.charAt(A)) && (!a.encode || !a.encode.includes(e.charAt(A)))) r.push("\\");
    else r.push(LA(e.charCodeAt(A))), c++
  }
...
function sS0() {
  ...
  function a(...t) {
    ...
    function i(c, ...s) {
      ...
      while (++l < t.length)
        if (s[l] === null || s[l] === void 0) s[l] = t[l];
      if (t = s, A) oS0(A, i)(...s);
      else h(null, ...s)
    }
  }
...
function oS0(T, R) {
  ...
  function e(...h) {
    ...
    if (!i)
      if (c && c.then && typeof c.then === "function") c.then(r, t);
      else if (c instanceof Error) t(c);
    else r(c)
  }
  ...
  function i(c, s) {
    ...
    while (b) {
      ...
      if (typeof P === "string") P = P.length > 0 ? {
        ...
        void 0;
      if (P === !1) l.lastIndex = y + 1;
      else {
        if (n !== y) m.push({
          type: "text",
          value: c.value.slice(n, y)
        });
        if (Array.isArray(P)) m.push(...P);
        else if (P) m.push(P);
        n = y + b[0].length, _ = !0
      }
      if (!l.global) break;
...
function zS0() {
  return {
    enter: {
      gfmFootnoteCallString: MS0,
      gfmFootnoteCall: DS0,
      gfmFootnoteDefinitionLabelString: wS0,
      gfmFootnoteDefinition: BS0
    },
    exit: {
      gfmFootnoteCallString: NS0,
      gfmFootnoteCall: US0,
      gfmFootnoteDefinitionLabelString: HS0,
      gfmFootnoteDefinition: WS0
    }
  }
}
    ...
    while (++l < s) {
      ...
      if (a.alignDelimiters !== !1) {
        ...
        if (x === 114) u = " ".repeat(k);
          ...
          if (k % 2) u = " ".repeat(k / 2 + 0.5), P = " ".repeat(k / 2 - 0.5);
          else u = " ".repeat(k / 2), P = u;
        else P = " ".repeat(k)
      }
...
function nO0(T) {
  ...
  if (R && R.type === "listItem" && typeof R.checked === "boolean") {
    ...
    if (e && e.type === "text") {
      ...
      if (h === a) {
        if (e.value = e.value.slice(1), e.value.length === 0) a.children.shift();
        else if (a.position && e.position && typeof e.position.start.offset === "number") e.position.start.column++, e.position.start.offset++, a.position.start = Object.assign({}, e.position.start)
      }
    }
  }
  this.exit(T)
...
function yO0(T, R, a) {
  ...
  function t(h) {
    if (h !== 87 && h !== 119 || !jQT.call(e, e.previous) || crT(e.events)) return a(h);
    return T.enter("literalAutolink"), T.enter("literalAutolinkWww"), T.check(_O0, T.attempt(fQT, T.attempt(IQT, r), a), a)(h)
  }

  function r(h) {
    return T.exit("literalAutolinkWww"), T.exit("literalAutolink"), R(h)
  }
}
...
function SO0(T, R) {
  ...
  let h = {
    ...
    c = [T[a + 1], T[a + 2],
      ["enter", t, R], T[a + 3], T[a + 4],
      ["enter", r, R],
      ["exit", r, R],
      ["enter", h, R],
      ["enter", i, R],
      ["exit", i, R],
      ["exit", h, R], T[T.length - 2], T[T.length - 1],
      ["exit", t, R]
    ];
...
function MO0(T) {
  ...
  function e(r, h) {
    let i = -1;
    while (++i < r.length)
      if (r[i][0] === "enter" && r[i][1].type === "strikethroughSequenceTemporary" && r[i][1]._close) {
        let c = i;
        while (c--)
          if (r[c][0] === "exit" && r[c][1].type === "strikethroughSequenceTemporary" && r[c][1]._open && r[i][1].end.offset - r[i][1].start.offset === r[c][1].end.offset - r[c][1].start.offset) {
            ...
            vh(l, l.length, 0, [
              ...
              ["exit", r[i][1], h],
              ["exit", s, h]
            ]), vh(r, c - 1, i - c + 3, l), i = c + l.length - 2;
            break
          }
      }
    i = -1;
    while (++i < r.length)
      if (r[i][1].type === "strikethroughSequenceTemporary") r[i][1].type = "data";
    return r
  }
...
function NO0(T, R, a) {
  ...
  function i(I) {
    ...
    while (S > -1) {
      ...
      if (d === "lineEnding" || d === "linePrefix") S--;
      else break
    }
...
function UO0(T, R) {
  ...
  while (++a < T.length) {
    ...
    if (n[0] === "enter") {
      ...
      } else if (p.type === "tableCellDivider")
        if (e) e = !1;
        else {
          if (r[1] !== 0) h[0] = h[1], l = F4(o, R, r, t, void 0, l);
          r = h, h = [r[1], a, 0, 0]
        }
    } else if (p.type === "tableHead") i = !0, c = a;
    else if (p.type === "tableRow" || p.type === "tableDelimiterRow") {
      if (c = a, r[1] !== 0) h[0] = h[1], l = F4(o, R, r, t, a, l);
      else if (h[1] !== 0) l = F4(o, R, h, t, a, l);
      t = 0
...
function F4(T, R, a, e, t, r) {
  let h = e === 1 ? "tableHeader" : e === 2 ? "tableDelimiter" : "tableData",
    i = "tableContent";
  if (a[0] !== 0) r.end = Object.assign({}, Wy(R.events, a[0])), T.add(a[0], 0, [
    ["exit", r, R]
  ]);
  ...
  if (t !== void 0) r.end = Object.assign({}, Wy(R.events, t)), T.add(t, 0, [
    ["exit", r, R]
  ]), r = void 0;
  return r
}
...
function iIT(T, R, a) {
  ...
  for (let i = 0; i < r; i++)
    if (T[i] === 0) return !0;
    else if ((T[i] < 7 || T[i] > 14) && (T[i] < 32 || T[i] > 127)) {
...
`), T.exit(1);
  ...
  let a = await T.settings.get("permissions", T.scope) ?? [],
    e = fj(a),
    t = "";
  if (e.success) t = J2(e.data);
  else {
    let c = [],
      s = [];
    for (let l = 0; l < a.length; l++) try {
      let o = fj([a[l]]);
      if (o.success && o.data[0]) c.push(o.data[0]);
      else {
  ...
  try {
    ...
    do {
      ...
      let A = await od0(r, "utf-8"),
        l = await DQT(A);
      if (l.success) await T.settings.set("permissions", l.entries, T.scope), s = !1;
      else if (c = l.contentWithErrors, s = !0, i++, i > 3) T.stderr.write("aborting, errors unresolved after multiple edit attempts"), T.exit(1)
    }
    while (s);
    T.exit(0)
  } catch (c) {
...
function ud0(T, R, a) {
  let {
    command: e,
    editorArgs: t
  } = wQT(T), r = RQ.basename(e).toLowerCase(), h = a?.line, i = a?.column;
...
class BQT {
  ...
  show(T, R = "success", a = aQ) {
    ...
    if (this._visibleToasts.length < nIT) this._visibleToasts.push(r), this._startTimer(e, a);
    else this._queuedToasts.push({
      ...r,
      duration: a
    });
...
function $d0(T, R) {
  switch (T) {
    case "error":
      return {
        color: R.toolError, icon: "\u2715"
      };
    case "warning":
      return {
        color: R.recommendation, icon: ""
      };
    case "success":
    default:
      return {
        color: R.toolSuccess, icon: "\u2713"
      }
  }
}
...
function Hd0(T) {
  ...
  function Zd0(T) {
    ...
    else if (T >= 1024) return `${Math.round(T/1024)} KB`;
    return `${T} bytes`
  }
...
${e}`
  ...
  function rE0(T) {
    switch (T.kind) {
      case "bash":
        return {
          name: "Bash", detail: T.command
        };
      case "edit":
        return {
          name: "Edit", detail: `${T.path} +${T.linesAdded} -${T.linesDeleted}`
        };
      case "create-file":
        return {
          name: "Create", detail: `${T.path} (${T.linesAdded} ${o9(T.linesAdded,"line")})`
        };
      case "painter":
        return {
          name: "Painter", detail: T.prompt ?? T.title
        };
      case "mermaid":
        return {
          name: "Mermaid"
...
`).filter(Boolean);
  ...
  class uZT {
    ...
    _handleTextChange = () => {
      ...
      if (!t) {
        if (this.currentState.isActive) this.dismiss();
        else this._clearPendingBuildTimer();
        return
      }
...
` || i === "\t";
      ...
      if (l === 0) t.push(h);
      else if (l >= 1) t.push(i);
      else if (c) t.push(l < 0.7 ? " " : i);
      else if (s) t.push(l < 0.3 ? h : " ");
      else if (l < 0.5) {
        let o = l * 2;
        if (Math.random() < o) t.push(V4[Math.floor(Math.random() * V4.length)]);
        else t.push(h)
      } else {
        let o = (l - 0.5) * 2;
        if (Math.random() < o) t.push(i);
        else t.push(V4[Math.floor(Math.random() * V4.length)])
      }
    }
  ...
  async function gIT(T, R) {
    ...
    let o = ezT(s),
      ...
      }, y = new vw(_, {}, !0), u = new mrT(y), P = new DXT(h, i, s, {
...
Examples:
  ...
  amp mcp add project-specific --workspace -- npx -y @some/server`).option("--env <kv>", "Environment variables as KEY=VALUE pairs (repeatable)", $IT, []).option("--header <kv>", "HTTP headers as KEY=VALUE pairs for URL-based MCP servers (repeatable)", $IT, []).option("--workspace", "Target workspace settings instead of global settings").allowUnknownOption(!1).action(async (e, t, r, h) => {
            ...
            try {
              ...
              if (t.length === 0) {
                if (r.env && r.env.length > 0) throw Error("No command provided a