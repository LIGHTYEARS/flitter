// Module: tui-thread-widgets
// Original: segment1[2238928:2304009]
// Type: Scope-hoisted
// Exports: ogT, LQ, _W, xN0, IN0, KTR, gN0, $N0, vN0, jN0, SN0, ON0, dN0, EN0, VTR, CN0, LN0, MN0, YP, DN0
// Category: rendering
...
function cRR(T, R) {
  let a = GN0(R);
  if (T >= a.danger) return "danger";
  else if (T >= a.warning) return "warning";
  else if (T >= a.recommendation) return "recommendation";
  return !1
}
...
function PU0(T, R, a, e, t) {
  ...
  let i;
  if (e) i = Math.max(DQ, r - h);
  else i = Math.min(a, r + h);
  let c = r - i,
    s = Math.round(c * t),
    A = c - s,
    l = T + s,
...
function kU0(T, R, a, e) {
  ...
  let r = Math.max(1, Math.round(t * yU0)),
    h;
  if (e) h = Math.min(a - t, T + r);
  else h = Math.max(0, T - r);
  if (h === T) return null;
  return {
    start: h,
    end: h + t
...
function IU0(T) {
  let R = 0;
  for (let a of T)
    for (let e of a.points)
      if (e.value > R) R = e.value;
  return R || 1
}
...
function $U0(T, R, a, e, t, r, h) {
  ...
  if (e.length <= 1) {
    if (l.push(new G(`${R.label}: `, c)), R.link) l.push(n(R.link, r(R.value)));
    else l.push(new G(r(R.value), s));
...
`, c));
    ...
    for (let m = 0; m < e.length; m++) {
      ...
      if (l.push(new G("\u25CF ", new cT({
          color: u
        }))), l.push(new G(`${b.name}: `, A)), y.link) l.push(n(y.link, r(y.value)));
      else l.push(new G(r(y.value), s));
...
class et {
  ...
  footerStyle;
  dimensions;
  constructor(T, R = "Info", a = "info", e = "default", t) {
    this.widget = T, this.title = R, this.type = a, this.footerStyle = e, this.dimensions = t
  }
}
...
function ThT(T, R = "#") {
  let {
    thread: a,
    threadViewState: e
  } = T, t = (h) => `${R} ${h}`, r = [t("Thread"), `- ID: \`${a.id}\``, `- Title: ${BET(a)}`, `- URL: \`${$P(new URL(T.ampURL),a.id).toString()}\``, `- Created: ${BQ(a.created)}`, `- Agent mode: ${Al(a.agentMode)}`, `- Effective mode: \`${T.effectiveAgentMode}\``, `- DTW backed: ${Tm(a)?"yes":"no"}`, `- Executor type: ${Al(a.meta?.executorType)}`, `- Messages: \`${a.messages.length}\` total / \`${ve(a)}\` human / \`${a.queuedMessages?.length??0}\` queued`];
...
function NU0(T) {
  ...
  if (Tm(T.thread)) R.push(`- Cloudflare Logs: <${JrT(T.thread.id)}>`, `- Cloudflare Data Studio: <${ZrT(T.thread.id)}>`), R.push("", "## DTW Commands", "```sh", wU0(T.thread.id, T.ampURL), "```");
  else R.push("", "## DTW Commands", "- This thread is not DTW-backed, so DTW commands are unavailable.");
  if (T.logFile && T.pid) {
    let a = ed(T.logFile);
