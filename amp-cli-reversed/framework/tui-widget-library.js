// Module: tui-widget-library
// Original: segment1[1726330:1793699]
// Type: Scope-hoisted
// Exports: afT, px0, bx0, W4, mx0, ux0, yx0, Px0, efT, tfT, M1T, IF, LH, rfT, kx0, xx0, fx0, Ix0, aa, gx0
// Category: widget

killBuffer = this._text.slice(a, e)
}
this.deleteText(R)
}
deleteWordRight() {
...
deleteCurrentLine() {
  ...
  let {
    line: T,
    column: R
  } = this.cursorTextPosition, a = this.positionToOffset(T, 0), e = T === this.getLineCount() - 1, t = e ? this.graphemes.length : this.positionToOffset(T + 1, 0), r = e && a > 0 ? a - 1 : a, h = e ? t : t, i = this._getStringPositionFromGraphemeIndex(r), c = this._getStringPositionFromGraphemeIndex(h);
...
_getLineStartPosition() {
  let T = this.graphemes,
    ...
    a = 0;
  for (let e = this.cursorPosition - 1; e >= 0; e--)
    if (T[e] === `
`) {
      a = e + 1;
      break
...
_getLineEndPosition() {
  let T = this.graphemes,
    R = T.length;
  for (let a = this.cursorPosition; a < T.length; a++)
    if (T[a] === `
`) {
      R = a;
      break
...
function ux0(T) {
  ...
  for (let [e, t] of T.entries())
    ...
    for (let [r, h] of t.content.entries())
      if (h.type === "manual_bash_invocation") R.push({
    ...
    if (n.type === "toolResult" && n.toolResult) {
      ...
      if (p === "slack_read") {
        ...
        let P = typeof _.type === "string" ? _.type : void 0,
          k;
        if (P === "channels") k = "searched channels";
        else if (P === "users") k = "searched users";
        else if (P === "thread") k = "read thread";
        else k = "read";
        u({
          kind: "generic",
          name: "Slack",
...
function ai(T) {
  for (let R of ["path", "filePattern", "pattern", "query", "url", "objective", "description", "prompt"])
    if (typeof T[R] === "string" && T[R].trim()) return T[R].trim();
  return
}
...
function Mx0(T) {
  let R = typeof T.file === "string" ? T.file.trim() : void 0,
    ...
    e = [];
  if (Array.isArray(T.lineRanges))
    for (let t of T.lineRanges) {
      if (!ye(t)) continue;
...
class NY {
  ...
  selectNextUserMessage() {
    ...
    for (let T of this.getUserMessageIndices())
      if (T > this._selectedMessageIndex) return this.setSelectedMessageIndex(T);
...
class Rf {
  constructor(T, R, a) {
    if (this.normal = R, this.property = T, a) this.space = a
  }
}
...
function Jx0(T, R) {
  ...
  while (t < a.length) {
    ...
    if (c) {
      ...
      else if (Array.isArray(e.className)) e.className.push(c);
      else e.className = [c];
      t += c.length
    }
    if (i) r = i[0], t++
  ...
  function t(r, h, ...i) {
    ...
    if (r === null || r === void 0) {
      ...
      let s = c.tagName.toLowerCase(),
        A = e ? e.get(s) : void 0;
      if (c.tagName = A || s, Tf0(h)) i.unshift(h);
      else
        for (let [l, o] of Object.entries(h)) Rf0(T, c.properties, l, o)
    }
...
function Tf0(T) {
  ...
  for (let e of a) {
    ...
    if (t && typeof t === "object") {
      ...
      let r = t;
      for (let h of r)
        if (typeof h !== "number" && typeof h !== "string") return !0
    }
  }
...
function Rf0(T, R, a, e) {
  ...
  if (typeof e === "number") {
    ...
    r = e
  } else if (typeof e === "boolean") r = e;
  else if (typeof e === "string")
    if (t.spaceSeparated) r = pfT(e);
    else if (t.commaSeparated) r = lfT(e);
  else if (t.commaOrSpaceSeparated) r = pfT(lfT(e).join(" "));
  else r = _fT(t, t.property, e);
  else if (Array.isArray(e)) r = [...e];
  else r = t.property === "style" ? af0(e) : String(e);
  if (Array.isArray(r)) {
    let h = [];
...
function HY(T, R) {
  if (R === null || R === void 0);
  else if (typeof R === "number" || typeof R === "string") T.push({
    type: "text",
    value: String(R)
  });
  else if (Array.isArray(R))
    for (let a of R) HY(T, a);
  else if (typeof R === "object" && "type" in R)
    if (R.type === "root") HY(T, R.children);
    else T.push(R);
...
function lf0(T, R, a) {
  ...
  if (R.type === "element") {
    ...
    if (T.verbose) {
      ...
      if (a.attrs) {
        for (h in a.attrs)
          if (hYT.call(a.attrs, h)) r[tYT(T.schema, h).property] = oP(a.attrs[h])
      }
...
class nYT {
  ...
  write(T, R) {
    if (this.html.length > 0) this.html += T;
    else this.html = T;
  ...
  _checkForProblematicCharacters(T) {
    if (sYT(T)) this._err(vR.controlCharacterInInputStream);
    else if (oYT(T)) this._err(vR.noncharacterInInputStream)
  }
  retreat(T) {
    this.pos -= T;
...
function lYT(T, R) {
  for (let a = T.attrs.length - 1; a >= 0; a--)
    if (T.attrs[a].name === R) return T.attrs[a].value;
  return null
}
...
class XtT {
  ...
  stateNumericHex(T, R) {
    ...
    while (R < T.length) {
      ...
      if (WY(e) || xf0(e)) R += 1;
      else return this.addToNumericResult(T, a, R, 16), this.emitNumericEntity(e, 3)
    }
  ...
  stateNumericDecimal(T, R) {
    ...
    while (R < T.length) {
      let e = T.charCodeAt(R);
      if (WY(e)) R += 1;
      else return this.addToNumericResult(T, a, R, 10), this.emitNumericEntity(e, 2)
    }
  ...
  emitNumericEntity(T, R) {
    ...
    if (T === Pe.SEMI) this.consumed += 1;
    else if (this.decodeMode === Fo.Strict) return 0;
  ...
  stateNamedEntity(T, R) {
    let {
      decodeTree: a
    } = this, e = a[this.treeIndex], t = (e & tA.VALUE_LENGTH) >> 14;
...
function Of0(T) {
  if (T === HT.NULL) return vR.nullCharacterReference;
  else if (T > 1114111) return vR.characterReferenceOutsideUnicodeRange;
  else if (cYT(T)) return vR.surrogateCharacterReference;
  else if (oYT(T)) return vR.noncharacterCharacterReference;
  else if (sYT(T) || T === HT.CARRIAGE_RETURN) return vR.controlCharacterReference;
  return null
}
class bYT {
  constructor(T, R) {
  ...
  emitCurrentTagToken() {
    ...
    if (this.prepareToken(T), T.tagID = DH(T.tagName), T.type === u8.START_TAG) this.lastStartTagName = T.tagName, this.handler.onStartTag(T);
    else {
  ...
  _flushCodePointConsumedAsCharacterReference(T) {
    if (this._isCharacterReferenceInAttribute()) this.currentAttr.value += String.fromCodePoint(T);
    else this._emitCodePoint(T)
  }
  _callState(T) {
    switch (this.state) {
      case zT.DATA: {
  ...
  _stateTagOpen(T) {
    if (Pl(T)) this._createStartTagToken(), this.state = zT.TAG_NAME, this._stateTagName(T);
    else switch (T) {
  ...
  _stateEndTagOpen(T) {
    if (Pl(T)) this._createEndTagToken(), this.state = zT.TAG_NAME, this._stateTagName(T);
    else switch (T) {
