// Module: connection-transport
// Original: segment1[137408:187883]
// Type: Scope-hoisted
// Exports: PpR, gpR, $pR, _N, vpR, jpR, zW, yL, dj, ED, OpR, h4T, E2, tcT, LpR, MpR, rcT, DpR, t9T, hcT
// Category: util

(), this.clearPendingRequests(), !T) {
  this.sendStatus({
    connected: !1,
    authenticated: !1,
    openFile: void 0,
...
handleReconnectResult(T) {
  if (this.isReconnecting = !1, !T) this.scheduleReconnect();
  else this.clearReconnectTimeout(), this.reconnectStartTime = void 0, this.sendStatus({
    connectionState: "connected",
    reconnectElapsedMs: void 0
  })
}
...
async function MpR(T, R, a) {
  try {
    let e = await LpR(T, R),
      t = ["~/.config/AGENT.md", ...a];
    for (let r of t)
      if (E2(e, r) || E2(T, r)) return null;
    for (let {
        key: r,
        pattern: h
      }
      of a9T)
      if (tcT(e, h.patterns) || tcT(T, h.patterns)) return {
        key: r,
        pattern: h
      };
    return null
...
function WpR({
  ...
  switch (T) {
    case 0:
      return {
        action: "allow", matchedEntry: a
      };
    case 1:
      return {
        action: "ask", matchedEntry: a
      };
    default:
      return {
        action: "reject", matchedEntry: a, error: R
      }
  }
}
...
function Ng(T) {
  ...
  let R = this.options;
  if (R.raw && R.windowBits > 0) R.windowBits = -R.windowBits;
  else if (R.gzip && R.windowBits > 0 && R.windowBits < 16) R.windowBits += 16;
  ...
  if (R.dictionary) {
    let e;
    if (typeof R.dictionary === "string") e = Xy.string2buf(R.dictionary);
    else if (F2.call(R.dictionary) === "[object ArrayBuffer]") e = new Uint8Array(R.dictionary);
    else e = R.dictionary;
...
function Ug(T) {
  ...
  if (this.header = new nLT, ns.inflateGetHeader(this.strm, this.header), R.dictionary) {
    if (typeof R.dictionary === "string") R.dictionary = Xy.string2buf(R.dictionary);
    else if (V2.call(R.dictionary) === "[object ArrayBuffer]") R.dictionary = new Uint8Array(R.dictionary);
    if (R.raw) {
...
`) this.advance()
  ...
  readString() {
    let T = "",
      R = this.current();
    this.advance();
    while (this.current() && this.current() !== R)
      if (this.current() === "\\" && this.peek() === R) this.advance(), T += this.advance();
      else T += this.advance();
  ...
  for (let [e, t] of Object.entries(T)) {
    ...
    if (Array.isArray(t))
      for (let h = 0; h < t.length; h++) {
        let i = `${r}.${h}`;
        if (typeof t[h] === "object" && t[h] !== null) a.push(...HD(t[h], i));
        else a.push({
          path: i,
          value: t[h]
        })
      } else if (typeof t === "object" && t !== null) a.push(...HD(t, r));
      else a.push({
        path: r,
        value: t
      })
...
function xbR(T, R) {
  let a = [];
  if (typeof R === "string" || typeof R === "boolean" || typeof R === "number" || R === null) a.push(`--${T} ${Ll(R)}`);
  else if (Array.isArray(R)) {
    let e = R.every((t) => typeof t === "string" || typeof t === "boolean" || typeof t === "number" || t === null);
    if (!T.includes(".") && e)
      for (let t of R) a.push(`--${T} ${Ll(t)}`);
    else
...
function Z2(T) {
  ...
  if (R.push(Ll(T.tool)), T.matches)
    for (let [a, e] of Object.entries(T.matches)) {
      let t = xbR(a, e);
      R.push(...t)
    }
  return R.join(" ")
...
async function PLT(T, R, a, e = "thread", t, r) {
  ...
  try {
    ...
    switch (c.action) {
      case "allow":
        return {
          permitted: !0, action: s, matchedEntry: c.matchedEntry, error: c.error, source: c.source
        };
      case "reject":
        return {
          permitted: !1, action: s, matchedEntry: c.matchedEntry, reason: `Rejected by ${c.source==="built-in"?"built-in":c.source||"unknown"} permissions rule ${c.matchIndex}: ${Z2(c.matchedEntry)}`, error: c.error, source: c.source
        };
      case "ask":
        return {
          permitted: !1, action: s, matchedEntry: c.matchedEntry, reason: `Matches ${c.source==="built-in"?"built-in":c.source||"unknown"} permissions rule ${c.matchIndex}: ${Z2(c.matchedEntry)}`, error: c.error, source: c.source
        };
      case null:
        if (c.error) return {
          permitted: !1,
        ...
        return {
          permitted: !0, action: null
        };
      default:
        return {
          permitted: !1, action: null, reason: "Unknown permission result"
        }
    }
  } catch (i) {
    return {
...
class y9T {
  ...
  decodeText(T, R) {
    try {
      let a;
      if (R !== "utf8" && kLT(T)) a = T.toString(R);
      else a = new TextDecoder("utf-8", {
        fatal: !0
      }).decode(T);
...
function $mR(T) {
  ...
  function A(o, n) {
    ...
    for (let _ of p)
      if (yy(_.spec, n).enabled) return _;
    return
  }
  ...
  function l(o, n, p) {
    ...
    if (m && JSON.stringify(m.spec.source) === JSON.stringify(o.source)) R[_] = b;
    else R.push(b);
    return a.next(), {
      dispose: () => {
        let y = R.indexOf(b);
