// Module: http-request-executor
// Original: segment1[1371665:1406816]
// Type: Scope-hoisted
// Exports: uA0, IA0, gA0, $A0, vA0, jA0, SA0, OA0, pS, dA0, ZKT, JKT, TtT, EA0, TVT, RVT, E4, DkT, CA0, LA0
// Category: util
...
class $w {
  ...
  entriesById;
  constructor(T, R = [], a, e) {
    if (this.metadata = T, this.entries = R, a && e) this.entriesByPath = a, this.entriesById = e;
    else {
      let t = new Map,
        r = new Map;
  ...
  applyChanges(T) {
    ...
    for (let t of [...R, ...e])
      if (this.entriesByPath.set(t.path, t), this.entriesById.set(t.id, t), !this.entriesArrayDirty) {
...
function ZA0(T, R) {
  ...
  while (a.files.length > 0)
    if (a.files.pop(), e = sy(a), e.messageSizeBytes <= R) return e.contentBase64;
...
function tp0(T = eVT.env, R = {}) {
  ...
  if (a) {
    for (let t of Object.keys(e))
      if (Tp0.includes(t) || t.startsWith("GIT_CONFIG_KEY_") || t.startsWith("GIT_CONFIG_VALUE_")) delete e[t];
...
async function rVT(T, R = {}) {
  ...
  } = R, e = Date.now(), t = await Qo(T, ["rev-parse", "--show-toplevel"]);
