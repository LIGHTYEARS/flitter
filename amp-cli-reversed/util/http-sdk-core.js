// Module: http-sdk-core
// Original: segment1[63848:137408]
// Type: Scope-hoisted
// Exports: at, ZoR, TnR, RnR, anR, enR, tnR, hnR, eiT, inR, cnR, BdT, tiT, snR, NdT, u2, UdT, _E, Pj, Ht
// Category: util

(r === "uri" || r === "uri-reference") t = t.check(p0.url());
else if (r === "uuid" || r === "guid") t = t.check(p0.uuid());
else if (r === "date-time") t = t.check(p0.iso.datetime());
else if (r === "date") t = t.check(p0.iso.date());
else if (r === "time") t = t.check(p0.iso.time());
else if (r === "duration") t = t.check(p0.iso.duration());
else if (r === "ipv4") t = t.check(p0.ipv4());
else if (r === "ipv6") t = t.check(p0.ipv6());
else if (r === "mac") t = t.check(p0.mac());
else if (r === "cidr") t = t.check(p0.cidrv4());
else if (r === "cidr-v6") t = t.check(p0.cidrv6());
else if (r === "base64") t = t.check(p0.base64());
else if (r === "base64url") t = t.check(p0.base64url());
else if (r === "e164") t = t.check(p0.e164());
else if (r === "jwt") t = t.check(p0.jwt());
else if (r === "emoji") t = t.check(p0.emoji());
else if (r === "nanoid") t = t.check(p0.nanoid());
else if (r === "cuid") t = t.check(p0.cuid());
else if (r === "cuid2") t = t.check(p0.cuid2());
else if (r === "ulid") t = t.check(p0.ulid());
else if (r === "xid") t = t.check(p0.xid());
else if (r === "ksuid") t = t.check(p0.ksuid())
}
...
case "number": case "integer": {
  ...
  if (typeof T.exclusiveMinimum === "number") t = t.gt(T.exclusiveMinimum);
  else if (T.exclusiveMinimum === !0 && typeof T.minimum === "number") t = t.gt(T.minimum);
  if (typeof T.exclusiveMaximum === "number") t = t.lt(T.exclusiveMaximum);
  else if (T.exclusiveMaximum === !0 && typeof T.maximum === "number") t = t.lt(T.maximum);
...
case "object": {
  ...
  if (T.patternProperties) {
    ...
    if (l.push(...A), l.length === 0) e = p0.object({}).passthrough();
    else if (l.length === 1) e = l[0];
    else {
  ...
  let i = p0.object(t);
  if (T.additionalProperties === !1) e = i.strict();
  else if (typeof T.additionalProperties === "object") e = i.catchall(at(T.additionalProperties, R));
  else e = i.passthrough();
  break
}
case "array": {
  let {
...
default: throw Error(`Unsupported type: ${a}`)
}
...
function at(T, R) {
  ...
  if (T.allOf && Array.isArray(T.allOf))
    if (T.allOf.length === 0) a = e ? a : p0.any();
    else {
...
function hnR() {
  if (typeof process === "object") return "posix";
  else if (typeof navigator === "object") return navigator.userAgent.indexOf("Windows") >= 0 ? "windows" : "posix";
  return "posix"
}
...
class zR {
  ...
  constructor(T, R, a, e, t, r = !1, h) {
    if (typeof T === "object") this.scheme = T.scheme || t3, this.authority = T.authority || t3, this.path = T.path || t3, this.query = T.query || t3, this.fragment = T.fragment || t3, this.platform = T.platform;
    else this.scheme = inR(T, r), this.authority = R || t3, this.path = cnR(this.scheme, a || t3), this.query = e || t3, this.fragment = t || t3, this.platform = h, eiT(this, r)
  }
  get fsPath() {
    return NdT(this, !1)
  }
  with(T) {
    ...
    if (R === void 0) R = this.scheme;
    else if (R === null) R = t3;
    if (a === void 0) a = this.authority;
    else if (a === null) a = t3;
    if (e === void 0) e = this.path;
    else if (e === null) e = t3;
    if (t === void 0) t = this.query;
    else if (t === null) t = t3;
    if (r === void 0) r = this.fragment;
    else if (r === null) r = t3;
    if (h === void 0) h = this.platform;
    else if (h === null) h = void 0;
  ...
  static file(T, R = hnR()) {
    ...
    if (T[0] === Rc && T[1] === Rc) {
      ...
      if (e === -1) a = T.substring(2), T = Rc;
      else a = T.substring(2, e), T = T.substring(e) || Rc
    }
...
function tiT(T, R, a) {
  ...
  for (let t = 0; t < T.length; t++) {
    ...
    if (r >= 97 && r <= 122 || r >= 65 && r <= 90 || r >= 48 && r <= 57 || r === 45 || r === 46 || r === 95 || r === 126 || R && r === 47 || a && r === 91 || a && r === 93 || a && r === 58) {
      ...
      let h = b0T[r];
      if (h !== void 0) e += h;
      else e += encodeURIComponent(T.charAt(t))
    }
  }
  return e !== void 0 ? e : T
}
...
function NdT(T, R) {
  let a;
  if (T.authority && T.path.length > 1 && T.scheme === "file") a = `//${T.authority}${T.path}`;
    ...
    if (!R) a = T.path[1].toLowerCase() + T.path.substring(2);
    else a = T.path.substring(1);
  else a = T.path;
...
function u2(T, R) {
  ...
  if (r) {
    ...
    if (s !== -1) {
      ...
      if (r = r.substring(s + 1), s = A.lastIndexOf(":"), s === -1) e += a(A, !1, !1);
      else e += a(A.substring(0, s), !1, !1), e += ":", e += a(A.substring(s + 1), !1, !0);
      e += "@"
    }
    if (r = r.toLowerCase(), s = r.lastIndexOf(":"), s === -1) e += a(r, !1, !0);
    else e += a(r.substring(0, s), !1, !0), e += r.substring(s)
  }
  if (h) {
...
function UdT(T) {
  try {
    ...
    if (T.length > 3) return T.substring(0, 3) + UdT(T.substring(3));
    else return T
  }
}
...
function nnR(T, R, a = "/", e = "/", t = !0) {
  ...
  if (a === "/") T = T.replaceAll(/\/{2,}/g, "/"), R = R.replaceAll(/\/{2,}/g, "/");
  else T = T.replaceAll(/\\{2,}/g, "\\"), R = R.replaceAll(/\\{2,}/g, "\\");
...
function dnR(T) {
  ...
  for (let a of R)
    if (JdT(T, a.type, a.value), T._state === "closed") break
}
...
function JdT(T, R, a) {
  ...
  try {
    switch (R) {
      case "next":
        ...
        break;
      case "error":
        if (x2(T), e && typeof e.error === "function") e.error(a);
        else throw a;
        break;
      case "complete":
  ...
  if (T._state === "closed") ZdT(T);
  else if (T._state === "running") T._state = "ready"
}
...
function U3(T) {
  ...
  else if (T && typeof T.unsubscribe === "function") T.unsubscribe()
}
class FS {
  _baseObserver;
  _pendingPromises;
...
async function eN(T, R) {
  return new Promise((a, e) => {
    ...
    R?.addEventListener("abort", i, {
      ...
      complete: () => {
        if (R?.removeEventListener("abort", i), r) a(t);
        else e("Observable completed without emitting a value")
      }
    })
  })
}
...
function f3(T) {
  ...
  return (t) => {
    let r = new W0,
      ...
      c, s = 0;
    return new AR((A) => {
      if (s++, i) e("new subscriber, emitting buffered value", c), A.next(c);
      else e("new subscriber, no buffered value to emit");
      if (!h) h = t.subscribe({
        next: (o) => {
          i = !0, c = o, r.next(o)
        },
  ...
  return (e) => new AR((t) => {
    let r = 0,
      ...
      s = e.subscribe({
        next: (A) => {
          let l = a.now();
          if (i = A, c = !0, l - r >= T) t.next(A), r = l, c = !1;
          else if (h === null) h = a.schedule(() => {
...
function KnR(T, R, a = !0) {
  ...
  let {
    ...
    isWindows: t,
    homeDir: r
  } = R, h = typeof T === "string" ? zR.parse(T) : zR.from(T);
  if (e)
    for (let i of e) {
      let c = I8(i);
      if (MR.hasPrefix(h, c)) {
...
function JnR(T, R, a) {
  let e;
  if (T) e = uiT(T.random ?? T.rng?.() ?? miT(), T.msecs, T.seq, R, a);
  else {
    let t = Date.now(),
      r = miT();
...
function TlR(T, R, a) {
  if (T.msecs ??= -1 / 0, T.seq ??= 0, R > T.msecs) T.seq = a[6] << 23 | a[7] << 16 | a[8] << 8 | a[9], T.msecs = R;
  else if (T.seq = T.seq + 1 | 0, T.seq === 0) T.msecs++;
  return T
}

function uiT(T, R, a, e, t = 0) {
  if (T.length < 16) throw Error("Random bytes length must be >= 16");
  if (!e) e = new Uint8Array(16), t = 0;
  else if (t < 0 || t + 16 > e.length) throw RangeError(`UUID byte range ${t}:${t+15} is out of buffer bounds`);
...
function uy(T) {
  ...
  switch (R.type) {
    ...
    case "string": {
      let a = T[T.length - 2];
      if (a?.type === "delimiter") return uy(T.slice(0, T.length - 1));
      else if (a?.type === "brace" && a.value === "{") return uy(T.slice(0, T.length - 1));
      break
    }
    case "delimiter":
      return uy(T.slice(0, T.length - 1))
...
function clR(T, R) {
  ...
  while (e < T.length) {
    let t = T[e];
    if (g0T(t)) break;
    if (t.startsWith("+")) a.push(t.slice(1));
    else if (t === "") a.push("");
    else throw Error(`Invalid patch format: Add File lines must start with '+', got: "${t.slice(0,20)}${t.length>20?"...":""}"`);
    e++
  }
  return {
    content: a.join(`
...
function olR(T, R) {
  ...
  while (a < T.length) {
    ...
    if (c.startsWith("-")) e.push(c.slice(1));
    else if (c.startsWith("+")) t.push(c.slice(1));
    else if (c.startsWith(" ")) e.push(c.slice(1)), t.push(c.slice(1));
    a++
  }
...
*** End Patch`);
  ...
  while (l < i) {
    ...
    else if (n.startsWith("*** Update File:")) {
      let {
        chunks: p,
        nextIdx: _
      } = slR(R, o.nextIdx);
...
function Tn(T, R) {
  for (let a = T.messages.length - 1; a >= 0; a--) {
    ...
    for (let t of e.content)
      if (t.type === "tool_use" && t.id === R) return t
  }
  return
}
...
function gj(T, R) {
  for (let [a, e] of T.messages.entries()) {
    if (e.role !== "assistant") continue;
    for (let [t, r] of e.content.entries())
      if (r.type === "tool_use" && r.id === R) return {
        message: e,
        messageIndex: a,
        block: r,
        blockIndex: t
...
function cN(T, R) {
  for (let a of T.messages) {
    ...
    for (let e of a.content)
      if (e.type === "tool_result" && e.toolUseID === R) return e
  }
  return
}
...
function O0T(T) {
  for (let R = T.messages.length - 1; R >= 0; R--) {
    ...
    for (let e of a.content)
      if (e.type === "tool_use" && e.name === llR && e.input) {
        let t = e.input.todos;
...
function sN() {
  ...
  let R;
  if (!Dg) R = "CLI";
  else if (["Neovim", "JetBrains", "Zed"].includes(Dg)) R = Dg;
  else R = `${Dg} CLI`;
  return {
    type: "cli",
    ...
    switch (T) {
      case "darwin": {
        try {
          let {
            execSync: R
          } = qT("node:child_process"), a = R("/usr/bin/sw_vers -productVersion", {
            encoding: "utf8",
            timeout: 3000,
...
`);
      ...
      case "windows":
        try {
          let {
            execSync: R
          } = qT("node:child_process"), a = R('systeminfo | findstr /B /C:"OS Name" /C:"OS Version"', {
            encoding: "utf8",
            timeout: 5000
          }).split(`
`), e = a.find((r) => r.includes("OS Name")), t = a.find((r) => r.includes("OS Version"));
          if (e && t) {
...
function JS() {
  if (!NW) {
    let T = typeof process < "u" && process.versions?.node,
      ...
      e = "unknown",
      t = "unknown";
    if (T) a = "darwin", e = TAR(a), t = "arm64";
    else if (typeof navigator < "u") {
      let r = navigator.platform.toLowerCase();
      if (r.includes("mac") || r.includes("ios")) a = "darwin";
      else if (r.includes("win")) a = "windows";
      ...
      if (h.includes("x86_64") || h.includes("WOW64")) t = "x64";
      else if (h.includes("i686")) t = "x86";
      else if (h.includes("ARM64") || h.includes("arm64")) t = "arm64";
      else if (h.includes("ARM")) t = "arm"
    }
    NW = {
      webBrowser: R,
      os: a,
...
function dn(T) {
  ...
  for (let e of Object.values(n8))
    if (e.provider === R && e.name === a) return e;
  return oN(R)
}

function E0T(T) {
  for (let a of Object.values(n8))
...
async function ICT(T) {
  return (await w0T(T, `select workspace_id, paths, timestamp from workspaces order by timestamp desc limit ${JAR}`)).map((R) => {
    let [a, e = "", t = ""] = R.split(q0T), r = Number.parseInt(a ?? "", 10);
...
async function GAR(T, R) {
  ...
  if (!a) return null;
  let [e, t = "", r, h] = a.split(q0T), i = Number.parseInt(e ?? "", 10);
...
function XAR() {
  ...
  if (!R) return null;
  for (let a of z0T)
    if (a !== "stable" && R.includes(a)) return a;
  return "stable"
}
...
function ECT(T) {
  if (epR(T)) return "Neovim";
  else if (tpR(T)) return "Zed";
  else if (G0T(T)) return "JetBrains";
  else if (AN(T)) return "VS Code"
}
...
function cpR(T, R, a) {
  let e = T.workspaceFolders.map(Oj),
    ...
    r = e.includes(a),
    h = t.includes(a);
  if (r && h) return T.ideName.localeCompare(R.ideName);
  else if (r) return -1;
  else if (h) return 1;
  let i = d2(e, a),
    c = d2(t, a),
    s = e[0]?.length ?? 0,
    A = t[0]?.length ?? 0;
  if (i && c) return A - s || T.ideName.localeCompare(R.ideName);
  else if (i) return -1;
  else if (c) return 1;
...
function NCT(T) {
  try {
    ...
    else J.debug("Invalid IDE config file", {
      file: T,
...
function Oj(T) {
  let R;
  if (T.startsWith("file://")) R = zR.parse(T);
  else T = T.replaceAll("\\", "/"), R = zR.file(T);
  try {
...
class ZCT {
  ...
  handleResponse(T) {
    ...
    let a = T[R.method];
    if (a) R.resolve(a);
    else R.reject(Error(`Invalid response for method ${R.method}. Got ${JSON.stringify(T,null,2)}`))
  }
  async tryConnect(T) {
      ...
      if (this.clearReconnectTimeout(), this.ws?.close