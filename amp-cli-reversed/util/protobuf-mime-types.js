// Module: protobuf-mime-types
// Original: segment1[1006471:1141570]
// Type: Scope-hoisted
// Exports: GqR, ezT, tzT, XqR, IuT, rzT, hzT, OaT, QqR, guT, JqR, T5R, vuT, R5R, a5R, e5R, c5R, SuT, iw, s5R
// Category: util
...
async delete(T) {
  ...
  for (let a of this.threadEntriesByID.values())
    if (a.mainThreadID === T) R.add(a.id);
          ...
          while (!0) {
            ...
            let b = performance.now(),
              y = 0,
              {
                ...
                if (m.message = S.message, m.reasoning = S.reasoning, m.isThinking = S.isThinking, S.toolCalls)
                  for (let j of S.toolCalls) {
                    let d = {};
                    try {
...
${m}` : _;
  ...
  async runTool(T, R, a, e, t, r) {
    ...
    return await eN(i, t).then((c) => {
      ...
      if (c.status === "done") this.setToolUseStatus(h, "done", c.result, r);
      else if (c.status === "error") this.setToolUseStatus(h, "error", c.error, r);
      else this.setToolUseStatus(h, c.status, void 0, r);
      return {
        id: h,
        result: c
      }
...
function s5R(T) {
  ...
  for (let {
    ...
    of T)
    if (e.status === "error") R.push(String(e.error));
...
`).map((r) => r.trim()).filter((r) => r);
  ...
  for (let r of R) {
    ...
    if (!i || !c) continue;
    if (i === "name" && !a) a = c;
    else if (i === "description") e.push(c);
    else {
      let s = c.indexOf(" ");
      if (s === -1) t[i] = {
        type: "string",
...
function j5R(T) {
  ...
  function a(e, t) {
    if (t === null || t === void 0) R.push(`${e}: ${String(t)}`);
    else if (typeof t === "boolean") R.push(`${e}: ${t?"true":"false"}`);
    else if (typeof t === "number" || typeof t === "string") R.push(`${e}: ${String(t)}`);
    else if (Array.isArray(t))
      for (let r = 0; r < t.length; r++) a(`${e}_${r}`, t[r]);
    else if (typeof t === "object")
      for (let [r, h] of Object.entries(t)) a(`${e}_${r}`, h);
    else R.push(`${e}: ${String(t)}`)
  }
...
function S5R({
  ...
  async function s(A, l, o) {
    ...
    for (let y of A) {
      ...
      try {
        ...
        if (await d5R(u, R, a, (x, f) => {
            ...
            if (k.push(S), P >= 0 && n[P]) {
              let O = n[P].tools.findIndex((j) => j.name === x && j.status === "pending");
              if (O >= 0) n[P].tools[O] = S;
              else n[P].tools.push(S);
              c.next([...n])
            }
          }), P >= 0 && n[P]) n[P] = {
          ...n[P],
    ...
    for (let [y, u] of o)
      if (!p.has(y)) u.dispose(), o.delete(y);
    return n
  }
  return {
    name: "Toolbox",
...
function E5R(T) {
  ...
  if (Array.isArray(T)) switch (T.length) {
    case 0:
      return {
        type: "string", description: "generic argument - use a sensible value"
      };
    case 1:
      return {
        type: "string", description: T[0]
      };
    default:
      return {
...
`),
  ...
  while (r < a.length || h < e.length) {
    let i = a[r],
      c = e[h];
    if (i === c) t.push(` ${i??""}`), r++, h++;
    else if (i !== void 0 && (c === void 0 || i !== e[h])) t.push(`-${i}`), r++;
    else t.push(`+${c??""}`), h++
  }
  return t.join(`
`)
...
class lzT {
  ...
  deleteByPrefix(T) {
    for (let R of this.map.keys())
      if (typeof R === "string" && R.startsWith(T)) this.map.delete(R)
  }
  get size() {
...
function fzT() {
  try {
    ...
    if (T?.code === "ENOENT") J.warn("os.userInfo() failed \u2013 user probably provided by NSS/FreeIPA");
    else J.warn("os.userInfo() failed", {
      err: T
    });
    return
  }
    ...
    return (async () => {
      try {
        ...
        let D = c ? {
          ...
          M = typeof B === "string" && /\/(bash|zsh|ksh)$/.test(B) ? `set -o pipefail;
 ${a.cmd}` : a.cmd;
        y = i4T(M, [], {
          shell: B,
          cwd: T,
  ...
  let R = [],
    ...
    e = (t) => {
      ...
      for (let r of t.values)
        if (typeof r === "object" && r !== null && r.type === "invocations") a.push(...r.trees)
    };
  while (a.length > 0) {
    let t = a.shift();
    if (!t) break;
    ...
    updateConversation: (R, a, e) => {
      ...
      if (R.contents.push({
        ...
        let r = e.map(({
          ...
          let [, c] = h.split("__"), s = {};
          if (i.status === "done") s.output = i.result;
          else if (i.status === "error") s.error = i.error?.message ?? "Error executing tool";
          else s.error = `Tool status: ${i.status}`;
          return {
            functionResponse: {
              name: c ?? "",
              response: s
...
async function NuT(T, R, a, e) {
  try {
    ...
    for (let r of t)
      if (!R.has(r.name)) R.set(r.name, r)
  } catch {}
}
...
async function uFR(T, R, a, e, t = null) {
  ...
  for (let s of i.keys()) {
    ...
    c.set(s, l);
    for (let o of l)
      if (!r.has(o.name)) r.set(o.name, o)
  }
...
function DFR(T) {
  let R = [];
  for (let a of T ?? [])
    for (let e of a.activeTools.values()) R.push(e);
  return R
}
...
function wFR(T) {
  ...
  switch (R.status) {
    ...
    case "done":
      return {
        status: R.status === "done" && r ? "done" : "in-progress", result: i, progress: c, "~debug": {
          mainAgent: R["~debug"],
    ...
    case "cancelled":
      return {
        status: "cancelled", reason: "Code review was cancelled"
      }
  }
}
...
function x2R(T, R) {
  switch (T.status) {
    case "in-progress":
      return {
        status: "in-progress", result: R, progress: T.turns.map((a) => ({
          message: a.message,
    ...
    case "done":
      return {
        status: "done", result: R, progress: T.turns.map((a) => ({
          message: a.message,
          tool_uses: [...a.activeTools.values()]
        })), "~debug": T["~debug"]
      };
    ...
    case "cancelled":
      return {
        status: "cancelled", reason: "Code tour was cancelled"
      }
  }
}
...
function C2R(T, R, a) {
  ...
  if (e.push("--regexp", R.pattern), R.path) c4T(R.path), e.push(An(R.path) ? R.path : MR.joinPath(T, R.path).fsPath);
  else e.push(T.fsPath);
...
function HzT(T) {
  switch (T.status) {
    case "in-progress":
      return {
        status: "in-progress", progress: T.turns.map((R) => ({
          message: R.message,
    ...
    case "done":
      return {
        status: "done", result: T.message, progress: T.turns.map((R) => ({
          message: R.message,
          tool_uses: [...R.activeTools.values()]
        })), "~debug": T["~debug"]
      };
    ...
    case "cancelled":
      return {
        status: "cancelled", reason: "Codebase search was cancelled"
      }
  }
}

function WzT(T) {
  let R = T.dir ? Kt(T.dir) : null,
    a = T.dir ? Kt(T.dir) : null;
  return D2R(a, R)
}

function D2R(T, R) {
  return `You are a fast, parallel code search agent.
...
async function cn(T, R, a = {}, e) {
  let {
    ...
    method: r = "GET",
    ...h
  } = a, i = T.trim().replace(/\/$/, ""), c = R.startsWith("/") ? R : `/${R}`, s = `${i}${c}`, A = cGR(T, e);
  if (!A) return {
    ok: !1,
    status: 401,
...
async function yGR(T, R, a, e, t, r, h, i, c) {
  ...
  while (!0) {
    ...
    let P = [],
      k = !0,
      x = l + o;
    if (Array.isArray(u.data)) P = u.data;
    else {
      let f = u.data;
      if (f.values) {
        for (let v of f.values)
          if (v.type === "FILE") P.push(v.path.toString)
      }
...
async function $GR(T, R, a, e, t, r, h) {
  ...
  let s = c.join("&"),
    A;
  if (R) A = `/rest/api/1.0/projects/${encodeURIComponent(R)}/repos?${s}`;
  else A = `/rest/api/1.0/repos?${s}`;
  let l = await cn(T, A, {
    signal: h
  }, i);
...
async function HGR(T) {
  let {
    ...
    args: a,
    signal: e
  } = T, t = Kx(a.repository), r = a.path.replace(/^\//, ""), h = r || "/", i = r.split("/").map(encodeURIComponent).join("/"), c = await R.fetchJSON(`repos/${t}/contents/${i}`, {
    signal: e
  });
...
async function WGR(T) {
  ...
  for (let m of p.items) {
    ...
    let b = _.get(m.path);
    for (let y of m.text_matches)
      if (y.property === "content" && y.fragment) {
...
async function qGR(T) {
  let {
    ...
    limit: t = 100
  } = a, r = Kx(a.repository), h = (a.path ?? "").replace(/^\//, "");
  if (h === "." || h === "") h = "";
...
async function zGR(T) {
  let {
    ...
    limit: r = 100,
    offset: h = 0
  } = a, i = `repos/${Kx(a.repository)}/git/trees/HEAD?recursive=1`, c = await R.fetchJSON(i, {
    signal: e
  });
  if (!c.ok || !c.data) return {
    ok: !1,
...
async function GGR(T) {
  let {
    ...
    head: r,
    includePatches: h = !1
  } = a, i = `repos/${Kx(a.repository)}/compare/${encodeURIComponent(t)}...${encodeURIComponent(r)}`, c = await R.fetchJSON(i, {
    signal: e
  });
  if (!c.ok || !c.data) return {
    ok: !1,
...
function GKR(T) {
  ...
  while (a < T.length) {
    let e = T[a++];
    if (e < 128) R += String.fromCharCode(e);
    else if (e < 224) {
      let t = T[a++] & 63;
...
class NaT {
  ...
  async findEndOfCentralDirectoryLocator() {
    ...
    for (let e = a.length - 4; e >= 0; e--)
      if (a[e] === t4[0] && a[e + 1] === t4[1] && a[e + 2] === t4[2] && a[e + 3] === t4[3]) return T.fileInfo.size - R + e;
    return -1
  }
  ...
  async unzip(T) {
    ...
    do {
      ...
      } else if (t.handler) Qc(`Reading compressed-file-data: ${e.compressedSize} bytes`), r = new Uint8Array(e.compressedSize), await this.tokenizer.readBuffer(r), await this.inflate(e, r, t.handler);
      else Qc(`Ignoring compressed-file-data: ${e.compressedSize} bytes`), await this.tokenizer.ignore(e.compressedSize);
...
function pz(T) {
  switch (T = T.toLowerCase(), T) {
    ...
    case "application/vnd.ms-visio.drawing":
      return {
        ext: "vsdx", mime: "application/vnd.visio"
      };
