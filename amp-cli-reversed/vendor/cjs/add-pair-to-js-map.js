...
(T) => {
  var R = SDT(),
    ...
    t = x8(),
    r = ym();
  function h(c, s, { key: A, value: l }) {
    if (t.isNode(A) && A.addToJSMap) A.addToJSMap(c, s, l);
  ...
  function i(c, s, A) {
    ...
    if (t.isNode(c) && A?.doc) {
      let l = e.createStringifyContext(A.doc, {});
      l.anchors = new Set();
      ...
      if (!A.mapKeyWarned) {
        ...
        (R.warn(
          ...
          `Keys with collection values will be stringified due to JS Object restrictions: ${n}. Set mapAsMap: true to use object keys.`,
        ),
          (A.mapKeyWarned = !0));
      }
      return o;
