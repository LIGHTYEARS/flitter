...
// Module: YD (CJS)
(T)=>{Object.defineProperty(T,"__esModule",{value:!0}),T.reportTypeError=T.checkDataTypes=T.checkDataType=T.coerceAndCheckDataType=T.getJSONTypes=T.getSchemaTypes=T.DataType=void 0;
var R=kMT(),a=xMT(),e=vN(),t=M9(),r=a8(),h;
(function(u){u[u.Correct=0]="Correct",u[u.Wrong=1]="Wrong"})(h||(T.DataType=h={}));
function i(u){let P=c(u.type);
if(P.includes("null")){if(u.nullable===!1)throw Error("type: null contradicts nullable: false")}else{if(!P.length&&u.nullable!==void 0)throw Error('"nullable" cannot be used without "type"');
if(u.nullable===!0)P.push("null")}return P}T.getSchemaTypes=i;
function c(u){let P=Array.isArray(u)?u:u?[u]:[];
if(P.every(R.isJSONType))return P;
throw Error("type must be JSONType or JSONType[]: "+P.join(","))}T.getJSONTypes=c;
function s(u,P){let{gen:k,data:x,opts:f}=u,v=l(P,f.coerceTypes),g=P.length>0&&!(v.length===0&&P.length===1&&(0,a.schemaHasRulesForType)(u,P[0]));
if(g){let I=_(P,x,f.strictNumbers,h.Wrong);
k.if(I,()=>{if(v.length)o(u,P,v);
else b(u)})}return g}T.coerceAndCheckDataType=s;
var A=new Set(["string","number","integer","boolean","null"]);
function l(u,P){return P?u.filter((k)=>A.has(k)||P==="array"&&k==="array"):[]}function o(u,P,k){let{gen:x,data:f,opts:v}=u,g=x.let("dataType",t._`typeof ${f}`),I=x.let("coerced",t._`undefined`);if(v.coerceTypes==="array")x.if(t._`${g} == 'object' && Array.isArray(${f}) && ${f}.length == 1`,()=>x.assign(f,t._`${f}[0]`).assign(g,t._`typeof ${f}`).if(_(P,f,v.strictNumbers),()=>x.assign(I,f)));x.if(t._`${I} !== undefined`);for(let O of k)if(A.has(O)||O==="array"&&v.coerceTypes==="array")S(O);x.else(),b(u),x.endIf(),x.if(t._`${I} !== undefined`,()=>{x.assign(f,I),n(u,I)});function S(O){switch(O){case"string":x.elseIf(t._`${g} == "number" || ${g} == "boolean"`).assign(I,t._`"" + ${f}`).elseIf(t._`${f} === null`).assign(I,t._`""`);return;case"number":x.elseIf(t._`${g} == "boolean" || ${f} === null
