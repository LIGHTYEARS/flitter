...
(T) => {
  (Object.defineProperty(T, "__esModule", { value: !0 }),
    (T.minDeadline = R),
    (T.getDeadlineTimeoutString = e),
    (T.getRelativeTimeout = r),
    (T.deadlineToString = h),
    (T.formatDateDifference = i));
  function R(...c) {
    let s = 1 / 0;
    for (let A of c) {
      let l = A instanceof Date ? A.getTime() : A;
      if (l < s) s = l;
    }
    return s;
  }
  ...
  function e(c) {
    ...
    for (let [l, o] of a) {
      let n = A / o;
      if (n < 1e8) return String(Math.ceil(n)) + l;
    }
    throw Error("Deadline is too far in the future");
  }
  var t = 2147483647;
  function r(c) {
    let s = c instanceof Date ? c.getTime() : c,
      A = new Date().getTime(),
      l = s - A;
    if (l < 0) return 0;
  ...
  function h(c) {
    if (c instanceof Date) return c.toISOString();
      ...
      else return s.toISOString();
    }
  }
  function i(c, s) {
    return ((s.getTime() - c.getTime()) / 1000).toFixed(3) + "s";
  }
};
