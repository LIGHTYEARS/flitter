...
(T) => {
  ...
  function a(i, c) {
    let s = i[0],
      A = s === "-" || s === "+" ? i.substring(1) : i,
      l = (n) => (c ? BigInt(n) : Number(n)),
      o = A.replace(/_/g, "")
        .split(":")
        .reduce((n, p) => n * l(60) + l(p), l(0));
    return s === "-" ? l(-1) * o : o;
  }
  function e(i) {
    let { value: c } = i,
    ...
    let l = s(60),
      o = [c % l];
    if (c < 60) o.unshift(0);
    else if (((c = (c - o[0]) / l), o.unshift(c % l), c >= 60))
      ((c = (c - o[0]) / l), o.unshift(c));
    return (
      A +
      o
  ...
  var t = {
      ...
      resolve: (i, c, { intAsBigInt: s }) => a(i, s),
      stringify: e,
    },
    r = {
    ...
    h = {
      ...
      resolve(i) {
        let c = i.match(h.test);
        if (!c)
          throw Error("!!timestamp expects a date, starting with yyyy-mm-dd");
        let [, s, A, l, o, n, p] = c.map(Number),
          _ = c[7] ? Number((c[7] + "00").substr(1, 3)) : 0,
          m = Date.UTC(s, A - 1, l, o || 0, n || 0, p || 0, _),
          b = c[8];
        if (b && b !== "Z") {
          let y = a(b, !1);
