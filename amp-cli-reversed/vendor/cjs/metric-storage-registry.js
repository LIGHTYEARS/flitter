...
(T) => {
  ...
  class t {
    ...
    register(r) {
      this._registerStorage(r, this._sharedRegistry);
    }
    registerForCollector(r, h) {
      let i = this._perCollectorRegistry.get(r);
      if (i == null) ((i = new Map()), this._perCollectorRegistry.set(r, i));
      this._registerStorage(h, i);
    }
    findOrUpdateCompatibleStorage(r) {
      let h = this._sharedRegistry.get(r.name);
      if (h === void 0) return null;
      return this._findOrUpdateCompatibleStorage(r, h);
    }
    findOrUpdateCompatibleCollectorStorage(r, h) {
      let i = this._perCollectorRegistry.get(r);
      if (i === void 0) return null;
      let c = i.get(h.name);
      if (c === void 0) return null;
      return this._findOrUpdateCompatibleStorage(h, c);
    }
    _registerStorage(r, h) {
      let i = r.getInstrumentDescriptor(),
        c = h.get(i.name);
      if (c === void 0) {
        h.set(i.name, [r]);
        return;
    ...
    _findOrUpdateCompatibleStorage(r, h) {
      let i = null;
      for (let c of h) {
        let s = c.getInstrumentDescriptor();
        if ((0, R.isDescriptorCompatibleWith)(s, r)) {
          if (s.description !== r.description) {
            if (r.description.length > s.description.length)
              c.updateDescription(r.description);
            a.diag.warn(
...
`,
              `Details:
`,
              (0, e.getIncompatibilityDetails)(s, r),
              `The longer description will be used.
To resolve the conflict:`,
              (0, e.getConflictResolutionRecipe)(s, r),
            );
          }
          i = c;
        } else
