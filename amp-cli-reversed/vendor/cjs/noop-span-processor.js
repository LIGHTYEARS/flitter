// Module: noop-span-processor
// Original: YtR
// Type: CJS (RT wrapper)
// Exports: NoopSpanProcessor
// Category: util
...
(T) => {
  ...
  var R = n0(),
    ...
    e = z$T(),
    t = Fs();
  class r extends e.MetricReader {
    _interval;
    _exporter;
    _exportInterval;
    _exportTimeout;
    constructor(h) {
      super({
        ...
        aggregationTemporalitySelector:
          h.exporter.selectAggregationTemporality?.bind(h.exporter),
    ...
    async _runOnce() {
      try {
        await (0, t.callWithTimeout)(this._doRun(), this._exportTimeout);
      } catch (h) {
        if (h instanceof t.TimeoutError) {
          R.diag.error(
        ...
        (0, a.globalErrorHandler)(h);
      }
    }
    async _doRun() {
      let { resourceMetrics: h, errors: i } = await this.collect({
      ...
      if (h.resource.asyncAttributesPending)
        try {
          ...
          (R.diag.debug("Error while resolving async portion of resource: ", s),
            (0, a.globalErrorHandler)(s));
        }
      if (h.scopeMetrics.length === 0) return;
      let c = await a.internal._export(this._exporter, h);
      if (c.code !== a.ExportResultCode.SUCCESS)
        throw Error(
          `PeriodicExportingMetricReader: metrics export failed (error ${c.error})`,
        );
    }
    onInitialized() {
      if (
  ...
  T.PeriodicExportingMetricReader = r;
};
