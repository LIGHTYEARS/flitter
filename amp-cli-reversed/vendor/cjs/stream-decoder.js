    ...
    write(e) {
      ...
      while (t < e.length)
        switch (this.readState) {
          ...
          case R.READING_SIZE:
            if (
              ((r = Math.min(e.length - t, this.readSizeRemaining)),
              e.copy(
              ...
              if (
                ...
                this.readState = R.READING_MESSAGE;
              else {
                let i = Buffer.concat(
                  [this.readCompressFlag, this.readPartialSize],
                  5,
                );
                ((this.readState = R.NO_DATA), h.push(i));
              }
            }
            break;
          case R.READING_MESSAGE:
            if (
              ((r = Math.min(e.length - t, this.readMessageRemaining)),
              this.readPartialMessage.push(e.slice(t, t + r)),
              (this.readMessageRemaining -= r),
              (t += r),
              this.readMessageRemaining === 0)
            ) {
              let i = [this.readCompressFlag, this.readPartialSize].concat(
                  this.readPartialMessage,
                ),
                c = Buffer.concat(i, this.readMessageSize + 5);
              ((this.readState = R.NO_DATA), h.push(c));
            }
            break;
          default:
