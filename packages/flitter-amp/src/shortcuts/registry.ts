...
export interface ShortcutContext {
  ...
  /**
   * Widget-level action hooks. These are provided by app.ts to handle
...
export interface ShortcutHooks {
  /** Show the command palette overlay. App.ts provides the builder. */
  showCommandPalette(): void;
  /** Show the shortcut help overlay. App.ts provides the builder. */
  showShortcutHelp(): void;
  /** Open current input text in $EDITOR (async TUI suspend) */
  openInEditor(): void;
  /** Navigate prompt history backward, returns true if navigation occurred */
