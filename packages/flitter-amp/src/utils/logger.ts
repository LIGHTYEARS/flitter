...
import { existsSync, mkdirSync, createWriteStream, readdirSync, unlinkSync, type WriteStream } from 'node:fs';
 ...
 */
export function initLogFile(retentionDays: number = 7): void {
  try {
 ...
 * Only targets files matching the `amp-YYYY-MM-DD.log` naming convention.
 */
export function pruneOldLogs(logDir: string, retentionDays: number): void {
  ...
  try {
    ...
    const logPattern = /^amp-(\d{4}-\d{2}-\d{2})\.log$/;
    ...
    // Non-fatal: log cleanup failure shouldn't prevent app startup
  }
}
