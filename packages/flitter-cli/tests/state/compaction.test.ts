...
// =============================================================================
// Actual compaction truncation (S1-5)
// =============================================================================
describe('Actual compaction truncation (S1-5)', () => {
  // -------------------------------------------------------------------------
  // 21. Compaction actually removes items from session
  // -------------------------------------------------------------------------
  test('compaction actually removes items from session when threshold exceeded', async () => {
    const session = makeSession();

    // Build up 6 user-assistant turns
    for (let i = 0; i < 6; i++) {
      session.startProcessing(`user msg ${i}`);
      session.beginStreaming();
      session.appendAssistantChunk(`assistant reply ${i}`);
    ...
    // 6 turns * 2 items (user + assistant) = 12 items
    ...
    // After compaction + the new prompt, items should be fewer than before + 2 (user + assistant)
    // The compaction removed old turns, so the total should be less
    const afterItems = session.items;
    // beforeCount was 12, we added 2 more (user + assistant) = 14 before compaction
    // Compaction should have removed some, so we should have fewer than 14
    expect(afterItems.length).toBeLessThan(14);
  ...
  // -------------------------------------------------------------------------
  // 22. Compaction preserves system messages from removed region
  // -------------------------------------------------------------------------
  test('compaction preserves system messages from removed region', async () => {
    const session = makeSession();
    ...
    const beforeItems = session.items;
    const beforeSystemMsgs = beforeItems.filter(i => i.type === 'system_message');
    expect(beforeSystemMsgs.length).toBe(1);
    ...
    // System message should be preserved even after compaction
    const afterItems = session.items;
    const afterSystemMsgs = afterItems.filter(i => i.type === 'system_message');
    expect(afterSystemMsgs.length).toBe(1);
    expect(afterSystemMsgs[0].type).toBe('system_message');
    expect((afterSystemMsgs[0] as any).text).toBe('Session started');
  });

  // -------------------------------------------------------------------------
  // 23. SessionState.truncateBefore() removes items correctly
  // -------------------------------------------------------------------------
  test('truncateBefore removes items before cut index', () => {
    const session = makeSession();

    // Add 4 user-assistant turns = 8 items
    for (let i = 0; i < 4; i++) {
      session.startProcessing(`msg ${i}`);
      session.beginStreaming();
      session.appendAssistantChunk(`reply ${i}`);
    ...
    expect(session.items.length).toBe(8);

    // Truncate before index 4 (keep items[4..7])
    session.truncateBefore(4);

    expect(session.items.length).toBe(4);
    // First kept item should be user_message "msg 2"
    expect(session.items[0].type).toBe('user_message');
    expect((session.items[0] as any).text).toBe('msg 2');
  });

  // -------------------------------------------------------------------------
  // 24. truncateBefore no-op for index <= 0
  // -------------------------------------------------------------------------
  test('truncateBefore is no-op for index <= 0', () => {
    const session = makeSession();
    ...
    expect(session.items.length).toBe(before);
  ...
  // -------------------------------------------------------------------------
  // 25. truncateBefore no-op for index >= items.length
  // -------------------------------------------------------------------------
  test('truncateBefore is no-op for index >= items.length', () => {
    const session = makeSession();
    ...
    const before = session.items.length;
    session.truncateBefore(100);
    expect(session.items.length).toBe(before);

    session.truncateBefore(before);
    expect(session.items.length).toBe(before);
  });

  // -------------------------------------------------------------------------
  // 26. truncateBefore preserves system messages and rebuilds tool call index
  // -------------------------------------------------------------------------
  test('truncateBefore preserves system messages and rebuilds tool call index', () => {
    const session = makeSession();

    // Add system message, then some turns
    session.addSystemMessage('System info');
    session.startProcessing('msg 0');
    session.beginStreaming();
    session.appendAssistantChunk('reply 0');
    session.completeStream('end_turn');
    session.reset();

    session.startProcessing('msg 1');
    session.beginStreaming();
    session.appendAssistantChunk('reply 1');
    session.completeStream('end_turn');
    session.reset();

    // Items: [system_message, user_message, assistant_message, user_message, assistant_message]
    expect(session.items.length).toBe(5);

    // Cut before index 3 (keep items[3..4] + preserved system_message)
    session.truncateBefore(3);

    // Should have: system_message + user_message + assistant_message = 3 items
    expect(session.items.length).toBe(3);
    expect(session.items[0].type).toBe('system_message');
    expect((session.items[0] as any).text).toBe('System info');
    expect(session.items[1].type).toBe('user_message');
    expect((session.items[1] as any).text).toBe('msg 1');
    expect(session.items[2].type).toBe('assistant_message');
  });

  // -------------------------------------------------------------------------
  // 27. truncateBefore bumps version and notifies listeners
  // -------------------------------------------------------------------------
  test('truncateBefore bumps version and notifies listeners', () => {
    const session = makeSession();
  ...
  // -------------------------------------------------------------------------
  // 28. compaction does not trigger when usage is below threshold
  // -------------------------------------------------------------------------
  test('compaction does not remove items when usage is below threshold', async () => {
    const session = makeSession();
    ...
    // 12 existing + 2 new = 14 (no compaction occurred)
    expect(session.items.length).toBe(14);
  ...
  // -------------------------------------------------------------------------
  // 29. compaction with minimal conversation (2 turns) does not cut
  // -------------------------------------------------------------------------
  test('compaction with only 2 turns cuts minimally when threshold exceeded', async () => {
    const session = makeSession();
    ...
    // Compaction triggers. 3 user messages total (2 original + 1 from submitPrompt).
    // keepTurns = max(2, floor(3/2)) = 2, cutIndex = userMessageIndices[3-2] = index of 2nd user msg.
    // So the first user-assistant turn gets cut, keeping last 2 user-assistant pairs.
    const status = controller.getCompactionStatus();
    expect(status.compactionState).toBe('complete');
    // Original 4 + 2 new - 2 cut = 4 items remaining
    expect(session.items.length).toBe(4);
  });
});
