...
// 1. 基本水平排列
const _row = new Row({
  children: [new Text({ data: "左" }), new Text({ data: "中" }), new Text({ data: "右" })],
});

// 2. 主轴居中 + 等间距
const _centeredRow = new Row({
  mainAxisAlignment: "spaceEvenly",
  children: [new Text({ data: "A" }), new Text({ data: "B" }), new Text({ data: "C" })],
});

// 3. 弹性布局：1:2 比例分配
const _flexRow = new Row({
  children: [
    new Expanded({
      flex: 1,
...
// 6. 嵌套布局
const _nested = new Column({
  children: [
    new Row({
      mainAxisAlignment: "spaceBetween",
      children: [new Text({ data: "标题" }), new Text({ data: "操作" })],
    }),
    new Row({
      children: [new Expanded({ child: new Text({ data: "内容区域" }) })],
    }),
  ],
});
