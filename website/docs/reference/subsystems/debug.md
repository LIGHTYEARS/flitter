...
本页介绍 Flitter 的调试工具集：结构化日志系统、Widget 树 JSON 快照接口，以及 Unix socket REPL 运行时检查工具。这些工具帮助你在开发过程中诊断问题。

:::warning 高级内容
这些是开发和调试工具，不用于生产运行时。当你的 Widget 行为不符合预期、需要检查树结构或追踪事件流时，这些工具非常有用。
:::

**何时需要用到：**
- 需要排查 Widget 行为异常（如布局不对、事件不触发）时
- 需要在运行时检查 Widget 树的结构和 State 时
- 需要追踪帧调度、输入解析等内部流程时
