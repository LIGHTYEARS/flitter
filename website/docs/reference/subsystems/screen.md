...
本页介绍 Flitter 的屏幕缓冲模型——双缓冲单元格矩阵和 ANSI 差分渲染器。这是框架将 Widget 树最终转化为终端输出的最后一环。

:::warning 高级内容
大多数开发者不需要直接使用这些 API。Screen 子系统由框架内部自动管理。只有在开发自定义 RenderObject 或调试渲染问题时才需要了解这些内容。
:::

**何时需要用到：**
- 编写自定义 `RenderObject` 需要直接操作 Screen buffer 绘制内容时
- 调试渲染输出（如字符被截断、样式不正确）时
- 需要控制颜色深度或光标形状时
