...
本页介绍 Flitter 的两级输入解析管线：底层 VtParser 将原始字节流解析为 VT 事件，上层 InputParser 将 VT 事件转换为面向 Widget 层的语义化输入事件（键盘、鼠标、粘贴等）。

:::warning 高级内容
大多数开发者不需要直接使用这些 API。输入解析由框架自动完成，Widget 层通过 `Focus.onKey`、`MouseRegion.onClick` 等高级接口接收事件即可。只有在需要处理自定义终端序列或调试输入问题时才需要了解这些内容。
:::

**何时需要用到：**
- 需要理解终端键盘/鼠标事件的底层编码方式时
- 调试「按键没反应」或「鼠标位置不对」等输入问题时
- 实现自定义的输入预处理逻辑时
