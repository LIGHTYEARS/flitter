...
本页涵盖 Flitter TUI 中文本渲染及字符宽度计算相关的全部组件。无论是显示一行简单文本，还是构建多样式的富文本段落，都需要用到这里介绍的 Widget。

:::tip 快速参考：最常用文本 Widget
- **Text** -- 显示单一样式的纯文本，最常用（类似于 HTML `<span>`）
- **RichText + TextSpan** -- 需要在同一段落中混合多种样式时使用
- **textWidth()** -- 计算文本在终端中占据的列数（处理中文、Emoji 等宽字符）
:::
...
**何时使用：** 显示一段单一样式的文本。这是最常用的文本 Widget。

:::info 类似于 HTML span
`Text` 相当于 HTML 的 `<span>` 标签 —— 内联显示一段文本，可选设置颜色和加粗等样式。
:::
...
**何时使用：** 需要在同一段文本中混合多种样式（如关键字高亮、错误标记等）时使用。
...
**何时使用：** 构建 `RichText` 的内容树时使用。每个 `TextSpan` 节点可以携带不同的样式，通过嵌套实现复杂的样式混排。
...
**何时使用：** 需要精确计算文本在终端中占据多少列宽度时使用——例如对齐表格列、截断过长文本、或计算光标位置。
